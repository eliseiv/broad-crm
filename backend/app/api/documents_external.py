...
from app.api.deps import DocumentServiceDep, KnowledgeBotLinkServiceDep
...
async def get_user_access(
    ...
    """Резолв пользователя CRM по telegram id (единый порядок ADR-076 §2).

    knowledge → sms → mail → bootstrap по `username`. Не найден/неактивен/системный →
    404 user_not_linked. `sees_all_documents` — полный каталог прав роли.
    """
    response.headers["Cache-Control"] = _NO_STORE
    return await service.user_access(telegram_user_id, username)

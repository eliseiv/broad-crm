...
class MailIngestService:
    ...
    async def link_oauth_account(
        self,
        payload: MailOauthIngestRequest,
        *,
        team_id: uuid.UUID | None,
        initiator_user_id: uuid.UUID | None,
    ) -> None:
        """Upsert каталожной записи Outlook-ящика по уведомлению агрегатора (ADR-045 §3).

        `team_id` уже извлечён из проверенного `crm_state` (роутер) и **детерминированно**
        перезаписывает `mail_accounts.team_id` (re-consent). Идемпотентно (`ON CONFLICT
        (id) DO UPDATE` по агрегаторскому `mail_account_id`). CRM пишет только каталожную
        запись — токены/креды Outlook живут в агрегаторе. Аудит — лог-based, без секретов.
        """
        session = self._session
        accounts = MailAccountRepository(session)
        async with session.begin():
            await accounts.upsert_catalog(
                account_id=payload.mail_account_id,
                email=payload.email,

...
Устроена по образцу `proxies` (модель со статусом + фоновый монитор), но проще:
секрета нет (Fernet не задействован); идентификатор `code` УНИКАЛЕН
(`409 backend_code_taken` при дубле); проверка — прямой `GET https://{domain}/health`;
список единый (без группировки). Домен хранится нормализованным (`host[:port]`,
без схемы/пути) — инвариант CHECK `domain ~ '^[^\s/]+$'` (ADR-020).
"""

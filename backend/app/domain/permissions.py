...
CATALOG: dict[str, tuple[str, ...]] = {
    ...
    # `assign_any` — «Назначение любых ролей»: разрешает выдавать пользователю ЛЮБУЮ
    # роль, не расширяя собственный доступ владельца к страницам. Без него действует
    # анти-эскалация «роль не шире собственных прав», из-за которой PM не мог выдать
    # роль с правами, которых у него самого нет (прод-жалоба 2026-08-20). Разделение
    # намеренное: «управляю людьми» ≠ «имею все права этих людей».
    "users": ("view", "create", "edit", "delete", "assign_any"),
}

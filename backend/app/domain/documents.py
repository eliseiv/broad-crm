...
class DocumentScope:
    ...
    - `role_ids` — **набор** ролей пользователя (`user_roles`, ADR-079 §4). Узел виден ⇔
      он публичен внутри модуля (нет `restricted`-предка до корня) ИЛИ эффективный набор
      ролей узла (`document_node_roles` ближайшего `restricted`-предка) пересекается с
      `role_ids` (видимость по **любой** из ролей — прямое требование фичи).
      **Пустой набор → узел виден только если публичен** (public-only), а НЕ `500` и не
      «видно всё»: у консольного супер-админа своих строк в `user_roles` нет, а его
      доступ обеспечивает отдельный admin-предикат `sees_all`.
    """

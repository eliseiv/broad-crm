...
пароль прокси, полный ключ ИИ-ключа, Admin API key контура остатка ИИ-ключа
(ADR-070), API KEY и ADMIN API KEY бэка (ADR-040)). Секрет присутствует ТОЛЬКО
в этой схеме — никогда в общих list/detail-ответах (ADR-035).
"""

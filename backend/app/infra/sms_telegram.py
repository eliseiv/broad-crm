"""Клиент SMS-delivery Telegram-бота (modules/sms, ADR-030 §4).

**Отдельный бот** (токен `SMS_TELEGRAM_BOT_TOKEN`) — доставка входящих SMS
операторам в личку. НЕ пересекается с notifier-ботом (`app/infra/telegram.py`,
ADR-009), который НЕ трогается. `sendMessage` c `reply_markup`, `setWebhook`,
`setMyCommands`. TLS verify включён (httpx, дефолт). Токен не логируется.

Типизированные ошибки: `TelegramForbiddenError` (403 / bot blocked / chat not
found — привязка мертва, `mark_dead`); `TelegramApiError` — прочее (retry).
"""
...
class SmsBotClient:
    """Обёртка над Bot API SMS-бота (отдельный токен, ADR-030 §4)."""
    ...
    async def send_message(
        ...
        *,
        ...
        """`sendMessage`. Успех → payload; иначе TelegramForbiddenError/TelegramApiError."""
    ...
    async def _call_method(self, method: str, payload: dict[str, Any]) -> dict[str, Any]:
    ...
    async def set_my_commands(self, commands: list[dict[str, str]]) -> dict[str, Any]:
        """`setMyCommands` — меню бота (только /start)."""

...
def test_revision_chain_0005_on_top_of_0004_single_head() -> None:
    # 0005 сидит поверх 0004; голова цепочки — последняя ревизия (0008_create_users_roles,
    # добавлена поверх 0007 фичей RBAC, ADR-021). Цепочка остаётся линейной
    # (одна голова). Проверка головы 0008 продублирована в test_users_roles_migration.
    script = ScriptDirectory.from_config(_alembic_config())

    assert script.get_heads() == ["0020_backends_domain_canon"]  # одна голова — линейна (ADR-038)

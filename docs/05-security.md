...
Порядок ключей каталога (= порядок строк матрицы в UI): `dashboard, servers, ai-keys, proxies, backends, backend-users, backend-economics, mail, sms, roles, teams, documents`. Порядок нормативен: он же — порядок строк матрицы прав в UI, и он же сверяется замороженным контракт-тестом каталога.

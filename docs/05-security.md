...
| `backend-users` | `view`, `edit` ([ADR-069](adr/ADR-069-backend-users-page-admin-contract.md)) |
| `backend-economics` | `view`, `edit` ([ADR-072](adr/ADR-072-crm-admin-api-v11-economics.md)) |
...
| `broadcast` | `view`, `send` ([ADR-076](adr/ADR-076-knowledge-bot-broadcast-and-admin-level.md)) |

Порядок ключей каталога (= порядок строк матрицы в UI): `dashboard, servers, ai-keys, proxies, backends, backend-users, backend-economics, mail, sms, roles, teams, documents, broadcast`. Порядок нормативен: он же — порядок строк матрицы прав в UI, и он же сверяется замороженным контракт-тестом каталога.

- **Страницы `backend-users` и `backend-economics`** — два **разных** ключа, а не один с двумя действиями ([ADR-069](adr/ADR-069-backend-users-page-admin-contract.md), [ADR-072](adr/ADR-072-crm-admin-api-v11-economics.md) §2). `backend-users`: `view` — список/карточка пользователя внешнего бэка, `edit` — admin-операции **над одним пользователем** (начисление токенов, выдача плана). `backend-economics`: `view` — каталог продуктов и тарифов списания, `edit` — **глобальное** изменение количества токенов продукта/тарифа, влияющее на все будущие покупки и генерации всех пользователей бэка. **`backend-economics` — НЕ алиас `backend-users:edit`**: держатель права «поправить баланс клиента» не обязан получать право «переписать цену всем»; роль с полным `backend-users:["view","edit"]` и без `backend-economics` получает `403` на всех путях `/api/backend-economics/*`. Обе страницы держат admin-ключ бэка (`admin_api_key_encrypted`, Fernet, [ADR-040](adr/ADR-040-backend-relations-secrets-reverse-lookup.md)) **только в памяти обработчика**: он уходит в бэк заголовком `X-Admin-Key` и **не попадает** во frontend, логи, аудит и URL. ⚠️ **Прежняя формулировка «обе страницы — прокси БЕЗ хранилища» более не действует** ([ADR-080](adr/ADR-080-backend-users-snapshot-and-api-costs.md) отменяет [ADR-069](adr/ADR-069-backend-users-page-admin-contract.md) §3 для `backend-users`): «Юзеры бэков» получила **Postgres-снимок** списка пользователей ([03-data-model.md](03-data-model.md#таблицы-снимка-юзеры-бэков-backend_user_snapshot_sources-backend_user_snapshots--adr-080)). **Секретов в снимке нет** — он хранит ровно те данные, что и раньше проходили транзитом. Для `backend-economics` норма «без хранилища» **в силе**. Заголовок **`X-Admin-Actor: crm:<user_uuid>`** — **заявление, а не аутентификация** (бэком не проверяется; строить на нём отчётность «кто менял» нельзя). Мутации — под `edit` + аудит `backend_admin_action` **после успеха бэка**.
...
- **Страница `documents`** ([ADR-059](adr/ADR-059-documents-module.md), [modules/documents](modules/documents/README.md)). Действия: `view` (страница/API `/api/documents/*` + дерево/узлы под per-node фильтром видимости), `create` (`POST /folders`/`/documents`/`/upload`/`/nodes/{id}/copy`), `edit` (`PATCH /nodes/{id}` — rename/content — и `PATCH /order`), `delete` (`DELETE /nodes/{id}` — soft-delete), **`share`** (смена видимости `PATCH /nodes/{id}/visibility` — отдельное чувствительное действие управления доступом, по образцу `mail:tags`/`sync` сверх CRUD; сюда же share-gated read-эндпоинты `GET /api/documents/role-refs` и `GET /api/documents/nodes/{id}/visibility`). См. [Видимость документов по ролям](#видимость-документов-по-ролям-нормативно-adr-059). Столбец `share` в матрице `/roles` специфицирован [ADR-076](adr/ADR-076-knowledge-bot-broadcast-and-admin-level.md) (закрытие [TD-068](100-known-tech-debt.md)).
- **Страница `broadcast`** ([ADR-076](adr/ADR-076-knowledge-bot-broadcast-and-admin-level.md), [modules/broadcast](modules/broadcast/README.md)). Действия: `view` (`GET /api/broadcasts/audience` + страница `/broadcast`), `send` (`POST /api/broadcasts`). Адресаты — активные несистемные ∩ выбранные роли ∩ активные `knowledge_bot_links`; дедуп по `telegram_user_id`. Текст 1–4096, без `parse_mode`. Пустой `KNOWLEDGE_BOT_TOKEN` → `503 knowledge_bot_not_configured` **только на POST** (`GET /audience` токен не проверяет). Частичный успех = `200`.
...
2. **Видимость по ролям (per-node)** — фильтр **внутри** модуля. У пользователя — **набор ролей** (`user_roles`, [ADR-079](adr/ADR-079-users-m2m-roles-full-name-telegram-table.md) §4; прежняя норма «ровно одна роль, `users.role_id`» **отменена**). Узел виден ⇔ он **публичен внутри модуля** ИЛИ его **эффективный набор ролей** пересекается с `role_ids` пользователя (`document_node_roles.role_id IN :role_ids`, SQLAlchemy `expanding`). **Пустой `role_ids` ⇒ видны ТОЛЬКО публичные узлы** (не `500` и не «видно всё») — так же читается супер-админ, чей доступ обеспечивает отдельный предикат `sees_all_documents`. Union ролей **расширяет** видимость — прямое следствие фичи, а не побочный эффект.
...
- **Admin-уровень видит всё:** `sees_all_documents = is_superadmin OR permissions_subset(full_catalog_permissions(), permissions)` — тот же admin-предикат, что «видит все SMS/почты» ([ADR-032](adr/ADR-032-sms-visibility-admin-full-catalog.md)); нового права не вводится. **`permissions` здесь — union прав ВСЕХ ролей актора** ([ADR-079](adr/ADR-079-users-m2m-roles-full-name-telegram-table.md) §2); то же относится к `sees_all_sms_teams` / `sees_all_mail_teams`.
...
- Страница **«Пользователи» (`users`) ВХОДИТ в каталог** (Спринт B): без неё выдать доступ к реестру не-админской роли было невозможно. Гейт эндпоинтов — **`require_admin_or("users", <action>)`**: пропускает `is_admin_level` **ИЛИ** право матрицы, то есть прежний admin-доступ сохранён полностью, а роль с `users:*` добавляется к нему. Эскалацию сдерживает **`UserService`**: непривилегированный актор не назначает роль с правами шире собственного union — **кроме случая, когда у него есть право `users:assign_any`** («Назначение любых ролей»). Оно введено после прод-инцидента 2026-08-20: правило «роль не шире собственных прав» запирало менеджера — PM с `users:create` мог выдать лишь 3 роли из 8, а чтобы завести тестировщика, обязан был бы сам получить доступ к «Продуктам и тарифам» и «Рассылке». Смешивать «управляю людьми» и «имею все права этих людей» неверно, поэтому право разделено: `assign_any` разрешает выдавать ЛЮБУЮ роль, **не** расширяя доступ владельца к страницам, выдаётся администратором осознанно и видно в матрице. Отказ без него называет конкретную роль (`403 role_above_actor`), а не безликое «Недостаточно прав». Прочие ограничения: актор не назначает роль шире своей, не редактирует/не удаляет admin-level пользователя и **не сбрасывает ему пароль** (сброс открывает беспарольный вход — [ADR-025](adr/ADR-025-passwordless-users-login-identifier-open-first-login.md) — и без этого запрета `users:edit` означал бы захват админской учётки). Прежняя формулировка «гейтится `require_admin` = `is_admin_level`» (**`is_superadmin OR "admin" ∈ roles OR permissions_subset(full_catalog, permissions)`**, `permissions` = union прав всех ролей — [ADR-076](adr/ADR-076-knowledge-bot-broadcast-and-admin-level.md) §4 в редакции [ADR-079](adr/ADR-079-users-m2m-roles-full-name-telegram-table.md) §2). Управление **ролями** (`/api/roles`) и **командами** (`/api/teams`) со Спринта A — под матрицей `roles:*`/`teams:*` ([ADR-022](adr/ADR-022-teams-nav-categories.md)). Оговорка: **создание/редактирование CRM-команд де-факто admin-only** — форма выбирает лидера/участников из `GET /api/users` (под `require_admin`), поэтому `teams:create`/`teams:edit` даёт полный контроль состава только вместе с admin-доступом; `teams:view` — полноценный просмотр. Осознанное следствие замыкания эскалации ([ADR-022](adr/ADR-022-teams-nav-categories.md#3-гейтинг-api-нормативно)), контракт `teams:*` не меняется.
...
`get_current_principal` декодирует JWT и **на каждый запрос** формирует `Principal(username, roles, permissions, is_superadmin, user_id, role_ids, …)` (поле `user_id` добавлено [ADR-030](adr/ADR-030-sms-module-full-merge.md) — см. [Расширение Principal](#расширение-principal-полем-user_id-нормативно); **`role: str` → `roles: tuple[str, ...]`** и **`role_id` → `role_ids: frozenset[uuid]`** — [ADR-079](adr/ADR-079-users-m2m-roles-full-name-telegram-table.md) §2):

- `superadmin=true` → полный доступ (`permissions` = полный каталог, все `require(...)` и `require_admin` проходят); `roles=("admin",)`, `role_ids=frozenset()`.
- иначе по `uid` грузятся `users` + **все** его роли (через `user_roles`, `lazy="selectin"`); если пользователь не найден **или** `is_active=false` → `401 unauthorized` (действующий JWT аннулируется **без пере-логина**). Иначе **`permissions = union_permissions(role.permissions for role in roles)`** — объединение по страницам с дедупом действий. Порядок `roles`/`role_ids` — `user_roles.created_at ASC, role_id ASC`.

> **JWT-claim `role` в enforcement НЕ участвует** ([ADR-079](adr/ADR-079-users-m2m-roles-full-name-telegram-table.md) §3): он информационный (имя первой роли) и устаревает сразу после `PATCH /api/users/{id}`. Ветка `"admin" ∈ roles` предиката `is_admin_level` читает **роли из БД**, а не claim. Гейтить что-либо по claim `role` **запрещено**.
...
- Фабрика `require_admin` → `403 forbidden`, если не `is_admin_level` (**`is_superadmin OR "admin" ∈ roles OR permissions_subset(full_catalog_permissions(), permissions)`**, где `permissions` — union прав всех ролей; редакция [ADR-079](adr/ADR-079-users-m2m-roles-full-name-telegram-table.md) §2 — прежняя запись `role=="admin"` описывала единственную роль и **более не действует**). Чистая функция — `app/domain/permissions.py` рядом с `permissions_subset`. Со Спринта A гейтит **только Users API** (`/api/roles`, `/api/teams`, каталог переведены на матрицу). Кириллическое «Админ» проходит **только** при полном покрытии текущего каталога. Тот же предикат отдаётся UI как `GET /api/auth/me.is_admin_level` ([ADR-078](adr/ADR-078-me-is-admin-level.md)) — UX-гейт `/users`; каталог ради этого гейта клиент **не** запрашивает. Граница безопасности — по-прежнему серверный `403`.
...
- **(а) subset:** для актора **без** `is_admin_level`, при `POST`/`PATCH /api/roles`: `permissions` роли ⊆ `permissions` актора (по каждой `page` набор `actions` — подмножество). Нарушение → `403 forbidden`. Носитель `is_admin_level` проходит всегда.
  > **Редакция [ADR-079](adr/ADR-079-users-m2m-roles-full-name-telegram-table.md) §2 (нормативно):** «`permissions` актора» = **union прав ВСЕХ его ролей**. По существу инвариант не ослабевает: актор уже обладает union'ом, поэтому выдать роли его подмножество — не эскалация. Проверка — **один** вызов `permissions_subset(child, union_permissions(актор))`; **поролевая** проверка («⊆ хотя бы одной роли актора») **запрещена** — она строже нормы и запретила бы законную операцию.
- **(б) защита `admin`:** роль `name == "admin"` меняет/удаляет **только** `is_admin_level`. Иначе → `403 forbidden`.
...
**Второй секрет того же ключа — Admin API key ([ADR-070](adr/ADR-070-ai-key-estimated-balance-monitor.md)).** Контур оценочного остатка ходит в Admin Cost API провайдера **отдельным** ключом (Organization → Admin keys), а не inference-ключом:

- Хранится в `ai_keys.billing_admin_key_encrypted` (`bytea`, Fernet, тот же `FERNET_KEY`), **опционален** (`NULL`, когда мониторинг остатка выключен). Задаётся полем `billing_admin_key` в теле `POST`/`PATCH`; пустое/отсутствующее значение в `PATCH` = «не менять» (симметрично `key`).
- **В list/detail-ответах не возвращается ни в каком виде** — даже `has_*`-флага нет; косвенный признак наличия — `balance_monitoring_enabled=true`. Раскрытие — только `GET /api/ai-keys/{id}/billing-admin-key` под `ai-keys:edit`; секрет не задан → `404 secret_not_set`.
- **Выключение мониторинга (`balance_monitoring_enabled=false`) удаляет секрет** (`billing_admin_key_encrypted := NULL`) — повторное включение требует ввести Admin key заново. Это осознанно: выключенный контур не должен хранить действующий admin-доступ к организации провайдера.
- Расшифровка — только in-memory перед HTTP-запросом к Cost API; уходит в заголовок (`Authorization: Bearer` / `x-api-key`) и **не логируется**. Радиус поражения этого ключа шире inference-ключа (доступ к биллингу организации) — тем строже правило «никогда в ответах и логах».
- В аудите reveal различается от inference-ключа: `resource_type="ai_key_billing_admin"` (см. [«Reveal секретов»](#reveal-секретов-по-требованию-adr-035)).
...
**Эндпоинты и расшифровка.** `GET /api/servers/{id}/ssh-password`, `GET /api/proxies/{id}/password`, `GET /api/ai-keys/{id}/key`, **`GET /api/ai-keys/{id}/billing-admin-key`** ([ADR-070](adr/ADR-070-ai-key-estimated-balance-monitor.md)), `GET /api/backends/{id}/api-key`, `GET /api/backends/{id}/admin-api-key` → `SecretRevealResponse {value}`. Расшифровка — `app/infra/crypto.decrypt_secret` в памяти обработчика непосредственно перед формированием ответа; plaintext не логируется и не сохраняется.
...
**Аудит (нормативно).** Каждый **успешный** reveal порождает структурированную запись лога `secret_revealed` со `structlog`-полями `actor` (`username`/`user_id` принципала), `resource_type` (`server`/`proxy`/`ai_key`/**`ai_key_billing_admin`**/`backend`), `resource_id`, `at` (timestamp). **Само значение секрета в лог НЕ пишется** (фильтр секретов). (У бэка два секрета `api_key`/`admin_api_key` на одном ресурсе — `resource_type="backend"` для обоих; конкретное поле в аудите не различается. **У ИИ-ключа два секрета различаются**: `ai_key` — inference-ключ, `ai_key_billing_admin` — Admin API key контура остатка ([ADR-070](adr/ADR-070-ai-key-estimated-balance-monitor.md)); контуры разные, поэтому и записи разные.) Это лёгкий аудит через логи; персистентная аудит-таблица действий пользователей остаётся [TD-001](100-known-tech-debt.md).

**Frontend (память).** Раскрытое значение хранится **только** в локальном стейте компонента, который его показал (`SecretRevealField`); **не** кладётся в TanStack Query-кэш / Zustand; скрывается по повторному клику и сбрасывается при размонтировании (закрытие модалки / сворачивание блока). Кнопка-глаз рендерится только при `<page>:edit` (для прокси — дополнительно только при `has_password=true`; для бэка — только при `has_api_key`/`has_admin_api_key`; строка **Admin API key** ИИ-ключа — только при `balance_monitoring_enabled=true`, [ADR-070](adr/ADR-070-ai-key-estimated-balance-monitor.md)).
...
| Admin API key ИИ-ключа (биллинг провайдера) | БД (`ai_keys.billing_admin_key_encrypted`) | **опционален** (только при включённом мониторинге остатка, [ADR-070](adr/ADR-070-ai-key-estimated-balance-monitor.md)); вводится через API, шифруется `FERNET_KEY`; в обычных ответах **отсутствует полностью** (нет даже `has_*`-флага); plaintext — только on-demand reveal под `ai-keys:edit` (`GET /api/ai-keys/{id}/billing-admin-key`, аудит `resource_type="ai_key_billing_admin"`); выключение мониторинга **стирает** секрет |
...
| `DOCUMENTS_API_KEY` | `.env` | статический ключ внешнего API документов (RAG, **GET** `/api/external/documents/*`) **и** write-регистрации ИИ-бота (`POST /api/external/knowledge-bot/link`); входящий `X-API-Key` сверяется constant-time (`hmac.compare_digest`); не в БД/логах/ответах/SPA/URL; ротация через деплой; пустой ⇒ внешний контур выключен (`503 documents_external_not_configured`) ([ADR-060](adr/ADR-060-documents-external-readonly-api-key.md), [ADR-076](adr/ADR-076-knowledge-bot-broadcast-and-admin-level.md), [modules/documents](modules/documents/README.md)) |
| `KNOWLEDGE_BOT_TOKEN` | `.env` | токен Telegram **ИИ-бота базы знаний** (значение = `BOT_TOKEN` соседнего `ba-knowledge-base`); класс `SMS_TELEGRAM_BOT_TOKEN`; CRM шлёт `sendMessage` сама; не в БД/логах/ответах/SPA/URL; ключ фильтра structlog — `knowledge_bot_token` (`backend/app/logging.py`); пустой ⇒ `POST /api/broadcasts` → `503 knowledge_bot_not_configured` ([ADR-076](adr/ADR-076-knowledge-bot-broadcast-and-admin-level.md)) |
...
| Утечка `DOCUMENTS_API_KEY` (чтение всего корпуса документов + регистрация линка ИИ-бота) | ключ обходит per-role фильтр (машина видит всё) на `/api/external/documents/*` (**только GET**); write — только `POST /api/external/knowledge-bot/link` (upsert линка, не мутация документов). Хранение только `.env` (не в БД/логах/ответах/SPA/URL, structlog-фильтр), HTTPS, ротация деплоем; сравнение constant-time; пустой ключ → внешний контур выключен (`503`) ([ADR-060](adr/ADR-060-documents-external-readonly-api-key.md), [ADR-076](adr/ADR-076-knowledge-bot-broadcast-and-admin-level.md)) |
| Спам/энумерация чатов через `POST /api/broadcasts` | гейт `broadcast:send`; адресаты только активные несистемные ∩ линки ИИ-бота; дедуп по `telegram_user_id`; ответ без списка chat_id и без текстов Telegram; текст без `parse_mode` (анти-инъекция разметки); пустой токен → `503` |
| Утечка `KNOWLEDGE_BOT_TOKEN` | только `.env`; не в ответах/логах/SPA/URL; тот же класс, что `SMS_TELEGRAM_BOT_TOKEN`; ротация синхронно с `BOT_TOKEN` соседнего деплоя |

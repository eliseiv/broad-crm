...
| `NOTIFIER_METRIC_WINDOW_SEC` | `90` | Окно наблюдения нотификатора (с): `max_over_time` для зоны CPU/RAM/SSD ([ADR-016](adr/ADR-016-notifier-max-over-window-zone.md)). Нормативно **≥ `NOTIFIER_POLL_INTERVAL_SEC`** (при `<` поднимается до poll_interval + warning). UI-карты не затрагивает |
...
| `BACKEND_USERS_SNAPSHOT_INTERVAL_SEC` | `900` | Период фонового воркера `BackendUsersSnapshotService` — снимок списка пользователей бэков (с, [ADR-080](adr/ADR-080-backend-users-snapshot-and-api-costs.md), [modules/backend-users](modules/backend-users/README.md)). Воркер стартует **всегда** (по образцу мониторов) и **не гейтится** ничем: без него страница «Юзеры бэков» покажет «Снимок формируется…» и останется пустой. `sleep` выполняется **после** завершения итерации ⇒ фактический период ≥ значения; наложения циклов нет. Фактический возраст данных виден оператору в поле `snapshot_at` |
| `BACKEND_USERS_SNAPSHOT_REVENUE_BATCH` | `2000` | Квота backfill экономики на **один бэк за цикл**: сколько карточек `GET {P}/users/{id}` довычитывается ради блока «Расходы API» ([ADR-080](adr/ADR-080-backend-users-snapshot-and-api-costs.md) §5). Пока backfill не завершён — `api_costs.partial = true` и UI помечает сумму как неполную; у бэков уровня v1 без блока `revenue` `partial` остаётся `true` **постоянно** (`revenue_supported IS FALSE`), а **бэк, добавленный в реестр между циклами воркера, поднимает `partial` до тех пор, пока у него не появится строка `backend_user_snapshot_sources`** — третий дизъюнкт формулы ([ADR-080](adr/ADR-080-backend-users-snapshot-and-api-costs.md) §5). **Ни один из трёх случаев не является регрессией деплоя.** Увеличение ускоряет холодный старт ценой нагрузки на бэк; `0` **не** отключает блок, а останавливает добор — отключения контура отдельным флагом нет |
| `BACKEND_USERS_SNAPSHOT_CONCURRENCY` | `2` | Сколько бэков воркер снимка обходит **параллельно** ([ADR-080](adr/ADR-080-backend-users-snapshot-and-api-costs.md), амендмент «бережность к источнику» 2026-08-20). **Снижено с 5 после прод-инцидента**, когда воркер собственной нагрузкой доводил бэки до `429`/`500`. Фоновой задаче спешить некуда: рост значения ускоряет цикл ценой пиковой нагрузки на источники |
| `BACKEND_USERS_SNAPSHOT_PAGE_DELAY_SEC` | `0.3` | Пауза между страницами обхода и между карточками добора экономики (с). **`0` выключает троттлинг.** Пауза берётся только при **закрытой** сессии БД — иначе она держала бы `idle in transaction` и выбирала пул ([TD-088](100-known-tech-debt.md)) |
| `BACKEND_USERS_SNAPSHOT_RETRY_ATTEMPTS` | `5` | Всего попыток upstream-вызова **внутри воркера** при `429`/`5xx`. `401`/`404`/`400`/`422` и транспортные ошибки **не ретраятся**. **Интерактивные пути API повторов не получают вовсе** — там ждёт человек. Исчерпание попыток даёт прежний исход: цикл бэка падает, `error_message`/`failed_at` пишутся, **снимок прошлого цикла цел** |
| `BACKEND_USERS_SNAPSHOT_RETRY_BASE_SEC` | `1.0` | База экспоненты backoff'а (`base * 2^attempt`), затем джиттер (итог — 50–100 % окна). Джиттер обязателен: без него соседние бэки, упавшие в лимит одновременно, вернулись бы к нему тоже одновременно |
| `BACKEND_USERS_SNAPSHOT_RETRY_CAP_SEC` | `30.0` | Потолок задержки. Им же ограничивается **числовой `Retry-After`** источника (`min(retry_after, cap)`): бэк лучше знает своё окно, но бесконечно ждать его воркер не станет |
...
| `DOCUMENTS_API_KEY` | `<external api key>` | **Секрет** (только env, класс `MAIL_API_KEY`): статический ключ внешнего контура документов для RAG (**GET** `/api/external/documents/*`) и write-регистрации ИИ-бота (`POST /api/external/knowledge-bot/link`) — [modules/documents](modules/documents/README.md), [ADR-060](adr/ADR-060-documents-external-readonly-api-key.md), [ADR-076](adr/ADR-076-knowledge-bot-broadcast-and-admin-level.md). Входящий `X-API-Key` сверяется constant-time (`hmac.compare_digest`); не в БД/логах/ответах/SPA/URL; ротация через деплой. Пусто → внешний контур выключен (`503 documents_external_not_configured`); внутренний API `/api/documents/*` под JWT продолжает работать. У соседнего ba-knowledge-base тот же секрет называется `CRM_API_KEY` |
| `KNOWLEDGE_BOT_TOKEN` | `123456:ABC-…` | **Секрет** (только env, класс `SMS_TELEGRAM_BOT_TOKEN`): токен Telegram ИИ-бота базы знаний. Значение = `BOT_TOKEN` соседнего `ba-knowledge-base`. Пусто → `POST /api/broadcasts` → `503 knowledge_bot_not_configured`. Ротация — синхронно в обоих деплоях. CRM шлёт `sendMessage` сама (egress `api.telegram.org`); HTTP процесса бота не вызывается. **Порядок с соседом:** этот инстанс с живым `POST /api/external/knowledge-bot/link` и миграцией `0037` — **до** выкатки ba-knowledge-base ADR-013; иначе бот не создаст линки (чужой 404 он не считает `user_not_linked`, но статус «Бот» не появится) |
...
#### Волна [ADR-079](adr/ADR-079-users-m2m-roles-full-name-telegram-table.md)/[ADR-080](adr/ADR-080-backend-users-snapshot-and-api-costs.md) — особенности выката (нормативно)

- **Релиз атомарный: frontend и backend выкатываются ВМЕСТЕ.** [ADR-079](adr/ADR-079-users-m2m-roles-full-name-telegram-table.md) ломает два контракта — `GET /api/auth/me` (`role: string` → `roles: string[]`) и Users API (`role_id`/`role_name` → `role_ids`/`roles`, ФИО, обязательный `telegram`). Частичный выкат оставит SPA неработоспособной на странице `/users` и на резолве прав. Порядок «миграции → приложение» уже гарантирован entrypoint'ом (`alembic upgrade head && uvicorn …`, [§Порядок запуска](#порядок-запуска)) — отдельного шага не нужно.
- **`downgrade` `0038_user_roles_m2m` — LOSSY.** Он восстанавливает колонку `users.role_id` по **первой** роли (`MIN(created_at)`); все дополнительные роли теряются безвозвратно. Поэтому при откате этой волны действует **fallback-путь** политики выше — **восстановление Postgres из pre-deploy-бэкапа**, а `alembic downgrade -1` допустим только если после выката роли никому не добавляли. Отдельный риск: пользователь **без** ролей (заведён прямым SQL) не даст восстановить `NOT NULL` — такую строку чинят вручную **до** отката.
- **`downgrade` `0040_backend_users_snapshot` — безопасен** и данных не теряет по смыслу: снимок производный, источник истины остаётся у бэков; на откатанной версии страница возвращается к live fan-out.
- **Новый фоновый воркер стартует всегда** (`BackendUsersSnapshotService`, [§Переменные окружения](#переменные-окружения)) и ничем не гейтится. **Первый цикл после деплоя даёт пустую страницу** «Юзеры бэков» с подписью «Снимок формируется…» — это штатное состояние, а не инцидент: полный обход крупного бэка занимает минуты, а backfill блока «Расходы API» — до ~2 суток (`api_costs.partial=true` всё это время; у бэков уровня v1 без блока `revenue` — постоянно, `revenue_supported IS FALSE`; у бэка, **добавленного в реестр между циклами**, — пока воркер не заведёт ему строку `backend_user_snapshot_sources`; всё три — ADR-080 §5, не инцидент). **Пока строк состояния нет ни у одного запрошенного бэка, `api_costs = null`, и блок «Расходы API» рендерится с прочерками `—`** (сверка `frontend/src/pages/BackendUsersPage.tsx` 2026-08-19) — это тоже штатный вид, а не сломанная вёрстка. В чек-листе после деплоя не считать регрессией ни пустой список, пока `snapshot_at === null`, ни прочерки в блоке расходов, ни поднятый `partial`.
- **Обязательных новых env нет — ни в волне, ни в хотфиксе 2026-08-20.** Все семь `BACKEND_USERS_SNAPSHOT_*` имеют рабочие дефолты (`INTERVAL_SEC=900`, `REVENUE_BATCH=2000`, `CONCURRENCY=2`, `PAGE_DELAY_SEC=0.3`, `RETRY_ATTEMPTS=5`, `RETRY_BASE_SEC=1.0`, `RETRY_CAP_SEC=30.0`); прод-`.env` править не обязано. Значения бережности (`CONCURRENCY`/`PAGE_DELAY_SEC`/`RETRY_*`) подняты **в коде**, а не в `.env` прода, именно чтобы фикс приехал деплоем без ручного шага.
- ⚠️ **Волна довела число фоновых задач в `lifespan` до девяти** (`asyncio.create_task` в `backend/app/main.py`, счёт по коду 2026-08-19). Пул соединений БД при этом **не сконфигурирован явно** — `backend/app/db.py::get_engine` работает на дефолтах SQLAlchemy: **[TD-088](100-known-tech-debt.md)**. Наблюдаемого исчерпания нет (воркеры берут короткие сессии на итерацию), но инвариант «воркеры + пиковая конкурентность HTTP ≤ `pool_size + max_overflow`» нигде не записан — учитывать **до** добавления десятого воркера и **до** перехода на несколько uvicorn-воркеров.

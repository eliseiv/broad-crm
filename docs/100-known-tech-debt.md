...
| TD-017 | Firewall `:9100` на remote-целях открывается вручную (вне плейбука) | Этап 1: настройка firewall целевого сервера — ответственность администратора ([09-provisioning.md](09-provisioning.md#сетевая-доступность-node_exporter-9100)) | Авто-открытие `9100` в Ansible-плейбуке (ufw/firewalld), restricted к IP CRM-сервера | medium |

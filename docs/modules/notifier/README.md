...
- Метрики: `MonitoringService.fetch_for_instances(instances, window_sec=..., offline_window_sec=...)` → `dict[instance -> InstanceMetrics]`. **Нотификатор** вызывает windowed-режим: CPU/RAM/SSD — `max_over_time` за `NOTIFIER_METRIC_WINDOW_SEC`; `online` — `max_over_time(up[offline_window]) == 1`, где `offline_window = NOTIFIER_OFFLINE_CONSECUTIVE_SCRAPES × PROMETHEUS_SCRAPE_INTERVAL_SEC` (default 5×15s). UI-карты — **без окна** (`window_sec=None`), мгновенные значения.
...
Offline-алерт только после **N подряд неуспешных scrape** Prometheus (default **5**, `NOTIFIER_OFFLINE_CONSECUTIVE_SCRAPES`). Окно: `W_offline = N × PROMETHEUS_SCRAPE_INTERVAL_SEC` (default 5×15s = **75s**).

```promql
max_over_time(up{matcher}[75s])   # 0 → offline; 1 → был хотя бы один успешный scrape
```

- **Одиночный таймаут scrape** (как на «Бэки Елисея» 27.07) **не** даёт offline-алерт — в окне остаётся успешный scrape.
- **Recovery** (`offline→online`) — при первом успешном scrape после серии провалов (как и раньше, [ADR-018](../../adr/ADR-018-notifier-windowed-offline-recovery-alert-log.md)).
- **Окно зон CPU/RAM/SSD** (`NOTIFIER_METRIC_WINDOW_SEC`, default 90s) **отделено** от offline-окна.
- UI/read-path (`window_sec=None`) — мгновенный `up`, без изменений.

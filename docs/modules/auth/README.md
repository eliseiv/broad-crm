...
- Каталог прав — серверная константа `app/domain/permissions.py::CATALOG`: `dashboard:[view]`; `servers`/`ai-keys`/`proxies`/`backends:[view,create,edit,delete]`; `mail:[view]`. Страница «Пользователи» — вне каталога. Отдаётся `GET /api/permissions/catalog` (`require_admin`).
- Модель БД — `roles`/`users` ([03-data-model.md](../../03-data-model.md#таблицы-roles-и-users-rbac)), миграция `0008_create_users_roles` (`down_revision=0007_create_backends`, сид роли `admin`).
...
- `require(page, action)` → `Principal` или `403 forbidden()` (новый в `app/errors.py`). Применить ко **всем** ресурсным роутерам вместо `_user: CurrentUser` (маппинг метод→действие — [04-api.md](../../04-api.md#rbac-и-enforcement-прав)).
- `require_admin` (`is_superadmin || role=="admin"`) → гейт Users/Roles/Permissions API.
- `GET /api/auth/me` расширить до `{username, role, is_superadmin, permissions}`.

### Users / Roles API
- `GET/POST/PATCH/DELETE /api/users` и `/api/roles` — схемы, поля и коды ошибок строго по [04-api.md](../../04-api.md#users) / [04-api.md](../../04-api.md#roles) (пароль только на вход; `permissions` валидируется против каталога → `422`; удаление роли с пользователями → `409 role_in_use`; уникальность `username`/`role.name` → `409`).

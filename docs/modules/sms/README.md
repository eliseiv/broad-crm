...
> **Fan-out Telegram-уведомлений доп-команды НЕ расширяют** ([ADR-055](../../adr/ADR-055-per-channel-teams-mail-sms.md) §7): получатели по-прежнему = участники команды по **`user_teams`** (`recipients_for_team`). Осознанная асимметрия «вижу и обрабатываю ≠ получаю уведомления».

...
Функции Этапа 1:
- **Вкладка «Сообщения»** — лента входящих SMS (newest-first, keyset-курсор). Карточка: `from_number → to_number` + бейдж команды (зелёная пилюля / серая «Команды нет») + дата; пилюли `Логин:`/`Приложение:`/`Примечание:`; текст SMS. Два фильтра (взаимо-**комбинируемые**): `ui/Combobox` «Все номера» (ввод сужает список, сортировка по номеру) + `Select` «Все команды».
...
- **Telegram-доставка** — бот (`SMS_TELEGRAM_BOT_TOKEN`): fan-out входящего SMS по **явному SMS-доступу** получателя ([ADR-055](../../adr/ADR-055-per-channel-teams-mail-sms.md) §7): `user_teams` ∪ доп-команды канала `sms` + флаг «Без команды»; retry pending/failed фоновым монитором, dead-link при `403`. Роль Админ / `sees_all_sms_teams` сами по себе пуш **не** дают. Привязка оператора — Mini App `/tg/sms` ([ADR-031](../../adr/ADR-031-sms-operator-mini-app.md)).
- **Доработка `/teams`** — `number_count` на карточке команды + список номеров команды (`GET /api/teams/{id}/numbers`) в detail-панели; строка номера показывает `login`/`app_name` ([ADR-034](../../adr/ADR-034-teams-number-login-app.md)).
...
- **Ручное создание номера** — строки `sms_phone_numbers` появляются **только** через `POST /api/sms/numbers/sync` (`bulk_upsert_unassigned` — `backend/app/repositories/sms_number_repository.py:138-163`); **входящее SMS номер НЕ заводит** (`backend/app/services/sms_ingest_service.py:120-130` — пишется только `sms_inbound`). Действие `create` в RBAC-каталоге `sms` отсутствует ([ADR-030](../../adr/ADR-030-sms-module-full-merge.md); формулировка «номера появляются из входящих SMS» — устарела, [ADR-055](../../adr/ADR-055-per-channel-teams-mail-sms.md) §3.1).
...
- **Схемы** (`app/schemas/sms.py`) — Pydantic-модели контракта [04-api.md#sms](../../04-api.md#sms): `SmsMessageItem`/`SmsMessagesResponse`, `SmsNumberItem`/`SmsNumbersResponse`, `SmsNumberRef`, `SmsTeamRef`, `SmsNumberUpdateRequest` (presence-семантика `login`/`app_name`/`note`), `SmsNumberTransferRequest`, `SmsSyncResult`, `TelegramLinkRequest`/`TelegramLinkResponse`, `TelegramAuthRequest`/`TelegramAuthResponse`, `TeamNumberItem`/`TeamNumbersResponse` (сужённая схема для teams-detail: `id`/`phone_number`/`team` + `login`/`app_name`, **без** `note`/`label` — [ADR-034](../../adr/ADR-034-teams-number-login-app.md); см. §Доработка `/teams`).
- **Репозитории** (`app/repositories/sms_*.py`) — порт `app/infrastructure/repositories.py` донора: `SmsNumberRepository`, `SmsInboundRepository`, `SmsDeliveryRepository`, `SmsTelegramLinkRepository` (`recipients_for_team` = `user_teams` ∪ `user_channel_teams[sms]` + живой линк; `recipients_for_unassigned` = `sms_includes_unassigned` + живой линк — [ADR-055](../../adr/ADR-055-per-channel-teams-mail-sms.md) §7).
...
- `GET /api/teams/{id}/numbers` — номера команды для detail-панели `/teams`, `require("teams","view")`. Отдаёт `TeamNumberItem` (`id`/`phone_number`/`team` + `login`/`app_name` — [ADR-034](../../adr/ADR-034-teams-number-login-app.md)) — **без** `note`/`label` (остаются сужёнными, [ADR-030](../../adr/ADR-030-sms-module-full-merge.md) §8; полный контекст — только под `sms:*`).
...
`Principal` несёт **`user_id: UUID`** (не-опционален с [ADR-051](../../adr/ADR-051-superadmin-db-anchor-personal-state.md)). **`SmsScope(sees_all_teams, team_ids, includes_unassigned)`** (фабрика `get_sms_scope` в `deps.py`; поле `includes_unassigned` добавлено [ADR-055](../../adr/ADR-055-per-channel-teams-mail-sms.md)):
- **admin-уровень** (`sees_all_teams = is_superadmin ИЛИ роль владеет полным каталогом прав` — `permissions_subset(full_catalog_permissions(), permissions)`; консольный супер-админ, seed-`admin`, кастомная «Админ» с полным каталогом) → видит **все** SMS/номера; опц. фильтры `number_id`/`team_id`/`no_team` применяются как есть. Признак **не изменён** — устойчив к переименованию роли, без нового права/миграции.
- **прочие роли** (неполный каталог: PM, «Пользователь» и т.п.) → **`team_ids` = `user_teams` ∪ доп-команды канала `sms`** (`user_channel_teams`, [ADR-055](../../adr/ADR-055-per-channel-teams-mail-sms.md); прежнее «только `user_teams`» расширено) + флаг **`includes_unassigned`** (`users.sms_includes_unassigned`). **Предикат (един для чтения и мутации):** `number.team_id ∈ team_ids` **OR** (`includes_unassigned` **AND** `number.team_id IS NULL`) — по **текущей** принадлежности номера, не по снимку. SMS — только на видимые `to_number`. Запрос `number_id`/`team_id`/`no_team` вне scope → **пустой результат** (анти-энумерация, не `403`/`404`); **мутация** вне scope → `403`. Правило симметрично для сообщений и номеров.

**Снимок `sms_inbound.team_id`** пишется на момент приёма (определяет получателей fan-out) и обнуляется `ON DELETE SET NULL` при удалении команды; **для отображения** (бейдж команды, пилюли) карточка использует **текущий** номер (`sms_phone_numbers` по `to_number`). SMS на unassigned-номер (`team_id IS NULL`) или на удалённый номер видны actor'у admin-уровня (`sees_all_teams`) **и** носителю `sms_includes_unassigned` ([ADR-055](../../adr/ADR-055-per-channel-teams-mail-sms.md); прежде — только admin).

> **Fan-out Telegram-уведомлений следует явному SMS-доступу** ([ADR-055](../../adr/ADR-055-per-channel-teams-mail-sms.md) §7, амендмент 2026-08-11): получатели = `user_teams` ∪ `user_channel_teams[sms]` (для номера с командой) либо носители `sms_includes_unassigned` (для номера без команды). Admin-роль / полный каталог прав сами по себе получателя **не** делают.

### Перенос номера — целевая команда обязана быть в scope (нормативно, [ADR-055](../../adr/ADR-055-per-channel-teams-mail-sms.md) §3.2)

`POST /api/sms/numbers/{id}/transfer` (гейт `sms:transfer`) — **три** проверки, в этом порядке:

1. **Номер** проходит предикат scope (выше) → иначе `403 forbidden`.
2. **`team_id = null`** (снять команду): admin-уровень — всегда; **не-админ — только при `sms_includes_unassigned = true`**, иначе `403 forbidden`. Без флага актор безвозвратно выбросил бы номер из своего scope (прежний [TD-060](../../100-known-tech-debt.md) — **закрыт** этой нормой).
3. **`team_id = <uuid>`**: **не-админ** — целевая команда обязана ∈ `SmsScope.team_ids` (базовые ∪ доп-команды) → иначе **`403 forbidden`**; проверка scope **первая**, поэтому несуществующая команда не-админу тоже даёт `403` (анти-энумерация). **Admin-уровень** — существование → `404 sms_team_not_found`.

> **⚠️ Разворот прежней нормы (оформлен явно).** Прежде целевая команда валидировалась **только на существование** (`backend/app/services/sms_number_service.py:94-97`), а scope целевой команды не проверялся ⇒ носитель `sms:transfer` мог вынести попавший в его scope номер в **любую** существующую команду, включая ту, где не состоит — обходной путь, симметричный тому, ради запрета которого перенос **ящика** оставлен admin-only ([ADR-044](../../adr/ADR-044-mail-full-merge-into-crm.md) §4).

### ⚠️ Риск флага «Без команды» в СМС (нормативно, [ADR-055](../../adr/ADR-055-per-channel-teams-mail-sms.md) §3.1)

Бесхозные **номера** — не редкость, как бесхозные ящики, а **штатный массовый поток**: `POST /api/sms/numbers/sync` вставляет **все** входящие номера Twilio-аккаунта как unassigned (`SmsNumberRepository.bulk_upsert_unassigned` — `backend/app/repositories/sms_number_repository.py:138-163`; вызов — `sms_sync_service.py:57`), команда назначается позже вручную (`transfer`).

> **`sms_includes_unassigned = true` = доступ ко ВСЕМУ ещё не распределённому потоку номеров** и всем SMS на них — с **полными** правами роли: правка (`sms:edit`), **удаление** (`sms:delete`), **перенос в свою команду** (`sms:transfer`). Каждый новый номер из синхронизации попадает к носителю флага **автоматически**.

Последствие **принято владельцем осознанно** (read-only-режим для бесхозных **отвергнут**). Митигация — обязательная UI-подсказка объёма у чекбокса «Без команды» в блоке «СМС» формы пользователя ([08-design-system.md](../../08-design-system.md#блоки-смс-и-почты-в-форме-пользователя-нормативно-adr-055)) и запись риска в [05-security.md](../../05-security.md#риск-флага-без-команды-в-смс-нормативно-adr-055-31). Асимметрия с почтой (бесхозные ящики заводит **только админ**, автоматического источника нет) — свойство модели.
...
4. Резолв получателей по явному SMS-доступу: при `team_id` — `recipients_for_team` (`user_teams` ∪ `user_channel_teams[sms]` + живой линк); при `team_id IS NULL` — `recipients_for_unassigned` (`sms_includes_unassigned=true` + живой линк). Админ-видимость «видит всё» получателей не добавляет.
...
5. Mini App сохраняет `access_token` в памяти auth-store → показывает **две вкладки «Сообщения» (дефолт) / «Номера»** ([ADR-037](../../adr/ADR-037-mini-app-tabs.md)) с лентой `GET /api/sms/messages` и списком `GET /api/sms/numbers` под этим JWT и `sms:view` (SMS-scope до команд оператора, §Видимость). **Статус-блок привязки** («Telegram привязан — …» + бейдж «Привязан») **не рендерится** ([ADR-037](../../adr/ADR-037-mini-app-tabs.md)); линк технически сохраняется. Фильтры «Все номера/команды» в Mini App не показываются.
...
- Оператор должен иметь **явный SMS-доступ** к команде номера (базовое членство или доп-команда канала СМС) либо флаг «Без команды» для unassigned — иначе fan-out его не достигнет. Роль Админ сама по себе не достаточна.
...
- **Состояния:** loading (SSO) · **успех** (JWT получен: **две вкладки «Сообщения» (дефолт) / «Номера»** под `sms:view`, [ADR-037](../../adr/ADR-037-mini-app-tabs.md); **без** статус-блока привязки) · `403 sms_operator_not_provisioned` (экран «Ваш Telegram не сопоставлен с оператором CRM — обратитесь к администратору») · `401 invalid_init_data`/`init_data_expired` (сообщение «Сессия Telegram устарела — откройте заново через бота») · «вне Telegram» (пустой/битый `initData` — подсказка «Откройте по кнопке бота в Telegram») · сетевая ошибка (сообщение + повтор). **Пароль/форма входа отсутствуют** (беспарольный SSO).
- **Вкладки «Сообщения»/«Номера» ([ADR-037](../../adr/ADR-037-mini-app-tabs.md)):** дефолт — «Сообщения» (лента `useSmsMessages` под SSO-JWT/scope), вторая — «Номера» (`useSmsNumbers`). Фильтры «Все номера/команды» в Mini App не показываются (оператор видит только своё). Информационный блок привязки убран (линк технически сохраняется).
- **Просмотр номеров/сообщений (`sms:view`):** реюз хуков `useSmsNumbers`/`useSmsMessages` (те же JWT-эндпоинты, SMS-scope) под полученным `access_token`. Без `sms:view` — вкладки пусты (**без** статус-блока привязки).
...
- **Вкладка «Сообщения»:** фильтр номера — `ui/Combobox` `mode='select'` («Все номера» `pinned`, опции из `useSmsNumbers`, **сортировка по `phone_number`**, ввод сужает список; ленту меняет только выбор — по аналогии с «Почта»/[ADR-052](../../adr/ADR-052-mail-mailbox-combobox.md) §2); **«Все команды»** — `ui/Select`, рендер и опции по [единому правилу пяти экранов](../../08-design-system.md#фильтр-команда--единое-правило-на-пяти-экранах-нормативно-adr-055): порог «≥ 2 доступных варианта канала»; **источник опций — `me.sms_teams` из `GET /api/auth/me` у ЛЮБОГО актора** (у admin-уровня там все команды системы), опция «Без команды» — при `me.sms_includes_unassigned`) + лента `SmsMessageCard` + `IntersectionObserver`-догрузка (`useInfiniteQuery`, курсор `cursor`). Фильтры комбинируемы (AND). Состояния loading/empty/error.

> **⚠️ Источник команд канала на клиенте — не только для фильтра ([ADR-055](../../adr/ADR-055-per-channel-teams-mail-sms.md) §6.3).** `Select` **переноса номера** в колонке «Команда» таблицы «Номера» (гейт `sms:transfer` — **бывает у не-админа**) сегодня наполняется из `useTeams(seesAllTeams)` (`frontend/src/pages/SmsPage.tsx:71`) ⇒ у не-админа список **пуст** (эндпоинт `GET /api/teams` под `teams:view`). Нормативно: источник — **`me.sms_teams`** (`GET /api/auth/me`) у **любого** актора, включая admin-уровень (ровно те команды, куда перенос разрешён по §3.2); `GET /api/teams` здесь не запрашивается. Опция **«Без команды»** (снятие) — только при `me.sms_includes_unassigned === true`. **Вариант, который пользователь не вправе выбрать, не ПРЕДЛАГАЕТСЯ к выбору** ([ADR-055](../../adr/ADR-055-per-channel-teams-mail-sms.md) §6.3). **Единственная допустимая добавка вне `me.sms_teams` — «зеркало текущего состояния»** (§6.3.1): если текущая команда номера почему-то отсутствует в `me.sms_teams`, она добавляется в опции **оборонительно** (`frontend/src/components/SmsNumberRow.tsx:89-91`), иначе `<select>` показал бы **чужую** команду; её выбор равен текущему значению ⇒ обработчик выходит **без запроса** (`SmsNumberRow.tsx:95`). По scope (§3.2) такая ситуация **недостижима** — это страховка от рассинхрона `/me` и списка номеров, а **не** право добавлять в контрол иные опции.
- **Вкладка «Номера»:** поиск `Input` (клиентский фильтр по номеру) + таблица `SmsNumberRow` с инлайн-полями (`Pencil` → `Input`/`Textarea` + `Check`/`X`). **Колонка «ДЕЙСТВИЯ» убрана ([ADR-033](../../adr/ADR-033-flat-nav-theme-toggle-numbers-table.md)):** перенос — в колонке **«Команда»** (`Select` коммитит перенос сразу при выборе, без кнопки «Перенести»); удаление — одиночная иконка `Trash2` (danger) в последней **колонке без заголовка** (confirm-модалка не меняется). **Карандаш инлайн-полей виден всегда** (не по hover). Пагинации нет. Состояния loading/empty/error/«ничего не найдено». Нормативная раскладка таблицы — [08-design-system.md#вкладка-номера](../../08-design-system.md#вкладка-номера).
...
- [ ] Приватные эндпоинты `GET /api/sms/messages`, `GET /api/sms/numbers`, `PATCH`/`POST .../transfer`/`DELETE /api/sms/numbers/{id}`, `POST /api/sms/numbers/sync`, `POST /api/sms/telegram/link`, `GET /api/teams/{id}/numbers` — схемы/коды строго по [04-api.md#sms](../../04-api.md#sms); RBAC-гейты `sms:view/edit/transfer/sync/delete`. `GET /api/teams/{id}/numbers` отдаёт `TeamNumberItem` (`id`/`phone_number`/`team` + `login`/`app_name` — [ADR-034](../../adr/ADR-034-teams-number-login-app.md), **без** `note`/`label`) под `teams:view`; полный `SmsNumberItem` — только под `sms:*`.
...
- [ ] **Per-channel scope СМС ([ADR-055](../../adr/ADR-055-per-channel-teams-mail-sms.md)):** `SmsScope += includes_unassigned`, `team_ids = user_teams ∪ user_channel_teams['sms']`; единый предикат применён и на чтении, и на мутации (бесхозный номер доступен **только** при `sms_includes_unassigned`); `GET /api/sms/messages += no_team`.
- [ ] **`POST /api/sms/numbers/{id}/transfer` — три проверки (§3.2):** номер ∈ scope → иначе `403`; `team_id=null` не-админу — **только при `sms_includes_unassigned`** (иначе `403`); `team_id=<uuid>` не-админу — целевая команда обязана ∈ `SmsScope.team_ids` (иначе `403`, **включая несуществующую** — анти-энумерация); admin-уровню несуществующая → `404 sms_team_not_found`. **[TD-060](../../100-known-tech-debt.md) закрыт.**
- [ ] **Фильтр «Команда» в Mini App `/tg/sms` у актора admin-уровня** (`sees_all_sms_teams=true`) **рендерится и СОДЕРЖИТ команды** (`me.sms_teams` = все команды системы, [ADR-055](../../adr/ADR-055-per-channel-teams-mail-sms.md) §5.1); `GET /api/teams` из Mini App **не вызывается**. Пустой контрол = дефект.
- [ ] Frontend: `Select` переноса номера и фильтр «Команда» наполняются **`me.sms_teams`** у **любого** актора (`GET /api/teams` не вызывается); опция «Без команды» — только при `me.sms_includes_unassigned` ([ADR-055](../../adr/ADR-055-per-channel-teams-mail-sms.md) §6.3). Единственная допустимая опция вне `me.sms_teams` — **зеркало текущей команды номера** (§6.3.1, оборонительно; выбор = no-op).
...
- [ ] Frontend: страница `/sms` (табы Сообщения/Номера), фильтры, инлайн-правка, перенос/удаление/синк, все состояния UI, словарь из [08-design-system.md](../../08-design-system.md#страница-смс); таблица «Номера» — **без колонки «Действия»** (перенос в колонке «Команда» авто-коммитом, удаление иконкой `Trash2`, карандаш виден всегда — [ADR-033](../../adr/ADR-033-flat-nav-theme-toggle-numbers-table.md)); `/teams` — `number_count` на карточке + detail-панель со списком номеров; светлая/тёмная тема (токены-переменные), без layout-регрессии `/teams` и `/mail`.
...
- 2026-08-11: **Фильтр «Все номера» на `/sms` «Сообщения» → `ui/Combobox` `mode='select'`** (по аналогии с «Почта»/[ADR-052](../../adr/ADR-052-mail-mailbox-combobox.md) §2). Ввод сужает выпадающий список; ленту меняет только выбор; опции отсортированы по `phone_number`. `Select` «Команда» не затронут.
- 2026-07-14 (амендмент того же ADR): **перенос номера гейтится по scope + риск флага «Без команды» зафиксирован явно** ([ADR-055](../../adr/ADR-055-per-channel-teams-mail-sms.md) §3.1/§3.2/§6.3). `POST /sms/numbers/{id}/transfer`: целевая команда не-админу обязана ∈ scope (прежде проверялось **только существование**) → `403`; `team_id=null` не-админу — только при `sms_includes_unassigned` ⇒ **[TD-060](../../100-known-tech-debt.md) закрыт**. Зафиксировано последствие `sms_includes_unassigned=true`: доступ ко **всему ещё не распределённому потоку** номеров из Twilio-синхронизации с полными правами (правка/удаление/перенос) — принято владельцем осознанно, митигация — UI-подсказка. `Select` переноса и фильтр «Команда» у не-админа наполняются из `me.sms_teams`, а не `GET /api/teams`.
- 2026-08-11: **SMS Telegram-fan-out = явный SMS-доступ** ([ADR-055](../../adr/ADR-055-per-channel-teams-mail-sms.md) §7 амендмент). Получатели: `user_teams` ∪ `user_channel_teams[sms]` для номера с командой; `sms_includes_unassigned` для номера без команды. Роль Админ / `sees_all_sms_teams` сами по себе пуш не дают. Закрывает кейс Анны (`anellie_sss`) и unassigned-номер `+12393595637`.
- 2026-07-14: **Отдельные команды для СМС у пользователя (per-channel scope)** ([ADR-055](../../adr/ADR-055-per-channel-teams-mail-sms.md), spec-ready). `SmsScope += includes_unassigned`; **`team_ids` = `user_teams` ∪ доп-команды канала `sms`** (`user_channel_teams`, миграция `0027`); бесхозный номер доступен не-админу **только** при `sms_includes_unassigned` (прежде — `403` всегда, `sms_number_service.py:124-127`). Доп-команда даёт **и видимость, и действия** (`sms:edit`/`transfer`/`delete`/`sync` — в пределах scope). Фильтр «Все команды» на `/sms` («Сообщения») и **в Mini App `/tg/sms`** рендерится при **≥ 2 доступных вариантах канала** (опции — из `GET /api/auth/me` у любого актора) ⇒ гейт «только admin» ([ADR-036](../../adr/ADR-036-sms-team-filter-admin-only.md)) и «фильтров в Mini App нет» ([ADR-037](../../adr/ADR-037-mini-app-tabs.md)) **отменены** (фильтр «Все номера» в Mini App по-прежнему нет). `GET /api/sms/messages += no_team`. Telegram fan-out на момент ADR — только `user_teams` (расширен амендментом 2026-08-11, §7). Найденная при разборе дыра — [TD-060](../../100-known-tech-debt.md) (снятие команды с номера не-админом = самоблокировка) — **закрыта тем же ADR** (§3.2); открытой записи в реестре не заводилось.
- 2026-07-09: **UI-пакет фиксов (architect, spec-ready):** (1) `TeamNumberItem += login`/`app_name` — detail-панель `/teams` показывает Логин/Приложение номеров ([ADR-034](../../adr/ADR-034-teams-number-login-app.md)); `note`/`label` остаются под `sms:*`. (3) Фильтр «Все команды» на `/sms` — ~~только admin-уровень (`me.sees_all_sms_teams`)~~ → **гейт «только admin» ОТМЕНЁН [ADR-055](../../adr/ADR-055-per-channel-teams-mail-sms.md) §6** (2026-07-14): рендер при **≥ 2 вариантах команд канала**, опции — из `GET /api/auth/me` (`sms_teams`). В силе: `GET /api/auth/me += sees_all_sms_teams` как производный backend-признак ([ADR-036](../../adr/ADR-036-sms-team-filter-admin-only.md)). (4) Операторская Mini App `/tg/sms` — вкладки «Сообщения» (дефолт) / «Номера», убран статус-блок привязки ([ADR-037](../../adr/ADR-037-mini-app-tabs.md)). Контракты SMS-эндпоинтов не меняются (кроме `TeamNumberItem` +2 поля и auth/me +1 поле).

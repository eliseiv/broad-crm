...
- Кнопка **«Добавить»** в правой зоне заголовка страницы ([ADR-046](../../adr/ADR-046-ui-infra-fix-pack.md) §2б; `AddProxyCard` **упразднена**) → `AddProxyModal` (Radix Dialog) в режиме **add**: поля **Название** (`Input`), **Тип** (`Select`: http/https/socks5), **Хост** (`Input`), **Порт** (`Input`, числовой), **Логин** (`Input`, опц.), **Пароль** (`Input type=password`, toggle видимости, опц.). Кнопки **Отмена** / **Добавить**.
...
- 2026-07-11: **UI-пакет** (architect, [ADR-046](../../adr/ADR-046-ui-infra-fix-pack.md); контракт/БД не затронуты): кнопка **«Добавить» → правая зона заголовка**, `AddProxyCard` удалена (empty → «Прокси пока нет»); **detail-модалка** показывает сразу только **Название**/**Хост**/**Порт**, а **Тип**/**Логин**/**Пароль** — в свёрнутом блоке **«Информация»**; **пустые поля не рендерятся** (строки «Логин: —»/«Пароль: —» упразднены). Алерты прокси перечнем бэков **не расширяются** (связи прокси↔бэк в модели нет). DnD на `/proxies` **сохраняется**.

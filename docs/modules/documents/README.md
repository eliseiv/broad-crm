...
2. **Видимость по ролям (per-node)** — фильтр **внутри** модуля. У пользователя — **набор ролей** (`user_roles`, [ADR-079](../../adr/ADR-079-users-m2m-roles-full-name-telegram-table.md) §4; прежняя норма «ровно одна роль, `users.role_id`» [ADR-021](../../adr/ADR-021-rbac-users-roles.md) **отменена**). Узел виден ⇔ он **публичен внутри модуля** ИЛИ его **эффективный набор ролей** пересекается с `role_ids` пользователя (`document_node_roles.role_id IN :role_ids`, SQLAlchemy `expanding`). **Пустой `role_ids` ⇒ видны ТОЛЬКО публичные узлы** (не `500` и не «видно всё»). `DocumentScope.role_id` → **`role_ids: frozenset`**; union ролей **расширяет** видимость — прямое следствие фичи.
...
- Найден ближайший `restricted`-узел `R` → **эффективный набор ролей** = `document_node_roles` узла `R`. Узел виден пользователю ⇔ этот набор **пересекается** с его `role_ids` ([ADR-079](../../adr/ADR-079-users-m2m-roles-full-name-telegram-table.md) §4; прежняя запись `role_id ∈ набор` описывала единственную роль).
...
- **`documents:share` выдаётся через UI** столбцом **«Видимость»** матрицы `/roles`: столбцы строятся из `GET /api/permissions/catalog`, не из зашитого CRUD ([ADR-076](../../adr/ADR-076-knowledge-bot-broadcast-and-admin-level.md)). [TD-068](../../100-known-tech-debt.md) закрыт.
...
| `GET /user-access/{telegram_user_id}?username=` | Резолв CRM-пользователя для ИИ-бота. Есть в коде; в docs закрыт дрейф ([ADR-076](../../adr/ADR-076-knowledge-bot-broadcast-and-admin-level.md)). Порядок: knowledge-линк → sms → mail → bootstrap по нику. `404 user_not_linked`. Write-регистрация линка — **не здесь**, а `POST /api/external/knowledge-bot/link` (роутер документов остаётся только GET). |
...
- **kebab-меню** (3 точки) на строке узла — обёртка над `@radix-ui/react-dropdown-menu` (**уже** в `package.json`, новой зависимости нет): «Удалить» (`delete`) / «Создать копию» (`create`) / «Переименовать» (`edit`) / «Сменить видимость» (`share`). Пункт рендерится ⇔ есть соответствующее право (UX-гейт; безопасность — сервер). **«Сменить видимость»** — `useCan('documents','share')` из `me.permissions` **или** `me.is_admin_level` ([ADR-078](../../adr/ADR-078-me-is-admin-level.md)). Если `/me.permissions.documents` содержит `share` — пункт **обязан** быть. Каталог ради share не запрашивается.
...
- Гейтинг пунктов/кнопок — по правам из `GET /api/auth/me` (`permissions` + `is_admin_level` для share-фолбэка, [ADR-078](../../adr/ADR-078-me-is-admin-level.md)); невидимые по роли узлы не приходят с сервера (фильтр backend).
...
- 2026-08-17 ([ADR-078](../../adr/ADR-078-me-is-admin-level.md), implemented, сверка `DocumentsPage.tsx`): UX-гейт «Сменить видимость» — `useCan('documents','share')` из `/me.permissions` + минимальный фолбэк `me.is_admin_level`; каталог не участвует.
- 2026-08-17 ([ADR-076](../../adr/ADR-076-knowledge-bot-broadcast-and-admin-level.md), сверка состава): внешний `GET …/user-access/{id}` с query `username`; резолв += `knowledge_bot_links` + bootstrap по нику; write — соседний префикс `/api/external/knowledge-bot/*`. Роутер документов — по-прежнему только GET.

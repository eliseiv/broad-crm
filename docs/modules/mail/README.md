...
- **Полнотекстовый поиск** по письмам — не реализован (данные в БД CRM есть, индекса/эндпоинта нет) — остаток [TD-024](../../100-known-tech-debt.md).
- Папки, пометка прочитано/непрочитано, архив.
...
- **Фильтры** `mail_account_id` (**повторяемый**) и `team_id` — **AND-комбинируемы**, пересекаются со scope пользователя.
...
- **`/tg/mail`** — Mini App: без заголовка, без таб-лейбла, лента напрямую; клик по письму → read-only full-text detail внутри того же webview; reply нет.

...
Зеркалит страницу `proxies`/`servers` (единый список карточек, клик=detail→карандаш=edit — [ADR-035](../../adr/ADR-035-detail-view-secret-reveal.md)), **но БЕЗ drag-and-drop** ([ADR-046](../../adr/ADR-046-ui-infra-fix-pack.md) §2а); детальный UI-гайд — [08-design-system.md](../../08-design-system.md#страница-бэки). Реализация строк — русский словарь ([08-design-system.md](../../08-design-system.md#локализация-страницы-бэки)).
...
- **Клик по карточке = read-only detail-модалка** `BackendDetailModal` ([ADR-035](../../adr/ADR-035-detail-view-secret-reveal.md), состав — [ADR-046](../../adr/ADR-046-ui-infra-fix-pack.md) §2в): **сразу видны Код / Название / Домен** (`https://host/`); **Сервер**, **ИИ-ключ**, **API KEY**, **ADMIN API KEY** (`••••` + глаз-reveal под `backends:edit`, если `has_*`), **Git**, **Примечания** ([ADR-040](../../adr/ADR-040-backend-relations-secrets-reverse-lookup.md)) — внутри **свёрнутого блока «Информация»**. **Пустые поля не рендерятся** (прочерк «—» упразднён — [ADR-046](../../adr/ADR-046-ui-infra-fix-pack.md) §3). Карандаш вверху справа → `AddBackendModal mode='edit'`. **Клик открывает detail НЕМЕДЛЕННО** (DnD убран → delay-жест не нужен). Кнопка **Удалить** — `stopPropagation`. Паттерн — [08-design-system.md](../../08-design-system.md#detail-view-карточных-страниц-read-only--карандаш--edit-adr-035).
...
- [ ] **UI-пакет ([ADR-046](../../adr/ADR-046-ui-infra-fix-pack.md) §2):** плоская сетка + сортировка по `name` (кластеры «name · N» и DnD **убраны**, разворот [ADR-039](../../adr/ADR-039-ui-server-inline-edit-backends-search-empty-sms-label.md)); кнопка «Добавить» в правой зоне заголовка (`AddBackendCard` удалена); empty → «Бэков пока нет»; detail-модалка — идентификаторы + свёрнутая «Информация», пустые поля не рендерятся. **Действует из [ADR-039](../../adr/ADR-039-ui-server-inline-edit-backends-search-empty-sms-label.md):** клиентский поиск по `code`/`name`/`domain` («Поиск по бэкам…», «Ничего не найдено»).

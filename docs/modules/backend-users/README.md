...
**Fan-out** по бэкам с admin-ключом, `Semaphore(5)` — тот же лимит, что у прежнего live fan-out. Алгоритм на один бэк:
...
7. **Любое исключение** → прошлый снимок **не трогается**, пишутся `error_message` / `failed_at`. Устаревшие данные с честной меткой возраста лучше пустого экрана.
   - ⚠️ **[Q-BU-2](../../99-open-questions.md):** постоянный отказ на одной и той же странице ⇒ вечное «Снимок формируется…» (`refreshed_at` не проставляется) при накапливающихся строках; backoff на `429`/`5xx` и признак «обход не завершён» — кандидаты, ни один не выбран.

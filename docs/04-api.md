...
| 409 | `email_taken` | Пользователь с таким `email` уже существует ([Users](#users)) |
...
| 409 | `user_is_team_leader` | Пользователь — лидер ≥1 команды, удаление запрещено ([Users](#users)) |
| 422 | `unprocessable` | Семантически некорректные данные (напр. невалидный IP; невалидный `username`; `permissions` вне каталога; несуществующий `role_id` при создании/правке пользователя — [Users](#users)/[Roles](#roles)) |
...
### POST `/api/auth/login`
Проверяет логин+пароль и возвращает JWT. **Две ветки (нормативно, [05-security.md](05-security.md#аутентификация-логин-и-выпуск-jwt)):** сперва `.env`-**супер-админ** (constant-time против `ADMIN_USER`/`ADMIN_PASSWORD` → JWT `role="admin"`, `superadmin=true`); иначе **БД-пользователь** (`users.username` + `verify_password` bcrypt + `is_active=true` → JWT `uid`, `role=role.name`, `superadmin=false`). Неудача любой ветки → единое `401 invalid_credentials`.
...
| Поле | Тип | Примечание |
|------|-----|-----------|
| `pages` | `PermissionCatalogPage[]` | Упорядоченный список страниц каталога (порядок = порядок строк матрицы в UI: dashboard, servers, ai-keys, proxies, backends, mail, roles, teams) |
...
Реестр дополнительных пользователей (супер-админ из `.env` сюда не входит). Модель — [03-data-model.md](03-data-model.md#таблицы-roles-и-users-rbac), решение — [ADR-021](adr/ADR-021-rbac-users-roles.md). Все эндпоинты — гейт `require_admin`. **Пароль (plaintext) никогда не возвращается** — только на вход.
...
| `username` | string | Логин (кириллица/юникод допускаются) |
| `email` | string \| null | Опциональный email ([ADR-022](adr/ADR-022-teams-nav-categories.md)); `null` — не задан |
...
Пароль (`password`/`password_hash`) в ответах **отсутствует** всегда. `teams` — CRM-команды ([Teams](#teams)), **не** mail-«команды».
...
```json
{ "username": "Никита", "email": "nikita@example.com", "password": "s3cret-pass", "role_id": "2a9f...c0", "team_ids": ["d3f0...a1"] }
```
| Поле | Тип | Правила |
|------|-----|---------|
...
| `email` | string? | **опц.** Отсутствует/`null`/`""` → без email. Задан → валидный формат email (нормализуется trim+lower); невалидный → `422 unprocessable` (`details[].field="email"`). Уникален среди заданных → дубликат `409 email_taken` |
...
**Response 201** — созданный `UserListItem` (с `email`/`teams`, без пароля).

**Прецеденция ошибок (нормативно):** схемная валидация (`400`/`422` — форма/`username`/`email`/`password`) → существование `role_id` и всех `team_ids` (`422 unprocessable`) → уникальность `username` (`409 username_taken`) → уникальность `email` (`409 email_taken`).

**Ошибки:** `401 unauthorized`, `403 forbidden`, `400 validation_error`, `422 unprocessable` (невалидный `username`/`email`/несуществующий `role_id`/`team_ids`), `409 username_taken`, `409 email_taken`.
...
```json
{ "email": "new@example.com", "role_id": "5b1e...aa", "is_active": false, "password": "new-pass-123", "team_ids": ["d3f0...a1", "e4a1...b2"] }
```
| Поле | Тип | Правила |
|------|-----|---------|
| `email` | string? | опц. Не передано → не менять; `null`/`""` → **убрать** email (`email=NULL`); валидный → установить (нормализация; дубль → `409 email_taken`; невалидный формат → `422`) |
...
| `password` | string? | опц. **Не передано → не менять**; передан непустой (8–128) → **сброс пароля** (re-hash bcrypt). Пустая строка `""` → `422 unprocessable` (не «очистка» — у пользователя всегда есть пароль) |
| `team_ids` | string[]? | опц. Не передано → членство не менять; передано → **полностью заменяет** набор CRM-команд пользователя (каждый `id` должен существовать → `422`; без дублей; `[]` → выйти из всех команд). Если пользователь — лидер команды, из которой его пытаются исключить, — команда сохраняет лидера как участника (инвариант «лидер ∈ участники», [Teams](#teams)); фактически лидер не исключается |

**Response 200** — обновлённый `UserListItem` (с `email`/`teams`).

**Ошибки:** `401 unauthorized`, `403 forbidden`, `404 user_not_found`, `400 validation_error`, `422 unprocessable` (несуществующий `role_id`/`team_ids` / невалидный `email`/`password`), `409 email_taken`.

### DELETE `/api/users/{id}`
Удаляет пользователя (hard delete). Гейт `require_admin`. Членства в командах (`user_teams`) снимаются `ON DELETE CASCADE`. **Если пользователь — лидер хотя бы одной CRM-команды** (`teams.leader_id`, `ON DELETE RESTRICT`) → `409 user_is_team_leader` (сначала сменить лидера/удалить команду).

**Response 204** (без тела).

> Удаление собственной учётки БД-администратором допустимо; его действующий JWT аннулируется на следующем запросе (`401`). Супер-админ (`.env`) не удаляется (его нет в таблице).

**Ошибки:** `401 unauthorized`, `403 forbidden`, `404 user_not_found`, `409 user_is_team_leader` (пользователь — лидер команды).
...
| `leader_id` | string (uuid) | ID пользователя-лидера |
| `leader_username` | string | Логин лидера (денормализовано для списка). Агрегат `JOIN users` |
| `member_count` | integer | Число участников (`= members.length`; включает лидера) |
| `members` | `TeamMember[]` | Участники команды (включая лидера). `TeamMember = { id: string(uuid), username: string }`. Отдаётся в списке для prefill формы редактирования (участников/команд немного, NFR-1) — отдельного `GET /api/teams/{id}` нет |
| `created_at` / `updated_at` | datetime | Метки |
...
### POST `/api/teams`
Создаёт команду. Гейт `require("teams","create")`. **Лидер автоматически добавляется в участники** (инвариант «лидер ∈ участники»).
...
| `leader_id` | string (uuid) | required. Пользователь-лидер должен существовать → иначе `422 unprocessable` (`details[].field="leader_id"`) |
| `member_ids` | string[]? | **опц.**, default `[]`. Участники (кроме лидера — он добавляется автоматически). Каждый `id` должен существовать → `422 unprocessable` (`details[].field="member_ids"`). Без дублей. `leader_id`, продублированный в `member_ids`, — не ошибка (идемпотентно) |

**Response 201** — созданный `TeamListItem` (`leader_id` присутствует в `members`, `member_count` включает лидера).

**Прецеденция ошибок (нормативно):** схемная валидация (`400`/`422` — форма/`name`) → существование `leader_id` и всех `member_ids` (`422 unprocessable`) → уникальность `name` (`409 team_name_taken`).
...
| `leader_id` | string (uuid)? | опц. Передано → сменить лидера (должен существовать → `422`). Новый лидер добавляется в участники автоматически |
| `member_ids` | string[]? | опц. Не передано → состав не менять; передано → **полностью заменяет** набор участников (каждый существует → `422`; без дублей). Лидер (текущий или новый `leader_id`) **всегда** включается в итоговый состав, даже если отсутствует в `member_ids` (инвариант «лидер ∈ участники») |

**Response 200** — обновлённый `TeamListItem`.

...
```json
{ "id": 7, "email": "inbox@example.com", "display_name": "5108 Klyro Forge (Codex)", "is_active": true }
```
| Поле | Тип | Примечание |
|------|-----|-----------|
| `id` | integer | ID ящика (`mail_accounts.id`) |
| `display_name` | string \| null | Производное имя ящика (`"<number> <app_name>"`, [ADR-047](adr/ADR-047-mail-fix-pack.md) §3); `null` — нет |
| `email` | string | Адрес ящика |

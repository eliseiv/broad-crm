...
| 403 | `forbidden` | Аутентифицирован, но нет права на действие/страницу ([RBAC](#users)) |
...
- **Users/Roles/Permissions API** — гейт `require_admin` (`is_superadmin || role=="admin"`), не через матрицу.
...
Отдаёт канонический каталог «страница × действия». Гейт `require_admin`.
...
| Поле | Тип | Примечание |
|------|-----|-----------|
| `pages` | `PermissionCatalogPage[]` | Упорядоченный список страниц каталога (порядок = порядок строк матрицы в UI) |

`PermissionCatalogPage = { page: string, actions: string[] }`. Страница `users` в каталог **не входит** (гейтится `require_admin`, не матрицей). Локализованные подписи страниц/действий — на стороне frontend ([08-design-system.md](08-design-system.md#страница-пользователи)).
...
```json
{ "username": "Никита", "password": "s3cret-pass", "role_id": "2a9f...c0" }
```
| Поле | Тип | Правила |
|------|-----|---------|
...
**Response 201** — созданный `UserListItem` (без пароля).

**Прецеденция ошибок (нормативно):** схемная валидация (`400`/`422` — форма/`username`/`password`) → существование `role_id` (`422 unprocessable`) → уникальность `username` (`409 username_taken`).

**Ошибки:** `401 unauthorized`, `403 forbidden`, `400 validation_error`, `422 unprocessable` (невалидный `username`/несуществующий `role_id`), `409 username_taken`.
...
```json
{ "role_id": "5b1e...aa", "is_active": false, "password": "new-pass-123" }
```
| Поле | Тип | Правила |
|------|-----|---------|
...
**Response 200** — обновлённый `UserListItem`.

**Ошибки:** `401 unauthorized`, `403 forbidden`, `404 user_not_found`, `400 validation_error`, `422 unprocessable` (несуществующий `role_id` / невалидный `password`).

### DELETE `/api/users/{id}`
Удаляет пользователя (hard delete). Гейт `require_admin`.
...
Реестр ролей с правами (`permissions`-матрица). Модель — [03-data-model.md](03-data-model.md#таблицы-roles-и-users-rbac), решение — [ADR-021](adr/ADR-021-rbac-users-roles.md). Все эндпоинты — гейт `require_admin`.
...
| `name` | string | Имя роли (уникально). `admin` — зарезервированное имя (доступ к «Пользователям») |
| `permissions` | object | `{ "<page>": ["<action>", ...] }`; ключи/действия — из каталога ([Permissions](#permissions)) |
| `created_at` / `updated_at` | datetime | Метки |

### GET `/api/roles`
Список ролей. Гейт `require_admin`.
...
### POST `/api/roles`
Создаёт роль. Гейт `require_admin`.
...
| `permissions` | object | required. Валидируется против каталога: ключи ∈ страниц каталога (кроме `users`), действия ∈ `CATALOG[page]`, без дублей. Нарушение → `422 unprocessable` (`details[].field="permissions"`) |

**Response 201** — созданный `RoleListItem`.

**Прецеденция ошибок (нормативно):** схемная валидация (`400`/`422` — форма/`name`/`permissions` вне каталога) → уникальность `name` (`409 role_name_taken`).

**Ошибки:** `401 unauthorized`, `403 forbidden`, `400 validation_error`, `422 unprocessable` (невалидный `name`/`permissions` вне каталога), `409 role_name_taken`.

### PATCH `/api/roles/{id}`
Редактирование роли (`name` и/или `permissions`-матрица). Гейт `require_admin`. Все поля опциональны; передаются только изменяемые. Правки прав применяются **без пере-логина** носителей роли (принципал грузится из БД на каждый запрос, [ADR-021](adr/ADR-021-rbac-users-roles.md#5-enforcement-сервер--единственная-граница-безопасности)).
...
| `permissions` | object? | опц. Передано → полностью заменяет матрицу прав (валидируется против каталога → `422`) |

**Response 200** — обновлённый `RoleListItem`.

**Ошибки:** `401 unauthorized`, `403 forbidden`, `404 role_not_found`, `400 validation_error`, `422 unprocessable` (невалидный `name`/`permissions` вне каталога), `409 role_name_taken`.

### DELETE `/api/roles/{id}`
Удаляет роль (hard delete). Гейт `require_admin`. **Запрещено удалять роль, назначенную хотя бы одному пользователю** (`ON DELETE RESTRICT`) → `409 role_in_use`.

**Response 204** (без тела).

**Ошибки:** `401 unauthorized`, `403 forbidden`, `404 role_not_found`, `409 role_in_use`.

...
2. **Существование всех переданных `id`** — если **любой** `id` из `ids` не существует в БД → `404` (`server_not_found` / `ai_key_not_found` / `proxy_not_found`). Проверяется раньше полноты множества.

...
### Ролевая видимость (`MailScope`, нормативно — [ADR-044](adr/ADR-044-mail-full-merge-into-crm.md) §7)

`MailScope(sees_all_teams: bool, team_ids: frozenset[UUID])` — **поля `group_ids` нет**. `sees_all_teams` = admin-предикат (`is_superadmin OR permissions_subset(full_catalog, permissions)`, тот же, что у SMS); `team_ids` = команды пользователя из `user_teams`. Владение ящиком — `mail_accounts.team_id`. Граница безопасности — **backend**.

- **Чтение ленты/ящиков** (`GET /api/mail/messages`, `/mailboxes`) при `sees_all_teams=false`: только ящики с `team_id ∈ team_ids`. Фильтры пользователя (`mail_account_id`/`team_id`) **пересекаются** со scope; вне scope → **пустая страница** (анти-энумерация), не `403`/`404`. Пустой `team_ids` у не-admin → пустой результат без выборки.
...
| `400` | `validation_error` | Невалидное тело запроса (в т.ч. машинных эндпоинтов); агрегатор отклонил запрос с `400` |
...
```json
{
  ...
  "mail_account": { "id": 3, "email": "inbox@example.com", "display_name": "5108 Klyro Forge" },
...
| `mail_account` | `MailAccountRef` | Ящик-получатель |

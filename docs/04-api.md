...
| 404 | `mail_message_not_found` | Письма с таким `id` нет **или** оно вне `MailScope` (собственная проверка CRM; письма читаются из БД CRM, не проксируются) |
...
| 404 | `document_node_not_found` | Узла с таким `id` нет **или** он невидим по роли (анти-энумерация; чтение вне видимости неотличимо от несуществующего) ([Documents](#documents)) |
| 409 | `document_node_conflict` | `PATCH /api/documents/nodes/{id}` с `expected_version` ≠ текущему `content_version` (optimistic-lock, [TD-064](100-known-tech-debt.md)) ([Documents](#documents)) |
| 422 | `document_upload_invalid` | `POST /api/documents/upload`: файл не `.md` / превышен `DOCUMENTS_MAX_MD_BYTES` / битый UTF-8 ([Documents](#documents)) |
| 422 | `document_copy_cycle` | `POST /api/documents/nodes/{id}/copy`: цель копирования — сам узел или его потомок (цикл) ([Documents](#documents)) |
| 404 | `document_attachment_not_found` | Вложения с таким `id` нет **ЛИБО** узел-владелец невидим по роли **ЛИБО** узел soft-deleted — **единый код на все три случая** (анти-энумерация: различие кодов сообщало бы о существовании невидимого узла) ([Вложения](#вложения-изображения-документов-adr-068), [ADR-068](adr/ADR-068-documents-image-attachments.md)) |
...
- **`auth_method`** — string, `password` \| `key` ([ADR-067](adr/ADR-067-server-ssh-key-auth.md)). **Не секрет** — это способ входа, а не материал; нужен UI, чтобы показать строку «Способ входа» и решить, рендерить ли кнопку-глаз reveal. Отдельных флагов `has_password`/`has_key` **не вводится**: CHECK `ck_servers_auth_material` ([03-data-model.md](03-data-model.md#таблица-servers)) делает наличие материала однозначной функцией `auth_method`. Присутствует также в `ServerCreatedResponse` (`POST`) и summary-объекте (`PATCH`).
...
**Два способа входа ([ADR-067](adr/ADR-067-server-ssh-key-auth.md)):** пароль (как прежде) **или** приватный SSH-ключ с опциональной парольной фразой. Тело — **дискриминированное** по `auth_method`.

**Request (password — прежняя форма)**
```json
{ "name": "Server 02", "ip": "10.0.0.13", "ssh_user": "root", "ssh_password": "p@ss" }
```
**Request (key)**
```json
{ "name": "Server 03", "ip": "10.0.0.14", "ssh_user": "root",
  "auth_method": "key",
...
| `auth_method` | string | **опционально**, `password` \| `key`; **default `password`** ⇒ прежнее тело `{name,ip,ssh_user,ssh_password}` остаётся валидным (ломающего изменения нет) |
| `ssh_password` | string | 1–256; **required ⇔ `auth_method='password'`**, запрещено при `key` |
| `ssh_private_key` | string | 1–`SSH_KEY_MAX_BYTES` байт (env, default `16384`); **required ⇔ `auth_method='key'`**, запрещено при `password` |
| `ssh_key_passphrase` | string | 1–256; **только при `auth_method='key'`**, опционально |

**Валидация (нормативно, прецеденция сверху вниз):**

1. **Ровно один способ.** Поле «чужого» режима присутствует (даже `null`/`""`) → `422 validation_error`, `details[].field` = имя **лишнего** поля. Обязательное поле режима отсутствует/пустое → `422`, `field` = имя **недостающего**.
2. **Размер** — до разбора (анти-DoS): `ssh_private_key` > `SSH_KEY_MAX_BYTES` → `422`, `field="ssh_private_key"`.
3. **Нормализация ключа перед разбором и шифрованием:** `CRLF → LF`, срез хвостовых пробелов, гарантированный завершающий `\n` (OpenSSH отвергает ключ без newline в конце). Хранится нормализованная форма.
4. **Формат ключа — реальный разбор** библиотекой `cryptography` (уже в зависимостях, [ADR-007](adr/ADR-007-shifrovanie-fernet.md)). Regex по `BEGIN/END` **недостаточен** — он пропускает битую base64-середину, и ошибка всплыла бы только на провижининге под `status=error`.

   **Процедура — 4 шага (нормативно, [ADR-067](adr/ADR-067-server-ssh-key-auth.md) §3 п.4).** Исходы задаёт **наша** проверка, а не поведение библиотеки: на `cryptography` 43.x все негативные ветки приходят одним `ValueError` (различить их по типу нельзя, по тексту — запрещено), а лишний пароль к незашифрованному ключу может быть **молча проигнорирован**.

   | Шаг | Что делает | Исход |
   |-----|-----------|-------|
   | **1. Структурный разбор** | Определить `is_encrypted` по документированным полям, **ветки сверху вниз, первое совпадение**: armor `ENCRYPTED PRIVATE KEY` → да; `… PRIVATE KEY` + `Proc-Type: 4,ENCRYPTED`/`DEK-Info` → да; **`PRIVATE KEY` / `RSA PRIVATE KEY` / `EC PRIVATE KEY` / `DSA PRIVATE KEY` БЕЗ `Proc-Type`/`DEK-Info` → НЕТ** (незашифрованный PKCS#8 / PKCS#1 / SEC1 — **самый частый случай**, `ssh-keygen -m PEM`, ключи облачных провайдеров; грузится `load_pem_private_key(…, password=None)`, тип проверяется шагом 4); для `OPENSSH PRIVATE KEY` — декодировать base64, проверить магию `openssh-key-v1\0` и прочитать первое length-prefixed поле **`ciphername`** (`none` → нет, иначе → да) | **Catch-all (последняя ветка):** нет armor / это не `*PRIVATE KEY`-armor / у OPENSSH-armor битая base64 или нет магии (сюда же **публичный** ключ `ssh-rsa AAAA…`, сертификат, `known_hosts`) → `422`, `field="ssh_private_key"`, «Не удалось разобрать приватный SSH-ключ».<br>⚠️ PEM `… PRIVATE KEY` **без** `Proc-Type` сюда **НЕ попадает** — он разобран веткой выше |
   | **2. Кросс-проверка ввода** (до загрузки) | Сверить `is_encrypted` с наличием `ssh_key_passphrase` | `is_encrypted=false` + фраза задана → `422`, `field="ssh_key_passphrase"`, «Ключ не защищён парольной фразой — уберите её».<br>`is_encrypted=true` + фразы нет → `422`, `field="ssh_key_passphrase"`, «Ключ защищён парольной фразой — укажите её» |
   | **3. Загрузка** | `load_ssh_private_key` (OpenSSH-armor) / `load_pem_private_key` (PEM) с `password = passphrase or None`. **Ветка отказа выбирается по уже известному `is_encrypted`**, не по типу/тексту исключения | Исключение при `is_encrypted=true` → `422`, `field="ssh_key_passphrase"`, «Неверная парольная фраза».<br>Исключение при `is_encrypted=false` → `422`, `field="ssh_private_key"` |
   | **4. Тип ключа** | Загруженный объект — **RSA**, **ECDSA** (P-256/384/521) или **Ed25519** | **DSA не поддерживается** → `422`, `field="ssh_private_key"`, «Тип ключа не поддерживается» (deprecated в `cryptography` 43, отключён в OpenSSH ≥ 7.0; пере-сериализация в OpenSSH-формат на DSA >1024 бит падает — без этой проверки ключ прошёл бы валидацию и упал на провижининге) |

   > Текст исключения `cryptography` **не пробрасывается** ни в ответ, ни в лог (может нести фрагменты материала) — сообщения фиксированы таблицей; внутреннее чтение исключения не запрещено, но процедурой не требуется. Разбор локальный, без сети. **Известная неточность (принята):** зашифрованный ключ с верной фразой, но повреждённым телом отдаст «Неверная парольная фраза».
5. Шифрование — Fernet (`encrypt_secret`, тот же `FERNET_KEY` для всех трёх материалов).

**Response 202 Accepted**
```json
{ "id": "a1b2...", "name": "Server 02", "ip": "10.0.0.13", "ssh_user": "root", "auth_method": "password", "exporter_port": 9100, "provision_status": "pending", "position": 0 }
```
> `202`, т.к. установка не мгновенна; статус отслеживается через `GET /api/servers/{id}/status`. Пароль/ключ/парольная фраза в ответе не возвращаются. `position` берёт `DEFAULT 0` — новая карточка появляется вверху списка (тай-брейк `created_at DESC`).

**Ошибки:** `400 validation_error`, `409 server_conflict` (дубликат `ip`), `422 validation_error` (нарушение правил 1–4 выше; поле — в `details[].field`), `422 unprocessable` (невалидный IP), `503 provisioning_unavailable`.

### PATCH `/api/servers/{id}`
Редактирование сервера. На Этапе 1 меняется **только `name`**. Требует JWT. `ip`/`ssh_user`/`exporter_port`/провижининг и **весь SSH-материал** (`auth_method`, пароль, приватный ключ, парольная фраза) НЕ затрагиваются: переустановка агента и ротация кредов вне scope ([Q-UI-4](99-open-questions.md), [TD-072](100-known-tech-debt.md)).
...
```json
{ "id": "8f1d...e2", "name": "Server 01 (renamed)", "ip": "10.0.0.12", "ssh_user": "root", "auth_method": "password", "exporter_port": 9100, "provision_status": "online", "position": 0, "created_at": "2026-06-28T18:00:00Z", "updated_at": "2026-07-01T12:00:00Z" }
```
...
{ "value": "p@ss" }
```
**Ошибки:** `401 unauthorized`, `403 forbidden` (нет `servers:edit`), `404 server_not_found`, **`404 secret_not_set`** — сервер с `auth_method='key'` (пароля нет; [ADR-067](adr/ADR-067-server-ssh-key-auth.md)).

> **Парного эндпоинта для приватного ключа НЕТ и он не вводится** ([ADR-067](adr/ADR-067-server-ssh-key-auth.md) §4): `ssh_private_key`/`ssh_key_passphrase` — write-only секреты. Обоснование — [«Reveal секретов»](#reveal-секретов-по-требованию-adr-035) (радиус поражения ключа = весь парк хостов; `servers:edit` правит лишь `name` ⇒ симметрии с `PATCH` нет).
...
> **Порядок строк reverse-lookup (нормативно, НЕ меняется):** `position ASC, created_at DESC, id ASC` ([ADR-040](adr/ADR-040-backend-relations-secrets-reverse-lookup.md); `BackendRepository.list_by_server`/`list_by_ai_key`). **Не путать с порядком перечня бэков в тексте Telegram-алертов** — там нормативен `position ASC, code ASC` ([ADR-046](adr/ADR-046-ui-infra-fix-pack.md) §1; `code` UNIQUE ⇒ тотальный порядок, нужен для побайтовой воспроизводимости сообщения). Два порядка сосуществуют **намеренно**; переупорядочение под алерт делается in-memory, `ORDER BY` эндпоинтов не трогается.
...
Тарифы бэка для формы «Установить план» → `{ items: [{ product_id, name, price, period, archived }] }`.

- **`archived`** (bool\|null, contract v1.2 — [ADR-073](adr/ADR-073-products-archive-and-price-columns.md)) транслируется CRM **намеренно**: без него форма не смогла бы пометить архивную опцию (см. `:860` ниже и [modules/backend-users § Архивные продукты в форме «Установить план»](modules/backend-users/README.md#архивные-продукты-в-форме-установить-план-нормативно-adr-073-5)). ⛔ Снимать поле как «лишнее» **нельзя** — оно единственный источник пометки.
- **Прочие поля v1.1 (`tokens`, `avatar_tokens`, `grantable`, `updated_at`) в этот ответ НЕ добавлены — и это решение, а не пропуск:** потребителя у них на данной странице нет (форма выдаёт план, а не показывает экономику), а полный каталог с этими полями отдаёт [Backend Economics](#backend-economics). Добавлять их сюда следует только вместе с появлением потребителя.

С contract v1.1 ([ADR-072](adr/ADR-072-crm-admin-api-v11-economics.md)) элемент бэка дополнительно несёт `tokens`, `avatar_tokens`, `grantable`, `updated_at`, а сам путь принимает `scope=grantable|all`; с v1.2 ([ADR-073](adr/ADR-073-products-archive-and-price-columns.md)) — ещё и **`archived`**.

⚠️ **`scope=grantable` МОЖЕТ вернуть архивные позиции** (подтверждено стороной сервиса: `archived` — поле, а не фильтр; сервер по нему не отбирает никогда). Оси ортогональны: `scope` — право выдачи (env-список), `archived` — товарный вид. **Форма «Установить план» их НЕ фильтрует** (архивный план остаётся законной операцией), но **помечает**: подпись опции получает суффикс **« (в архиве)»** ([ADR-073](adr/ADR-073-products-archive-and-price-columns.md) §5, словарь — [08-design-system.md](08-design-system.md#локализация-страницы-продукты-и-тарифы-нормативный-словарь)). **CRM на этом эндпоинте `scope` НЕ шлёт** — у бэка действует умолчание `grantable`, то есть сегодняшнее поведение формы «Установить план» сохраняется без изменений. Полный каталог (`scope=all`) читает страница «Продукты и тарифы» — [Backend Economics](#backend-economics).
...
| `provider_cost_estimated` | `bool \| null` | `true` — себестоимость выведена из тарифной пачки (оценка сверху, UI ставит `≈`); `false` — точное значение; **`null` — поле не отдано** (бэк уровня v1) или себестоимости нет. Как и у соседних полей, CRM нормализует отсутствующее в `null` |
...
```json
{
  "items": [
    { "product_id": "pro_month", "name": "Pro (месяц)", "price": null, "period": null,
      "tokens": 1000, "avatar_tokens": 50, "grantable": true, "archived": false,
      "updated_at": null }
  ],
  "capabilities": {
    "contract_version": 1,
...
- **`archived`** (**bool\|null**, contract v1.2 — [ADR-073](adr/ADR-073-products-archive-and-price-columns.md)): продукт скрыт с витрины. **Эндпоинт ВСЕГДА возвращает полный список, включая архивные** — фильтрация клиентская, чтобы контракт CRM→frontend не зависел от формы `GET` у бэка ([Q-ECON-1](99-open-questions.md)). **`null`/отсутствует = у бэка нет понятия архива** ⇒ все продукты считаются активными, переключатель и контрол архива не рендерятся (это штатное состояние, **не** ошибка). **Архив — признак ВИТРИНЫ:** начисление и выдача работают для всех продуктов, включая архивные; форма «Установить план» их **не фильтрует** ([ADR-073](adr/ADR-073-products-archive-and-price-columns.md) §5).
...
**Тело:** `{ tokens?: int ≥ 0, avatar_tokens?: int ≥ 0, archived?: bool, if_updated_at?: ISO }` — **хотя бы одно** из `tokens`/`avatar_tokens`/**`archived`** (все три значимые), иначе `400 validation_error`. **`archived`** (contract v1.2, [ADR-073](adr/ADR-073-products-archive-and-price-columns.md)) — скрыть/вернуть продукт на витрине; гейт — `backend-economics:edit` **И** **`products.write_archived` ∈ `features`** (**отдельная** фича: `products.write_tokens` — запись токенов, бэк вправе поддержать одно без другого). Операция **обратима тем же контролом**, поэтому подтверждения не требует; на **начисление и выдачу не влияет** (§5 ADR-073). `if_updated_at` — значение `updated_at`, которое видел оператор (защита от «двух операторов»).

**Response 200:** элемент продукта (поля как выше) + `previous_tokens` (int) + `changed` (bool) + `effective_after_seconds` (int) — **все три опциональны в схеме CRM**, см. врезку ниже.

> **Три поля ответа: семантика названа, схема толерантна** ([ADR-073](adr/ADR-073-products-archive-and-price-columns.md) §8). **Правило действует для ОБОИХ `PATCH`-эндпоинтов модуля — и для `/products/{product_id}`, и для `/pricing/{tariff_id}`** (симметрично, см. §8 п.4).
> **Что бэк обязан слать при правке ОДНОГО `archived`:** `previous_tokens` — **текущее, неизменившееся** значение `tokens`; `changed` — менялось ли что-либо фактически (в т.ч. сам `archived`); `effective_after_seconds` — окно применения. Обязанность слать все три **не снята**.
> **Но `200` без любого из них НЕ даёт `502`:** схема CRM их не требует. Причина — ответ разбирается **после необратимого side-effect** (признак у бэка уже переключён), и строгость там означала бы «действие выполнено, оператору красная ошибка, аудита нет». Прецедент — [ADR-057](adr/ADR-057-mail-send-contract-fix.md) §5. Поведение при отсутствии: `previous_tokens` → тост **без дельты**; `changed` → трактуется как «изменилось» (тост успеха, не нейтральный); `effective_after_seconds` → предложение о задержке опускается. **Аудит пишется независимо от полноты ответа** — разбор не вправе бросить исключение раньше записи аудита. ⚠️ Правило **шире трёх полей**: `200` с телом-массивом, не-JSON или обрезанным телом обязан дать тот же исход (факт зафиксирован, затем интерпретация), иначе повторяется ровно тот дефект — правка состоялась, на экране `502`, аудит пуст ([ADR-073](adr/ADR-073-products-archive-and-price-columns.md) §8 п.3).
...
- **Бэк ОБЯЗАН прислать `previous_tokens`** — он даёт дельту для тоста и аудита. ⚠️ Это **обязанность контрагента, а НЕ требование схемы**: схема CRM поле не требует и на его отсутствии `502` не даёт (врезка выше, [ADR-073](adr/ADR-073-products-archive-and-price-columns.md) §8). Смешение этих двух утверждений и породило исходный дефект, когда разбор падал на неполном ответе **после уже состоявшейся** правки. Подтверждающей модалки нет (решение владельца) — видимость обеспечивает дельта, а при отсутствии `previous_tokens` тост показывается **без неё**.
...
- **Аудит-деталь называет изменённое поле:** `tokens=<prev>-><new>` — только если `tokens` был в теле; `avatar_tokens=<new>` — если в теле был он; обе части — при правке обеих величин ([ADR-072](adr/ADR-072-crm-admin-api-v11-economics.md) §10). Запись `tokens=1000->1000` при правке одних аватар-токенов — дефект. **Смена `archived` пишется ОТДЕЛЬНЫМ действием `product_archived_updated`** с деталью `archived=<new>` ([ADR-073](adr/ADR-073-products-archive-and-price-columns.md) §7) — записывать её действием `product_tokens_updated` запрещено: имя действия обязано называть изменённое.
...
**Тело:** `{ tokens: number ≥ 0, if_updated_at?: ISO }`. **Response 200:** элемент тарифа + `previous_tokens` + `changed` + `effective_after_seconds` — **все три опциональны в схеме CRM, ровно как у продукта** ([ADR-073](adr/ADR-073-products-archive-and-price-columns.md) §8, п.4: правило **симметрично** для обоих `PATCH`-эндпоинтов).

> ⚠️ **Строгость здесь была бы дефектом, а не защитой.** Side-effect правки тарифа **так же необратим**: значение списания у бэка уже изменено, и `502` из-за неполного ответа означал бы «правка применена, оператору красная ошибка, аудит молчит» — он повторит уже выполненную операцию. Довод «у тарифа одна значимая величина, поэтому `previous_tokens` всегда применим» **верен, но покрывает 1 поле из 3** и не отменяет цены ошибки. Поведение при отсутствии — то же, что у продукта: без дельты / «изменилось» / без предложения о задержке; **аудит пишется независимо от полноты ответа**.
...
> **ОТМЕНЕНО (не реализовывать).** Прежняя read-through-прокси-модель ([ADR-012](adr/ADR-012-mail-read-through-proxy.md)/[ADR-038](adr/ADR-038-mail-headless-integration.md)/[ADR-043](adr/ADR-043-lazy-mail-group-provisioning.md)): параметров/полей **`group_id`**, схемы **`MailTeam`**, эндпоинта **`GET /api/mail/teams`**, режимов пагинации **`order`/`since_id`/`before_id`/`has_more`**, кода **`mail_group_not_found`** — в действующем контракте **НЕТ**.

Ключ `MAIL_API_KEY` подставляется backend'ом только в заголовок `X-API-Key` исходящего запроса к агрегатору; в ответах CRM/логах/URL не появляется. Пользовательские эндпоинты — под JWT + RBAC. **Машинные** эндпоинты (`/api/mail/ingest`, `/api/mail/mailbox-status`, `/api/mail/oauth/ingest`) — без JWT, под HMAC. Telegram-эндпоинты (`/api/mail/telegram/*`) — без JWT, под секретом webhook / подписью `initData`.

### Ролевая видимость (`MailScope`, нормативно — [ADR-044](adr/ADR-044-mail-full-merge-into-crm.md) §7 в редакции [ADR-055](adr/ADR-055-per-channel-teams-mail-sms.md) §3)

`MailScope(sees_all_teams: bool, team_ids: frozenset[UUID], includes_unassigned: bool)` — **поля `group_ids` нет**; поле **`includes_unassigned` добавлено [ADR-055](adr/ADR-055-per-channel-teams-mail-sms.md)**. `sees_all_teams` = admin-предикат (`is_superadmin OR permissions_subset(full_catalog, permissions)`, тот же, что у SMS — **не изменён**); **`team_ids` = `user_teams` ∪ доп-команды канала `mail` (`user_channel_teams`)**; `includes_unassigned` = `users.mail_includes_unassigned`. Владение ящиком — `mail_accounts.team_id`. Граница безопасности — **backend**.

> **Предикат scope (нормативно, применять ВЕЗДЕ — и на чтении, и на мутации):**
> `account.team_id IN team_ids` **OR** (`includes_unassigned` **AND** `account.team_id IS NULL`).
> Прямое сравнение `account.team_id in scope.team_ids` **без** ветки `includes_unassigned` в новом коде = **дефект** ([ADR-055](adr/ADR-055-per-channel-teams-mail-sms.md) §3).

- **Чтение ленты/ящиков** (`GET /api/mail/messages`, `/mailboxes`) при `sees_all_teams=false`: только ящики, прошедшие предикат выше. Фильтры пользователя (`mail_account_id`/`team_id`/`no_team`) **пересекаются** со scope; вне scope → **пустая страница** (анти-энумерация), не `403`/`404`. Пустой `team_ids` **И** `includes_unassigned=false` у не-admin → пустой результат **без выборки**.
- **Reply** (`POST /api/mail/messages/{id}/reply`): письмо вне scope → `404 mail_message_not_found` (чужое неотличимо от несуществующего).
- **Создание ящика** (`POST /mailboxes`, `POST /mailboxes/oauth/authorize`) при `sees_all_teams=false`: `team_id` обязан ∈ `team_ids` (**включая доп-команды канала** — [ADR-055](adr/ADR-055-per-channel-teams-mail-sms.md) §4), иначе `403 forbidden`. **`team_id = null` (ящик без команды) — по-прежнему ТОЛЬКО admin-уровень**; не-admin с `null` → `403` **даже при `includes_unassigned=true`** ([ADR-055](adr/ADR-055-per-channel-teams-mail-sms.md) §3: флаг даёт работу с **существующими** бесхозными ящиками, но не право создавать новые вне командной модели). Несуществующая `team_id` → `404 team_not_found` (осознанный мягкий оракул: `team_id` — неперечислимый UUID на write-пути).
- **Перенос ящика** (`PATCH .../{id}` с присутствующим `team_id`) — **ТОЛЬКО admin-уровень** (`sees_all_teams=true`), даже если пользователь состоит в обеих командах / имеет их как доп-команды канала (требование владельца; [ADR-055](adr/ADR-055-per-channel-teams-mail-sms.md) §3 **это правило НЕ разворачивает**). Отдельного действия `mail:transfer` нет.
- **Мутация/удаление/синк ящика по `id`** при `sees_all_teams=false`: ящик обязан пройти **предикат scope** (выше), иначе `403 forbidden`. **Изменение [ADR-055](adr/ADR-055-per-channel-teams-mail-sms.md):** ящик **без команды** доступен не-админу **только** при `includes_unassigned=true` (прежде — `403` всегда).
...
**Супер-админ из `.env`** ([ADR-008](adr/ADR-008-admin-iz-env.md)) — **работает с прочитанностью КАК ВСЕ** ([ADR-051](adr/ADR-051-superadmin-db-anchor-personal-state.md); прежняя норма [ADR-050](adr/ADR-050-mail-search-team-filter-personal-read-state.md) §2.5 — `403`/`is_unread=false`/пустая страница/скрытые контролы — **ОТМЕНЕНА**). Его `Principal.user_id` — константный **`SUPERADMIN_USER_ID`** системной строки-якоря в `users` ([03-data-model.md](03-data-model.md#системная-строка-якорь-супер-админа-adr-051)), поэтому FK `mail_message_reads.user_id` выполним:

| Путь | Поведение для супер-админа |
|------|----------------------------|
| `POST` / `DELETE /api/mail/messages/{id}/read` | **`204 No Content`** (идемпотентно) — как у любого носителя `mail:view` |
| `is_unread` в `MailMessage` | **реальное личное значение** (его отметки не влияют на `is_unread` у коллег, и наоборот) |
| `GET /api/mail/messages?unread=true` | **обычный серверный фильтр** (анти-джойн), а не пустая страница |
| UI (`/mail`) | Индикатор непрочитанного, фильтр «Непрочитанные», кнопка «Отметить непрочитанным» — **рендерятся** (гейта по `me.is_superadmin` в прочитанности **нет**) |

> **`GET`/`PATCH /api/mail/me/settings` для супер-админа — по-прежнему `403`** ([ADR-051 §1.6](adr/ADR-051-superadmin-db-anchor-personal-state.md)), но уже **не** «потому что нет БД-строки», а по security-основанию: bootstrap-учётке **запрещена Telegram-привязка** (иначе Telegram-SSO дал бы беспарольный, неотзываемый из UI путь к admin-уровню в обход `ADMIN_PASSWORD`) ⇒ Telegram-уведомлений она не получает и настраивать ей нечего. Прочитанность в этот запрет **не входит**: она не канал доставки и не путь входа, а состояние чтения того, что супер-админ и так вправе прочитать.

### Коды ошибок модуля

| HTTP | `code` | Когда |
|------|--------|-------|
| `400` | `invalid_limit` | `GET /api/mail/messages`: `limit` вне `1..200` |
| `400` | `invalid_cursor` | `GET /api/mail/messages`: битый/неразбираемый `before` |
| `400` | `validation_error` | Невалидное тело запроса (в т.ч. машинных эндпоинтов); агрегатор отклонил запрос с `400`; **`GET /api/mail/messages` с одновременно заданными `team_id` и `no_team=true`** ([ADR-055](adr/ADR-055-per-channel-teams-mail-sms.md) §5.3 — значения взаимоисключающи) |
...
| `403` | `forbidden` | Нет действия в правах роли (RBAC `require`); мутация/синк/удаление ящика вне `MailScope`; создание ящика для чужой команды; `team_id = null` у не-admin; **перенос** ящика не-админом; `GET`/`PATCH /api/mail/me/settings` для супер-админа из `.env` (**Telegram-привязка bootstrap-учётке запрещена** — [ADR-051](adr/ADR-051-superadmin-db-anchor-personal-state.md) §1.6). **`POST`/`DELETE /api/mail/messages/{id}/read` супер-админу `403` БОЛЬШЕ НЕ ОТДАЮТ** (норма [ADR-050](adr/ADR-050-mail-search-team-filter-personal-read-state.md) §2.5 отменена [ADR-051](adr/ADR-051-superadmin-db-anchor-personal-state.md) — у него есть строка-якорь в `users`) |
...
| `404` | `mail_message_not_found` | Письмо не найдено **или** вне `MailScope` (анти-энумерация) — **собственная проверка CRM по своей БД**; **`POST`/`DELETE /api/mail/messages/{id}/read` по неизвестному/чужому письму** ([ADR-050](adr/ADR-050-mail-search-team-filter-personal-read-state.md) §2.2). **Проброс внешнего `404` при reply — ОТМЕНЁН** ([ADR-057](adr/ADR-057-mail-send-contract-fix.md) §3): `404` от `send` означает «нет **ящика**», см. строку ниже |
| `404` | `mail_mailbox_not_found` | Ящик не найден (`PATCH`/`DELETE`/`sync` по неизвестному `id`); **а также внешний `404` от `POST /api/external/mailboxes/{id}/send` при reply** — ящика нет в почтовом сервисе (рассинхрон каталога, [ADR-057](adr/ADR-057-mail-send-contract-fix.md) §3) |
...
| Поле | Тип | Примечание |
|------|-----|-----------|
| `id` | integer | `mail_messages.id` (BIGSERIAL). **Ключ ленты — не порядок сортировки** (см. курсор ниже) |
| `subject` | string \| null | Тема; `null` — без темы |
| `internal_date` | datetime (ISO 8601 UTC) | Истинная дата письма — **ключ сортировки ленты** |
| `from_addr` | string | Адрес отправителя |
| `from_name` | string \| null | Имя отправителя |
| `to_addrs` | string | Получатели (строка адресов) |
| `cc_addrs` | string \| null | Копия |
| `mail_account` | `MailAccountRef` | Ящик-получатель (**расширен** [ADR-056](adr/ADR-056-mail-mini-app-mailbox-context.md): += `number`/`app_name`/`team`) |
...
| `is_unread` | boolean | **ЛИЧНОЕ производное** ([ADR-050](adr/ADR-050-mail-search-team-filter-personal-read-state.md)): `true` ⇔ для **текущего принципала** нет строки `mail_message_reads(user_id, message_id)`. **Не nullable.** Один и тот же `id` письма у разных пользователей даёт **разные** значения. **Супер-админ из `.env` — не исключение:** у него реальное личное значение (строка-якорь в `users`, [ADR-051](adr/ADR-051-superadmin-db-anchor-personal-state.md); прежнее «всегда `false`» отменено). Меняется вызовами `POST`/`DELETE /api/mail/messages/{id}/read`. Поле `read_at` наружу **не отдаётся** |
...
#### Схема `MailAccountRef`

Расширена [ADR-056](adr/ADR-056-mail-mini-app-mailbox-context.md) — **аддитивно**, ничего не удалено:

| Поле | Тип | Примечание |
|------|-----|-----------|
| `id` | integer | `mail_accounts.id` (равен id ящика в агрегаторе) |
| `email` | string | Адрес ящика |
| `display_name` | string \| null | **ПРОИЗВОДНОЕ** — склейка `"<number> <app_name>"` ([TD-052](100-known-tech-debt.md)). Остаётся в схеме (десктоп использует); **в Mini App не рендерится** (иначе те же значения печатались бы дважды) |
| `number` | string \| null | **«Номер»** ящика (`mail_accounts.number`, миграция `0024`). Слово владельца «**Код**» = это поле ([ADR-056](adr/ADR-056-mail-mini-app-mailbox-context.md)) |
| `app_name` | string \| null | **«Приложение»** ящика (`mail_accounts.app_name`). Слово владельца «**Название**» = это поле |
| `team` | `MailTeamRef` \| null | **Команда-владелец ящика**; `null` — ящик без команды (`team_id IS NULL`). Источник — **LEFT JOIN `teams`** в том же запросе страницы ленты (**N+1 запрещён**) |

`MailTeamRef = { id: string(uuid), name: string }` — зеркало [`SmsTeamRef`](#схема-smsteamref) (отдельный тип: модули не импортируют схемы друг друга).

> **Почему команда приходит внутри письма:** имя команды на клиенте недостижимо — `GET /api/teams` гейтится `teams:view`, которого у mail-оператора может не быть ([ADR-056](adr/ADR-056-mail-mini-app-mailbox-context.md), [TD-058](100-known-tech-debt.md)). **Утечки нет:** актор и так видит только письма ящиков своего `MailScope` ⇒ имя команды/номер/приложение уже доступного ему ящика новой информации не раскрывают (тот же довод, что [ADR-048](adr/ADR-048-teams-mailbox-count-mail-row.md) §4). Креды и статус синка в ленту по-прежнему **не** попадают.
...
Лента писем **из БД CRM**. Требует JWT (`mail:view`).

**Порядок (нормативно): `internal_date DESC, id DESC`** — по истинной дате письма, **НЕ** по `id` (BIGSERIAL отражает порядок прихода push'а; recovery-ре-пуш старого письма иначе «всплыл» бы в топ).

**Query**

| Параметр | Тип | Правила |
|----------|-----|---------|
| `before` | string? | Непрозрачный курсор пары `(internal_date, id)`, полученный как `next_cursor` предыдущей страницы. Не задан → первая (самая свежая) страница. Битый → `400 invalid_cursor`. **Клиент курсор не интерпретирует** |
...
**Response 200** — `MailListResponse`:

```json
{ "messages": [ /* MailMessage[] */ ], "next_cursor": "MjAyNi0wNy0wMlQwOToxNTowMFp8MTA0Mg==" }
```

| Поле | Тип | Примечание |
|------|-----|-----------|
| `messages` | `MailMessage[]` | Страница писем в порядке `internal_date DESC, id DESC` |
...
Ответ на письмо. Требует JWT (`mail:view`). Письмо берётся из БД CRM; threading (`In-Reply-To` = `message_id_header` исходного, `References` = `refs_header` + `message_id_header`) формирует CRM; SMTP-отправку выполняет агрегатор (`POST /api/external/mailboxes/{id}/send`); факт отправки пишется в `mail_sent_messages`.

**Request** — `MailReplyRequest`:

```json
{ "to": ["sender@example.com"], "cc": null, "subject": "Re: Отчёт за июнь", "body": "Спасибо, получил." }
```

| Поле | Тип | Правила |
|------|-----|---------|
| `to` | string[]? | опц.; default — `[from_addr]` исходного письма. Каждый адрес — валидный e-mail |
| `cc` | string[] \| null? | опц., копия |
| `subject` | string? | опц.; default — `"Re: " + subject` исходного; **≤ 998** символов |
| `body` | string | required, **непустой**, **≤ 1 MiB** |

Нормы (нарушение → **`422 unprocessable`**): `to`+`cc` суммарно **≤ 100** адресов; явный пустой `to` **и** пустой `cc` → `422` (письмо без получателей отклоняется **до** вызова агрегатора).

**Response 200** — `MailReplyResponse`: `{ "sent_id": "9f1c2e40-6a1b-4f3e-9d2a-7b8c0d1e2f34", "smtp_message_id": "<abc123@example.com>" }`.

| Поле ответа | Тип | Источник |
|------|-----|---------|
| `sent_id` | string (uuid) | **`mail_sent_messages.id` самой CRM** (запись факта отправки). ~~`int` от агрегатора~~ — отменено ([ADR-057](adr/ADR-057-mail-send-contract-fix.md) §1): агрегатор `sent_messages` не ведёт и `sent_id` не возвращает |
| `smtp_message_id` | string \| null | `Message-ID` отправленного письма (из ответа агрегатора `{ smtp_message_id }`). **`null`** — агрегатор ответил `200`, но идентификатор не прислал: письмо **отправлено** и записано в `mail_sent_messages`, идентификатора нет ([ADR-057](adr/ADR-057-mail-send-contract-fix.md) §5; колонка БД nullable). Неполный ответ агрегатора **не** превращается в `502` — факт отправки важнее (в логе — `mail_send_missing_smtp_message_id`) |

**Ошибки:** `401`, `403 forbidden`, `404 mail_message_not_found` (нет письма **или** вне scope — собственная проверка CRM), **`404 mail_mailbox_not_found`** (агрегатор ответил `404` на `send` = ящика нет в почтовом сервисе, рассинхрон каталога — [ADR-057](adr/ADR-057-mail-send-contract-fix.md) §3), `400 validation_error`, `422 unprocessable`, `409 mail_conflict`, **`502 mail_send_failed`** (удалённый SMTP не принял письмо — [ADR-053](adr/ADR-053-mail-timeouts-error-passthrough.md) §2), **`504 mail_timeout`** (бюджет `MAIL_API_MAILSERVER_TIMEOUT_SEC` = 75 с > 55 с бюджета отправки у агрегатора; отправка **не подтверждена** — письмо могло уйти, автоповтора нет), `502 mail_unavailable`, `503 mail_not_configured`.
...
**Ошибки:** `401 unauthorized`; **`403 forbidden`** (нет `mail:view`; **супер-админ из `.env` — НЕ исключение, отметка ему доступна**, [ADR-051](adr/ADR-051-superadmin-db-anchor-personal-state.md)); **`404 mail_message_not_found`** (письмо не существует **или** вне `MailScope` — анти-энумерация, чужое неотличимо от несуществующего).
...
**Ошибки:** те же — `401`, **`403 forbidden`** (нет `mail:view`; супер-админу отметка **доступна** — [ADR-051](adr/ADR-051-superadmin-db-anchor-personal-state.md)), **`404 mail_message_not_found`**.
...
```json
{
  ...
  "team_id": "a2c9…f0",
  "is_active": true,
  "last_synced_at": "2026-07-11T08:00:00Z",
...
| Поле | Тип | Примечание |
|------|-----|-----------|
| `id` | integer | ID ящика (**равен** id в агрегаторе; он же `mail_account_id` в фильтре ленты и в `PATCH`/`DELETE`/`sync`) |
| `email` | string | Адрес ящика |
| `number` | string \| null | **«Номер»** ([ADR-047](adr/ADR-047-mail-fix-pack.md) §3) |
| `app_name` | string \| null | **«Приложение»** ([ADR-047](adr/ADR-047-mail-fix-pack.md) §3) |
| `display_name` | string \| null | **ПРОИЗВОДНОЕ** (read-only для клиента): `"<number> <app_name>"` (пустые части опускаются; обе пусты → `null`). Считает сервер; **в запросах не принимается**. Единственная форма имени, уходящая в агрегатор |
| `team_id` | string(uuid) \| null | Команда-владелец; `null` — ящик без команды (unassigned) |
| `is_active` | boolean | Активен ли ящик (кружок статуса, счётчик «Дашборда») |
| `last_synced_at` | datetime \| null | Время последнего успешного синка (зеркалится status-каналом) |
| `last_sync_error` | string \| null | Текст последней ошибки синка |
| `consecutive_failures` | integer | Число подряд неудачных синков (0 — здоров) |

### GET `/api/mail/mailboxes`

Список ящиков **из БД CRM**. Требует JWT (`mail:view`). Фильтруется `MailScope` — **единым предикатом** ([ADR-055](adr/ADR-055-per-channel-teams-mail-sms.md) §3): `account.team_id ∈ team_ids` (базовые ∪ доп-команды канала) **OR** (`includes_unassigned` **AND** `account.team_id IS NULL`).
...
> **⚠️ Уточнение [ADR-055](adr/ADR-055-per-channel-teams-mail-sms.md) §5.3 — «эндпоинт не меняется» относится к КОНТРАКТУ, но НЕ к результату.** **Контракт** (query-параметры, схема ответа, отсутствие пагинации) — **без изменений**: `no_team`/`team_id`/`q` здесь **не вводятся** (фильтры вкладки «Почты» — клиентские). **Результат — меняется:** каталог обязан отдавать **бесхозные ящики** (`team_id IS NULL`) пользователю с `mail_includes_unassigned = true`. Иначе клиентский фильтр «Без команды» покажет ему пустоту при том, что эти ящики он вправе **править, синкать и удалять**. Выборка каталога использует **предикат выше**, а не `team_id IN team_ids`.
...
Проверка IMAP/SMTP-соединения **без сохранения** (проброс в агрегатор). Требует JWT (`mail:create`). `Cache-Control: no-store`. Пароли — транзитом, не логируются, в ответ не попадают.

**Request** — `MailMailboxTestRequest`:
...
Создание ящика. Требует JWT (`mail:create`). `Cache-Control: no-store`.

**Поток:** CRM → агрегатор `POST /api/external/mailboxes` (владелец там — служебный `crm-service`, **без** `group_id`) → присвоенный `id` → вставка строки `mail_accounts { id, email, number, app_name, display_name, team_id, is_active }`. Сбой вставки каталога → best-effort компенсация (удалить ящик в агрегаторе), чтобы не оставить невидимую сироту.

**Request** — `MailMailboxCreateRequest` = поля `MailMailboxTestRequest` **плюс**:

| Поле | Тип | Правила |
|------|-----|---------|
| `number` | string \| null? | опц., **«Номер»** |
| `app_name` | string \| null? | опц., **«Приложение»** |
| `team_id` | string(uuid) \| null? | Команда-владелец. Не-admin: обязан ∈ `MailScope.team_ids`, иначе `403`. **`null` (без команды) — только admin-уровень** |

> **`display_name` в запросе НЕ принимается** — сервер вычисляет его из `number`/`app_name` ([ADR-047](adr/ADR-047-mail-fix-pack.md) §3).
>
> **В агрегатор уходит БЕЛЫЙ СПИСОК** (креды + `email` + вычисленный `display_name`). `team_id`, `number`, `app_name` в агрегатор **не уходят никогда**.
...
Правка ящика (presence-семантика: присутствие поля = «изменить»). Требует JWT (`mail:edit`). `Cache-Control: no-store`. Не-admin: ящик ∈ `MailScope`, иначе `403`.

**Request** — `MailMailboxUpdateRequest` (все поля опц.):

| Поле | Тип | Правила |
|------|-----|---------|
| `email` | string? | Новый адрес |
| `number` | string \| null? | **«Номер»** (`null` — очистить) |
| `app_name` | string \| null? | **«Приложение»** (`null` — очистить) |
| `imap_host` / `imap_port` / `imap_ssl` | ? | Параметры IMAP (`imap_port` — `1..65535`) |
| `smtp_host` / `smtp_port` / `smtp_ssl` / `smtp_starttls` / `smtp_username` | ? | Параметры SMTP (`smtp_port` — `1..65535`) |
| `password` | string? | Новый IMAP-пароль (транзит; агрегатор ре-проверяет соединение) |
| `smtp_password` | string? | Новый SMTP-пароль (транзит) |
| `is_active` | boolean? | Активация/деактивация ящика |
| `team_id` | string(uuid) \| null? | **Перенос между командами — ТОЛЬКО admin-уровень** (`sees_all_teams=true`); при `false` присутствие `team_id` → `403 forbidden`. `null` — открепить (unassigned). Перенос — **локальный `UPDATE`**, сетевого вызова к агрегатору не делает |

> **`display_name` в запросе НЕ принимается.** При изменении `number` **или** `app_name` сервер пересчитывает `display_name` и отправляет **его** в агрегатор (ключ `display_name` — часть белого списка исходящего payload). Сами `number`/`app_name` наружу не уходят.
>
> **Сетевой вызов к агрегатору выполняется только при изменении кредов / `email` / `is_active` / (пересчитанного) `display_name`.** Правка одного `team_id` агрегатор не трогает.
...
Удаление ящика. Требует JWT (`mail:delete`). Не-admin — ящик ∈ `MailScope`. Агрегатор `DELETE /api/external/mailboxes/{id}` + удаление строки каталога (**CASCADE** удаляет письма и reply ящика). Агрегаторский `404` (ящик там уже удалён) — best-effort: логируется, локальная строка всё равно чистится.

**Response 204.** **Ошибки:** `401`, `403 forbidden`, `404 mail_mailbox_not_found`, `502 mail_unavailable`, `503 mail_not_configured`.

### POST `/api/mail/mailboxes/{id}/sync`

Форс-синхронизация ящика (проброс в агрегатор). Требует JWT (`mail:sync`). Не-admin — ящик ∈ `MailScope`.

**Response 202** — `MailMailboxSyncResponse`: `{ "queued": true }`. **Ошибки:** `401`, `403 forbidden`, `404 mail_mailbox_not_found`, `502 mail_unavailable`, `503 mail_not_configured`.

### Outlook OAuth (headless) — [ADR-045](adr/ADR-045-mail-outlook-oauth-headless-reonboarding.md)

Authorize-URL генерирует и токены хранит **агрегатор**; CRM инициирует flow и привязывает ящик к команде.

#### POST `/api/mail/mailboxes/oauth/authorize`

Требует JWT (`mail:create`). `Cache-Control: no-store`.

**Тело** — `MailOauthAuthorizeRequest`: `{ "team_id": string(uuid) | null }`. Правила `team_id` — **как при создании ящика** (не-admin: ∈ `MailScope.team_ids`; `null` — только admin).

Логика: CRM минтит непрозрачный HMAC-подписанный **stateless** `crm_state` (`{team_id, initiator_user_id, exp}`, секрет `MAIL_PUSH_SECRET`, TTL `MAIL_OAUTH_STATE_TTL_SEC`) → агрегатор `POST /api/external/mailboxes/oauth/authorize { crm_state }` → Microsoft authorize URL.

**Response 200** — `MailOauthAuthorizeResponse`: `{ "authorize_url": "https://login.microsoftonline.com/…" }`. Фронт показывает ссылку для открытия **в нужном профиле OctoBrowser** (не auto-redirect).

**Ошибки:** `401`, `403 forbidden`, `404 team_not_found`, `503 mail_not_configured` (**единый код**: почта не настроена ИЛИ Outlook-OAuth выключен на агрегаторе), `502 mail_unavailable`.

#### POST `/api/mail/oauth/ingest`

**Машинный** приёмник уведомления агрегатора о созданном/переподключённом Outlook-ящике. **HMAC** тем же механизмом и секретом `MAIL_PUSH_SECRET`, что `/api/mail/ingest`. Без JWT, CSRF-exempt. `Cache-Control: no-store`.

**Тело** — `MailOauthIngestRequest`:

| Поле | Тип | Примечание |
|------|-----|-----------|
| `crm_state` | string | Эхо-возврат токена шага authorize; CRM проверяет подпись и `exp`, извлекает `team_id`/`initiator` |
| `mail_account_id` | integer | id ящика (присвоен агрегатором) |
| `email` | string | Адрес ящика |
| `display_name` | string \| null | Имя ящика (как его знает агрегатор) |
| `is_active` | boolean | Статус ящика |

Логика: HMAC + timestamp-окно → проверка `crm_state` → **upsert** `mail_accounts` `ON CONFLICT (id) DO UPDATE` (идемпотентно; `team_id` детерминированно перезаписывается значением из `crm_state`).

**Response 200** — `{ "ok": true }`. **Ошибки:** `401 not_authenticated`, `410 oauth_state_expired`, `400 validation_error`, `503 mail_ingest_not_configured`.

### Схема `MailTagRule`

```json
{ "id": "3b8e…", "type": "subject_contains", "pattern": "счёт", "created_at": "2026-07-01T10:00:00Z" }
```

| Поле | Тип | Примечание |
|------|-----|-----------|
| `id` | string(uuid) | ID правила |
| `type` | enum | ∈ {`subject_contains`, `body_contains`, `sender_contains`, `sender_exact`}. UI-лейблы — [08-design-system.md](08-design-system.md#вкладка-теги-нормативно). **`sender_exact` не предлагается в UI для создания**, но остаётся валидным значением контракта и корректно отображается ([ADR-047](adr/ADR-047-mail-fix-pack.md) §2, [TD-055](100-known-tech-debt.md)) |
| `pattern` | string | Паттерн, `1..256` |
| `created_at` | datetime | Время создания |

### Схема `MailTagFull`

Полный тег с правилами (вкладка «Теги»). **Глобальный админский каталог — у тега нет владельца.**

```json
{
  "id": "7f1c…", "name": "Релиз", "color": "#16a34a", "match_mode": "all",
  "rules": [ { "id": "3b8e…", "type": "sender_contains", "pattern": "App Store Connect", "created_at": "2026-07-01T10:00:00Z" } ],
...
| `name` | string | Имя тега (`1..64`), **глобально уникально** (`uq_mail_tags_name`) |
| `color` | string | HEX из палитры (`^#[0-9A-Fa-f]{6}$`, 8 цветов — [08-design-system.md](08-design-system.md#вкладка-теги-нормативно)) |
| `match_mode` | enum | ∈ {`any`, `all`}: тег вешается при совпадении любого / всех правил |
| `rules` | `MailTagRule[]` | Правила тега |
| `created_at` / `updated_at` | datetime | Время создания/изменения |

> **Поля `is_builtin` НЕТ** ([ADR-047](adr/ADR-047-mail-fix-pack.md) §1): признак «встроенный тег» упразднён, колонка `mail_tags.is_builtin` дропнута (миграция `0023`). **Удалить можно ЛЮБОЙ тег.**

### GET `/api/mail/tags`

Список глобальных тегов с правилами **из БД CRM**. Требует JWT (`mail:view`). Без параметров.

**Response 200** — `MailTagsResponse`: `{ "tags": [ /* MailTagFull[] */ ] }`. **Ошибки:** `401`, `403 forbidden`.

### POST `/api/mail/tags`

Создание тега. Требует JWT (`mail:tags`).

**Request** — `MailTagCreateRequest`: `{ "name": string(1..64), "color": string(HEX), "match_mode"?: "any"|"all" }` (`match_mode` опц., default `any`).

**Response 201** — `MailTagFull` (без правил). **Ошибки:** `401`, `403 forbidden`, `400`/`422 unprocessable` (невалидный `color`/`name`), `409 mail_conflict` (имя занято).

### PATCH `/api/mail/tags/{id}`

Правка тега. Требует JWT (`mail:tags`). **Request** — `MailTagUpdateRequest` (все опц.): `{ "name"?, "color"?, "match_mode"? }`. **Response 200** — `MailTagFull`. **Ошибки:** `401`, `403 forbidden`, `404 mail_tag_not_found`, `409 mail_conflict` (имя занято), `422 unprocessable`.

### DELETE `/api/mail/tags/{id}`

Удаление тега (вместе с правилами и привязками — CASCADE). Требует JWT (`mail:tags`). **Удалить можно ЛЮБОЙ тег** — ветки `409` больше нет ([ADR-047](adr/ADR-047-mail-fix-pack.md) §1).

**Response 204.** **Ошибки:** `401`, `403 forbidden`, `404 mail_tag_not_found`.

### POST `/api/mail/tags/{id}/rules`

Добавление правила тегу. Требует JWT (`mail:tags`). **Request** — `MailTagRuleCreateRequest`: `{ "type": enum, "pattern": string(1..256) }`. **Response 201** — `MailTagRule`. **Ошибки:** `401`, `403 forbidden`, `400`/`422 unprocessable`, `404 mail_tag_not_found`.

### DELETE `/api/mail/tags/{id}/rules/{rule_id}`

Удаление правила. Требует JWT (`mail:tags`). **Response 204.** **Ошибки:** `401`, `403 forbidden`, `404 mail_tag_not_found` (нет тега **или** правила).

### POST `/api/mail/tags/{id}/apply-to-existing`

Применить правила тега ко всем письмам в БД CRM. Требует JWT (`mail:tags`). Идемпотентно (`ON CONFLICT (message_id, tag_id) DO NOTHING`).

**Response 200** — `MailTagApplyResponse`: `{ "applied_count": integer }`. **Ошибки:** `401`, `403 forbidden`, `404 mail_tag_not_found`.

### GET / PATCH `/api/mail/me/settings`

Self-настройки Telegram-уведомлений текущего пользователя (opt-out). Требует JWT (`mail:view`). Upsert по `principal.user_id`; дефолт (нет строки) — уведомления **включены**.

**`PATCH` Request** — `MailUserSettingsUpdateRequest`: `{ "tg_notifications_enabled": boolean }`.
**Response 200** (оба метода) — `MailUserSettingsResponse`: `{ "tg_notifications_enabled": boolean }`.

**Ошибки:** `401`, `403 forbidden` (нет `mail:view` **или** супер-админ из `.env` — bootstrap-учётке **запрещена Telegram-привязка**, поэтому персональной настройки уведомлений у неё нет: [ADR-051 §1.6](adr/ADR-051-superadmin-db-anchor-personal-state.md); прежнее основание «нет БД-строки» устарело — строка-якорь у него есть).

### Машинные эндпоинты push (агрегатор → CRM) — [ADR-044](adr/ADR-044-mail-full-merge-into-crm.md) §3

**Без JWT, CSRF-exempt.** Аутентификация — **HMAC-SHA256**, тело-связанная:

- Заголовки: `X-Mail-Signature: sha256=<hex>`, `X-Mail-Timestamp: <unix_seconds>`.
- Каноническая подпись (**побайтно**, над **сырым** телом до JSON-парсинга): `mac_input = str(timestamp).encode("ascii") + b"." + raw_body_bytes`; `signature = hmac_sha256(MAIL_PUSH_SECRET, mac_input).hexdigest()`.
- Окно: `abs(now - timestamp) <= MAIL_PUSH_MAX_SKEW_SEC` (default `300`). Сравнение — constant-time.
- **Порядок проверок:** пустой `MAIL_PUSH_SECRET` → `503 mail_ingest_not_configured` → невалидная подпись/skew → `401 not_authenticated` → битое тело → `400 validation_error`.
- Детали модели угроз (окно ≠ анти-replay; повтор гасится идемпотентностью приёмника) — [05-security.md](05-security.md#push-контракт-агрегатор--crm-hmac-нормативно).

#### POST `/api/mail/ingest`

Приём батча новых писем.

**Тело** — `MailIngestRequest`: `{ "messages": MailIngestMessage[] }`, размер батча `1..MAIL_INGEST_MAX_BATCH` (default `100`).

`MailIngestMessage`:

| Поле | Тип | Примечание |
|------|-----|-----------|
| `mail_account_id` | integer | id ящика (= `mail_accounts.id`) |
| `uidvalidity` | integer | Часть ключа идемпотентности |
| `uid` | integer | Часть ключа идемпотентности |
...
| `internal_date` | datetime (ISO 8601 UTC) | |
| `body_text` | string | |
...
**Идемпотентность:** `INSERT ... ON CONFLICT (mail_account_id, uidvalidity, uid) DO NOTHING`. При фактической вставке — применяются теги. **Telegram-рассылку хендлер НЕ делает** (`notified_at` остаётся `NULL`; доставку берёт фоновый `MailDispatcherService`).
...
**Response 200** — `MailIngestResponse`: `{ "accepted": int, "duplicate": int, "unknown_mailbox": int }`. `2xx` ⇒ агрегатор считает батч доставленным.

**Ошибки:** `401 not_authenticated`, `400 validation_error` (битое тело / батч вне лимита), `503 mail_ingest_not_configured`.

> **nginx:** `client_max_body_size` на `location /api` обязан вмещать батч (сейчас `50m`) — иначе прокси режет запрос `413` **до** backend и приём молча теряется ([07-deployment.md](07-deployment.md#reverse-proxy-nginx--требования), [TD-045](100-known-tech-debt.md)).

#### POST `/api/mail/mailbox-status`

Зеркало статуса синка ящика.

**Тело** — `MailboxStatusRequest`:

| Поле | Тип | Примечание |
|------|-----|-----------|
| `mail_account_id` | integer | id ящика |
| `is_active` | boolean | Актуальный статус синка |
| `last_synced_at` | datetime \| null | опц. Время последней **УСПЕШНОЙ** синхронизации — ошибочные ветки синка агрегатора его **не обновляют** (агрегаторский `ADR-0046` §1). У сбоящего ящика значение не освежается: это норма, а не потерянный push |
| `last_sync_error` | string \| null | опц. Текст последней ошибки синка. Маркер `oauth_needs_consent: …` = Outlook-ящик требует переподключения (агрегатор отражает needs-consent через это поле; отдельного поля в контракте нет) |
| `consecutive_failures` | integer | опц., default `0` |

**Когда агрегатор шлёт статус (нормативно):** на **каждое** изменение статуса синка — исчерпывающий перечень точек hook (H1–H7) и не-hook точек (N1–N6) в агрегаторском `ADR-0046` §3/§4 (парная норма CRM — [ADR-044](adr/ADR-044-mail-full-merge-into-crm.md) §3). Дедупа на стороне агрегатора нет: за один сбойный цикл ящика может прийти до **3** статус-событий — это штатно, приёмник идемпотентен (идемпотентность «ровно один алерт на переход» держит `down_alert_sent_at`).

Переход `is_active: true→false` оставляет `down_alert_sent_at = NULL` → mailbox-down-алерт разошлёт проход C диспетчера **ровно один раз** (guarded `UPDATE`). Переход `false→true` сбрасывает `down_alert_sent_at = NULL`.

**Response 200** — `MailboxStatusResponse`: `{ "updated": boolean }` (неизвестный `mail_account_id` → `{ "updated": false }`, no-op, [TD-041](100-known-tech-debt.md)).

**Ошибки:** `401 not_authenticated`, `400 validation_error`, `503 mail_ingest_not_configured`.

### Telegram-эндпоинты почты — [ADR-044](adr/ADR-044-mail-full-merge-into-crm.md) §6/§9

**Без JWT, CSRF-exempt.** Префикс `/api/mail/telegram` (не конфликтует с SMS `/api/sms/telegram`).

| Эндпоинт | Гейт | Назначение |
|----------|------|-----------|
| `POST /api/mail/telegram/webhook/{secret}` | `{secret}` constant-time == `MAIL_BOT_WEBHOOK_SECRET` (+ заголовок `X-Telegram-Bot-Api-Secret-Token`, если прислан); mismatch → **`404`** (анти-энумерация) | Основной бот: `/start` → SSO-линковка (`mail_telegram_links`); `callback_query` `mail:{id}` → отправить тело письма |
| `POST /api/mail/telegram/push-webhook/{bot_name}` | per-bot `MAIL_BOT_<NAME>_WEBHOOK_SECRET`, **header-only fail-closed** (missing/mismatch → `404`); `bot_name ∈ {ivan, alexandra, andrei, business2}` | 4 push-бота команд (маппинг бот→команда — по `MAIL_BOT_<NAME>_TEAM_ID`) |
| `POST /api/mail/telegram/auth` | Подпись Telegram **`initData`** (HMAC бота) — **граница безопасности**; TTL `MAIL_TG_INITDATA_TTL_SEC` (300 с) | SSO Mini App `/tg/mail` |

**`POST /api/mail/telegram/auth`** — Request `MailTelegramAuthRequest`: `{ "init_data": string }`. Резолв: приоритет — иммутабельный `telegram_user_id` (существующий линк); иначе bootstrap по username → `lower(users.telegram)` (регистронезависимо, ведущий `@` снимается) → auto-upsert линка.

**Response 200** — `MailTelegramAuthResponse`:

| Поле | Тип | Примечание |
|------|-----|-----------|
| `access_token` | string | CRM access-JWT (как у SMS Mini App) |
| `token_type` | string | `"bearer"` |
| `expires_in` | integer | TTL токена, сек |
| `telegram_user_id` | integer | Telegram-id (= `chat_id`) |
| `linked` | boolean | Линк существует/создан |

**Ошибки:** `400 validation_error` (пустой/битый `init_data`), `401 invalid_init_data` (неверная подпись), `401 init_data_expired` (протух), `403 mail_operator_not_provisioned` (Telegram-пользователь не сопоставлен с CRM-пользователем — фронт показывает понятное сообщение, **не** пустую ленту и **не** экран логина).
...
> **⚠️ `PATCH /api/backends/order` — UI его больше НЕ вызывает** ([ADR-046](adr/ADR-046-ui-infra-fix-pack.md) §2а, [TD-054](100-known-tech-debt.md)). DnD на странице «Бэки» **убран**: порядок карточек задаётся клиентской сортировкой по `name` (tie-break `code`). Эндпоинт и колонка `backends.position` **сохраняются** в контракте и в БД (`position` по-прежнему определяет порядок ответа `GET /api/backends` и порядок перечня бэков в Telegram-алертах — [ADR-046](adr/ADR-046-ui-infra-fix-pack.md) §1). DnD на `/servers`, `/ai-keys`, `/proxies` — **без изменений**.
...
Модуль «Документы» ([ADR-059](adr/ADR-059-documents-module.md), [modules/documents](modules/documents/README.md)). Внутренние эндпоинты — под JWT, префикс `/api/documents`, гейт `require("documents", <action>)` + **per-node фильтр видимости по роли** (невидимый узел → `404 document_node_not_found`, анти-энумерация). Модель — [03-data-model.md](03-data-model.md#таблицы-модуля-документы-document_nodes-document_node_roles), видимость — [05-security.md](05-security.md#видимость-документов-по-ролям-нормативно-adr-059).

**Форма узла в ответах (`DocumentNode`):**

| Поле | Тип | Примечание |
|------|-----|-----------|
| `id` | uuid | |
| `node_type` | string | `folder` \| `document` |
| `parent_id` | uuid \| null | `null` = корень |
| `name` | string | 1–255 |
| `content_md` | string \| null | только у документа и **только в `GET /nodes/{id}`**. Во **всех** остальных ответах (списки, дерево, а также мутационные `POST`/`PATCH`, включая `PATCH /nodes/{id}`) — **`null`**: единственный read-канал контента — `GET /nodes/{id}` ([ADR-063](adr/ADR-063-documents-editor-cache-lifecycle-focus.md) §A) |
| `owner_id` | uuid | автор (для отображения) |
| `visibility_mode` | string | `inherit` \| `restricted` |
| `content_version` | integer | версия контента |
| `position` | integer | порядок в уровне |
| `created_at` | string | ISO 8601 UTC |
| `updated_at` | string | ISO 8601 UTC |

### GET `/api/documents/tree`
Всё видимое пользователю дерево (папки+документы, `deleted_at IS NULL`). Гейт **`require("documents","view")`** + фильтр видимости. **Response 200** — массив `DocumentNode` (без `content_md`) в порядке `position ASC, created_at DESC, id`; клиент строит дерево по `parent_id`. **Ошибки:** `401 unauthorized`, `403 forbidden` (нет `documents:view`).

### GET `/api/documents/nodes?parent_id={uuid|—}`
Дети узла. `parent_id` не задан/пуст → узлы корня. Гейт **`require("documents","view")`** + фильтр. **Response 200** — `DocumentNode[]` (без `content_md`). **Ошибки:** `401`, `403`.

### GET `/api/documents/nodes/{id}`
Один узел (+`content_md` для документа). Гейт **`require("documents","view")`** + фильтр. **Response 200** — `DocumentNode`. **Ошибки:** `401`, `403`, `404 document_node_not_found` (нет ИЛИ невидим по роли).

### POST `/api/documents/folders`
Создать папку. Гейт **`require("documents","create")`**.
**Request** `{ "parent_id": "…"|null, "name": "Гайды" }` — `name` 1–255; `parent_id` — существующий видимый узел-папка или `null`.
**Response 201** — `DocumentNode` (`node_type="folder"`, `content_version=1`, `owner_id`=актор). **Ошибки:** `401`, `403`, `404 document_node_not_found` (невидимый/несуществующий `parent_id`), `400 validation_error`.

### POST `/api/documents/documents`
Создать документ. Гейт **`require("documents","create")`**.
**Request** `{ "parent_id": "…"|null, "name": "README", "content_md": "# …" }` — `content_md` опц. (default `""`); размер ≤ `DOCUMENTS_MAX_MD_BYTES`.
**Response 201** — `DocumentNode` (`node_type="document"`). **Ошибки:** `401`, `403`, `404 document_node_not_found`, `400 validation_error`, `422` (контент больше лимита — `validation_error`, поле `content_md`).

### POST `/api/documents/upload`
Загрузка `.md`-файла как документа (multipart). Гейт **`require("documents","create")`**.
**Request** `multipart/form-data`: `file` (**только `.md`**), `parent_id` (uuid|пусто), `name` (опц.; по умолчанию — имя файла без `.md`). Валидация: расширение `.md`, размер ≤ `DOCUMENTS_MAX_MD_BYTES`, декодируемость UTF-8.
**Response 201** — `DocumentNode`. **Ошибки:** `401`, `403`, `404 document_node_not_found` (`parent_id`), **`422 document_upload_invalid`** (не `.md` / размер / битый UTF-8).

### PATCH `/api/documents/nodes/{id}`
Переименование и/или правка контента. Гейт **`require("documents","edit")`** + видимость (`404` если невидим).
**Request** (любое подмножество) `{ "name"?: "…", "content_md"?: "…", "expected_version"?: 7 }`.
- Изменение `name`/`content_md` → **`content_version += 1`**, `updated_at = now()`.
- `expected_version` **опционален** ([TD-064](100-known-tech-debt.md)): если передан и ≠ текущему `content_version` → **`409 document_node_conflict`**; не передан → last-write-wins.
- `content_md` для папки → `422 validation_error` (папка не хранит контент).
**Response 200** — обновлённый `DocumentNode` с **`content_md: null`** (нормативно, [ADR-063](adr/ADR-063-documents-editor-cache-lifecycle-focus.md) §A): `PATCH` — операция **мутации**, а не чтения контента; ответ подтверждает изменение и несёт новую `content_version`, но контент в нём не отдаётся (тот же принцип, что в списках/дереве — см. таблицу [`DocumentNode`](#documents)). **Единственный read-канал контента — `GET /nodes/{id}`.**

> **Правило клиента (нормативно, [ADR-063](adr/ADR-063-documents-editor-cache-lifecycle-focus.md) §A).** Ответ этого `PATCH` **ЗАПРЕЩЕНО** класть в кэш узла целиком — это затрёт контент значением `null` (видимая потеря текста на экране при сохранении **и** при переименовании открытого документа). Ответ **сливается** поверх предыдущего значения кэша: `content_md` берётся из тела запроса, если оно там было (строгая проверка `!== undefined`; `''` — валидный контент), иначе — из предыдущего значения кэша; при отсутствии предыдущего значения частичная запись **не сеется**. Инвалидация узла на успешном пути не используется (лишний round-trip) — только в ветке `409`.

**Ошибки:** `401`, `403`, `404 document_node_not_found`, `409 document_node_conflict`, `400/422 validation_error`.

### POST `/api/documents/nodes/{id}/copy`
Рекурсивная копия узла (для папки — всё поддерево). Гейт **`require("documents","create")`** + видимость исходного узла.
**Request** `{ "target_parent_id": "…"|null }` — куда положить копию (default — тот же `parent_id`).
- Новые `id` всем узлам; **сохраняются** структура/`position`/`visibility_mode` и строки `document_node_roles`; `owner_id` копий = актор; `content_version` = 1. Одна транзакция.
- **Цикл:** `target_parent_id` = сам узел или его потомок → **`422 document_copy_cycle`**.
**Response 201** — корневой `DocumentNode` копии. **Ошибки:** `401`, `403`, `404 document_node_not_found` (исходный или `target_parent_id`), `422 document_copy_cycle`.

### PATCH `/api/documents/nodes/{id}/visibility`
Смена видимости узла. Гейт **`require("documents","share")`** + видимость.
**Request** `{ "visibility_mode": "restricted", "role_ids": ["…","…"] }`.
- `restricted` → `role_ids` задаёт **собственный** набор ролей узла (перезапись строк `document_node_roles` **данного** узла — ровно то, что читает `GET /nodes/{id}/visibility`); пустой `role_ids` при `restricted` — узел не виден никому, кроме admin-уровня (осознанно).
- `inherit` → `role_ids` игнорируется/пуст; строки `document_node_roles` узла удаляются (наследование/публичность).
- `role_ids` — существующие `roles.id` (несуществующий → `422 validation_error`).
**Response 200** — `DocumentNode`. **Ошибки:** `401`, `403` (нет `documents:share`), `404 document_node_not_found`, `422 validation_error`.

### GET `/api/documents/nodes/{id}/visibility`
Собственные настройки видимости узла — для **предзаполнения** модалки «Сменить видимость» (чекбоксы/`MultiSelect` ролей). Гейт **`require("documents","share")`** + видимость (`404` если невидим). **Дополнение контракта** ([ADR-059](adr/ADR-059-documents-module.md)): `GET /nodes/{id}` отдаёт `visibility_mode`, но **не** собственные роли узла, поэтому read-сторона модалки вынесена в отдельный share-gated под-ресурс, **симметричный** write-контракту `PATCH /nodes/{id}/visibility` (тот же путь, та же форма тела).
**Response 200** — `{ "visibility_mode": "restricted"|"inherit", "role_ids": ["…","…"] }`:
- `role_ids` — **СОБСТВЕННЫЕ** роли узла (строки `document_node_roles` **данного** узла), заданные при `restricted`, а **НЕ** эффективные/унаследованные от `restricted`-предка. Возвращают ровно то, что принимает write `PATCH /nodes/{id}/visibility` ⇒ форму можно отправить обратно без преобразования (read↔write симметрия).
- `restricted` → собственный набор `role_ids` узла; `inherit` → `role_ids = []` (строк `document_node_roles` у узла нет). Порядок не значим (множество); для детерминизма — `ORDER BY role_id`.

> Имя поля — `role_ids` (**не** `visibility_role_ids`): здесь оно зеркалит поле тела write-контракта `PATCH …/visibility` и несёт **собственные** роли узла. Внешнее `ExternalDocumentNode.visibility_role_ids` ([§External Documents](#external-documents-read-only-rag)) — иная семантика (**эффективный** набор для RAG-фильтрации, публичный → `[]`) и иной контур (X-API-Key); совпадения имён нет намеренно.

**Ошибки:** `401`, `403` (нет `documents:share`), `404 document_node_not_found`.

### PATCH `/api/documents/order`
Перестановка узлов **внутри одного уровня** (одного `parent_id`). Гейт **`require("documents","edit")`**. Полная перестановка — как reorder серверов; прецеденция ошибок — см. [«Прецеденция ошибок валидации»](#прецеденция-ошибок-валидации-нормативно-едино-для-всех-order-эндпоинтов).
**Request** `{ "parent_id": "…"|null, "ids": ["…","…","…"] }` — `ids` — **полная перестановка** множества видимых **не удалённых** детей `parent_id`.
Backend присваивает `position = 0..N-1` по индексу (в одной транзакции).
**Response 204** (без тела). Frontend инвалидирует `GET /api/documents/tree`.
**Ошибки:**
- `400 validation_error` — тело некорректно (не массив UUID). Первым.
- `404 document_node_not_found` — какой-либо `id` не существует/невидим. До полноты множества.
- `422 unprocessable` — все `id` существуют, но `ids` не полная перестановка детей `parent_id` (пропуск/дубль/лишний/чужой уровень).

### DELETE `/api/documents/nodes/{id}`
**Soft-delete** узла. Гейт **`require("documents","delete")`** + видимость. Папка — каскадный soft-delete **всего поддерева** (tombstone на каждый узел) в одной транзакции.
**Response 204**. **Ошибки:** `401`, `403`, `404 document_node_not_found`.

### GET `/api/documents/role-refs`
Лёгкий список ролей для модалки видимости. Гейт **`require("documents","share")`** (НЕ admin-gated `/api/roles`).
**Response 200** — `[{ "id": "…", "name": "Менеджеры" }, …]`. **Ошибки:** `401`, `403` (нет `documents:share`).

### Вложения (изображения) документов ([ADR-068](adr/ADR-068-documents-image-attachments.md))

Изображения, вставленные в документ. **Байты — файлом на volume**, метаданные — в `document_attachments` ([03-data-model.md](03-data-model.md#таблица-document_attachments-adr-068)); в `content_md` — обычная markdown-ссылка `![alt](/api/documents/attachments/{id})`. Безопасность — [05-security.md](05-security.md#вложения-изображения-документов-нормативно-adr-068).

**Схема `DocumentAttachment`:**

| Поле | Тип | Примечание |
|------|-----|-----------|
| `id` | uuid | |
| `document_node_id` | uuid | узел-владелец (обязательно `node_type='document'`) |
| `filename` | string | исходное имя (1–255) — для `Content-Disposition`/alt; **в пути на диске не участвует** |
| `mime` | string | `image/png` \| `image/jpeg` \| `image/webp` \| `image/gif` — определён **по содержимому** при загрузке |
| `size_bytes` | integer | |
| `checksum` | string | sha256 hex (64 симв.); он же `ETag` отдачи |
| `url` | string | **канонический адрес — `"/api/documents/attachments/<id>"`. Формирует сервер**; клиент его не конструирует (иначе форма ссылки стала бы неявным контрактом, размазанным по фронту) |
| `created_at` | string | ISO 8601 UTC |

#### POST `/api/documents/nodes/{id}/attachments`
Загрузка изображения в документ (multipart). Гейт **`require("documents","edit")`** + видимость узла (невидим → `404 document_node_not_found`).

> **Почему `edit`, а не `create`:** вложение — часть **контента существующего** узла, а `create` в этом модуле означает «создать узел» ([RBAC](#rbac-и-enforcement-прав)).

**Request** `multipart/form-data`: `file` (обязателен).
**Валидация (прецеденция сверху вниз):**
1. Узел — `node_type='document'` (папка контента не хранит) → иначе `422 validation_error`.
2. **Размер** ≤ `DOCUMENTS_MAX_IMAGE_BYTES` (env, default `5242880` = 5 МБ). Проверяется **по мере чтения потока** с обрывом на превышении — файл целиком в память до проверки не читается.
3. **Тип — по фактическому содержимому (magic bytes), НЕ по `Content-Type` клиента:** `\x89PNG\r\n\x1a\n`, `\xFF\xD8\xFF`, `RIFF….WEBP`, `GIF87a`/`GIF89a`. Заявленный тип вне whitelist **или** расхождение заявленного и фактического → отказ. Клиентский `Content-Type` подделывается тривиально, а сохранённый `mime` управляет заголовком отдачи.
4. Whitelist — ровно `image/png`, `image/jpeg`, `image/webp`, `image/gif`. **SVG исключён нормативно** ([ADR-068](adr/ADR-068-documents-image-attachments.md) §2.3): это активный документ (скрипты/`foreignObject`), отдаваемый с нашего origin ⇒ XSS-вектор, который не закрывают ни `nosniff`, ни CSP страницы.
5. Любой отказ п.2–4 → **`422 document_attachment_invalid`** (единый код).

**Порядок записи (нормативно, [ADR-068](adr/ADR-068-documents-image-attachments.md) §2):** стриминг во временный файл **в корне `DOCUMENTS_ATTACHMENTS_DIR`** (та же ФС, что финальный путь ⇒ `os.replace` останется атомарным) с подсчётом **sha256 и размера** → проверки → `INSERT` с **готовыми** `checksum`/`size_bytes` → `flush` (получен `id` ⇒ известен шард-путь) → `mkdir -p` шардов → `os.replace` → `commit`; сбой до `commit` → `rollback` + `unlink` temp. **Строки без файла не бывает.** ⚠️ Обратный порядок («`INSERT` → потом считать файл») невозможен: `checksum`/`size_bytes` — `NOT NULL` и известны только после вычитывания потока.

**Response 201** — `DocumentAttachment`. **Ошибки:** `401`, `403` (нет `documents:edit`), `404 document_node_not_found`, `422 validation_error` (узел-папка), `422 document_attachment_invalid`.

#### GET `/api/documents/attachments/{id}`
Отдача байтов. Гейт **`require("documents","view")`** + **тот же per-node фильтр видимости** узла-владельца (рекурсивный CTE, [05-security.md](05-security.md#видимость-документов-по-ролям-нормативно-adr-059)).

**Response 200** — тело файла. Заголовки:
- `Content-Type: <mime>` — **из БД**, не из имени файла;
- `Content-Length`, `Content-Disposition: inline; filename*=UTF-8''<pct-encoded filename>`;
- **`Cache-Control: private, max-age=300, must-revalidate`** + **`ETag: "<checksum>"`**; `If-None-Match` совпал → **`304`**.
  > **`public` ЗАПРЕЩЁН:** ответ зависит от прав запрашивающего (per-node видимость) ⇒ shared-кэш прокси отдал бы картинку постороннему.
- `X-Content-Type-Options: nosniff` уже ставит backend-middleware на весь `/api` ([05-security.md](05-security.md#http-заголовки-безопасности-нормативно)) — дублировать не нужно.

**Ошибки:** `401`, `403` (нет `documents:view`), **`404 document_attachment_not_found`** — вложения нет **ЛИБО** узел невидим по роли **ЛИБО** узел soft-deleted (**единый код**, анти-энумерация).

> **⚠️ Правило клиента (нормативно, [ADR-068](adr/ADR-068-documents-image-attachments.md) §3).** `<img src="/api/documents/attachments/{id}">` **напрямую не работает**: JWT живёт в `localStorage` ([ADR-041](adr/ADR-041-login-theme-session-ux.md)), браузер отправит запрос картинки **без** `Authorization` → `401`. Клиент обязан загрузить изображение **авторизованным `fetch`** и подставить `URL.createObjectURL(blob)`, освобождая его `URL.revokeObjectURL` при размонтировании. **Делать эндпоинт анонимным ради «просто `<img>`» ЗАПРЕЩЕНО** — это утечка картинки в обход видимости узла. Отсюда же расширение CSP `img-src` на `blob:` ([05-security.md](05-security.md#csp)).

#### DELETE `/api/documents/attachments/{id}`
Удаление вложения. Гейт **`require("documents","edit")`** + видимость узла-владельца.
Строка удаляется в транзакции, файл — **после успешного `commit`** (best-effort; сбой → лог + GC [TD-076](100-known-tech-debt.md)). Обратный порядок дал бы строку, указывающую в пустоту.
Ссылка в `content_md` **не переписывается автоматически** — редактор удаляет узел изображения сам; висячая ссылка деградирует до alt-текста, документ не ломается.
**Response 204**. **Ошибки:** `401`, `403`, `404 document_attachment_not_found`.

#### Вложения при копировании и soft-delete (нормативно)
- **`POST /nodes/{id}/copy`** — вложения копируются **физически** (новые `id`, новые файлы), и **в `content_md` копии ссылки переписываются** со старых `id` на новые: **литеральная** замена подстроки `/api/documents/attachments/<old_uuid>` → `…/<new_uuid>` по карте копирования. **Regex-разбор markdown запрещён** (ложные срабатывания в коде/цитатах). Всё — в той же транзакции, что и копия поддерева. Общий файл с refcount отклонён: копия перестала бы быть независимой ([ADR-068](adr/ADR-068-documents-image-attachments.md) §5).
- **`DELETE /nodes/{id}`** (soft-delete) — строки и файлы **не трогаются**; отдача вложения такого узла → `404 document_attachment_not_found`. Физическая уборка — вместе с GC tombstones ([TD-067](100-known-tech-debt.md)).

## External Documents (read-only, RAG)

Внешний машинный контур для RAG-базы ИИ ([ADR-060](adr/ADR-060-documents-external-readonly-api-key.md)). **Без JWT, CSRF-exempt, `Cache-Control: no-store`.** Аутентификация — статический ключ **`X-API-Key`** (env `DOCUMENTS_API_KEY`), сравнение constant-time (`hmac.compare_digest`).

**Порядок проверок (нормативно, образец [mail_ingest](#машинные-эндпоинты-push-агрегатор--crm)):** пустой `DOCUMENTS_API_KEY` → **`503 documents_external_not_configured`** → неверный/отсутствующий `X-API-Key` → **`401 not_authenticated`**.

**Машина видит ВСЕ узлы** (обходит per-role фильтр), каждый ответ несёт `visibility_role_ids[]` (эффективный набор ролей узла; публичный → `[]`) + `content_version`. Порядок в списках — компаундный keyset **`(updated_at, id)` ASC** (форвард-walk от водяного знака; техника — образец [mail-курсора](#get-apimailmessages), направление ASC).

**Форма элемента (`ExternalDocumentNode`):** `id`, `node_type`, `parent_id`, `name`, `visibility_role_ids: uuid[]`, `content_version`, `updated_at`, `deleted_at: string|null`. Поле `content_md` — только в `GET /{id}`.

> **Вложения-изображения ([ADR-068](adr/ADR-068-documents-image-attachments.md) §6) — контур НЕ расширяется, и это зафиксированное ограничение, а не пробел.** Бинарных эндпоинтов наружу нет (`/api/external/documents/attachments/{id}` **не существует**); `ExternalDocumentNode` полем `attachments[]` **не дополняется**. Обоснование: вложения **иммутабельны** — перезаписи файла не бывает, «замена картинки» = новая загрузка = новый `id` = изменение `content_md` = `content_version += 1` ⇒ `content_version` и водяной знак `updated_at` **уже полностью** описывают состав узла для RAG, и список метаданных не даёт ему ни одного нового решения. Ссылки `![alt](/api/documents/attachments/{id})` приходят в `content_md` **как есть**: RAG индексирует alt-текст и видит факт наличия изображения, байты не получает. Выдача бинарников наружу — [TD-074](100-known-tech-debt.md) (отдельный ADR при появлении требования).

### GET `/api/external/documents`
Список для синхронизации.
**Query:** `updated_after` (ISO 8601, опц. — фильтр `updated_at >= updated_after`), `include_deleted` (bool, default `false` — включать ли tombstones), `cursor` (непрозрачный keyset `(updated_at,id)`, опц.), `limit` (`1..500`, default `100`).
**Response 200** — `{ "items": ExternalDocumentNode[], "next_cursor": "…"|null }` (`items` в порядке `updated_at ASC, id ASC`; `next_cursor=null` — конец). **Ошибки:** `503 documents_external_not_configured`, `401 not_authenticated`, `400 validation_error` (битый cursor/limit).

### GET `/api/external/documents/{id}`
Полный узел с контентом.
**Response 200** — `ExternalDocumentNode` + `content_md` (для документа). **Удалённый узел → `410 document_node_gone`**, тело: `{ "error": { "code": "document_node_gone", "message": "…", "details": { "id": "…", "deleted_at": "…", "content_version": 12 } } }` (tombstone). **Ошибки:** `503`, `401`, `404 document_node_not_found` (никогда не существовал), `410 document_node_gone` (был удалён).

### GET `/api/external/documents/{id}/access`
Эффективный уровень доступа узла.
**Response 200** — `{ "id": "…", "is_public": false, "visibility_role_ids": ["…"], "content_version": 12 }`. **Ошибки:** `503`, `401`, `404 document_node_not_found`.

### GET `/api/external/documents/changes`
Дельта с водяного знака (изменённые + tombstones).
...
```json
{
  ...
  "mail_extra_teams": [ { "id": "9b2c...7e", "name": "Business 2" } ],
...
> **Per-channel наборы (нормативно, [ADR-055](adr/ADR-055-per-channel-teams-mail-sms.md)).** Видимость и действия в почте/СМС считаются по **объединению** базового членства и добавки канала. Базовую команду **нельзя** исключить из канала — она входит в scope обоих каналов всегда; блок канала задаёт только **дополнительные** команды. Поэтому в `user_channel_teams` **не хранятся** команды, входящие в `team_ids` того же пользователя: сервис **вычитает** базовый набор из присланного `*_extra_team_ids` (§2.3 ADR-055) — присланная базовая команда **не ошибка**, она просто не сохраняется.
...
> Удаление собственной учётки БД-администратором допустимо; его действующий JWT аннулируется на следующем запросе (`401`). Супер-админ (`.env`) не удаляется: его системная строка-якорь **невидима** для Users API — `DELETE`/`PATCH` по её `id` → `404 user_not_found` ([ADR-051](adr/ADR-051-superadmin-db-anchor-personal-state.md) §1.4). Авто-передача лидерства выполняется в той же транзакции до удаления (правило порядка — [Teams](#teams), [ADR-026](adr/ADR-026-teams-optional-leader-auto-transfer.md)).

#### Пользователь-владелец документов/вложений → `409 user_in_use` (нормативно)

**Удаление БЛОКИРУЕТСЯ, если пользователь — автор хотя бы одного узла документов или загрузил хотя бы одно вложение:**

| Источник блокировки | FK | Подтверждение в репозитории |
|---|---|---|
| `document_nodes.owner_id` ([ADR-059](adr/ADR-059-documents-module.md)) | `ON DELETE RESTRICT` | `backend/app/models/document_node.py:75-77`, `backend/alembic/versions/0029_document_nodes.py`; [03-data-model.md](03-data-model.md#таблицы-модуля-документы-document_nodes-document_node_roles) |
| `document_attachments.created_by` ([ADR-068](adr/ADR-068-documents-image-attachments.md)) | `ON DELETE RESTRICT` | `backend/app/models/document_attachment.py:85`, `backend/alembic/versions/0032_document_attachments.py:71-73`; [03-data-model.md](03-data-model.md#таблица-document_attachments-adr-068) |

- Исход — **`409 user_in_use`**, сообщение: **«Пользователь владеет документами или вложениями — удаление запрещено»**. Ответ **не перечисляет** конкретные узлы/вложения (в т.ч. невидимые актору по роли) — анти-энумерация модуля «Документы» ([ADR-059](adr/ADR-059-documents-module.md)) не ослабляется.
- **`500` недопустим.** Это тот же принцип, что для `409 role_in_use`: прикладной исход обязан быть **зеркалом FK `ON DELETE RESTRICT`**, иначе нарушение целостности всплывает наружу как `internal_error` ([ADR-051](adr/ADR-051-superadmin-db-anchor-personal-state.md) §1.5 — там гард роли специально сделан зеркалом FK по этой же причине).
- **Требование к реализации (нормативно):** `UserService.delete_user` **обязан** перехватывать `IntegrityError` от `commit`/`flush`, выполнять `session.rollback()` и поднимать `user_in_use`. Именно перехват, а не только проактивный `EXISTS`-запрос: перечень FK `RESTRICT` на `users.id` растёт (ADR-059 → ADR-068), и предварительная проверка рассинхронизируется с ним молча, а перехват — нет. Проактивный `EXISTS` (по `ix_document_nodes_owner_id` / `ix_document_attachments_created_by`) допустим **дополнительно** — как способ вернуть `409` без исключения в логе.
- **Откат затрагивает и авто-передачу лидерства:** она выполняется в той же транзакции **до** удаления ([ADR-026](adr/ADR-026-teams-optional-leader-auto-transfer.md)), поэтому `rollback` возвращает лидерство прежнему владельцу — после `409` состояние БД не изменено вовсе (частичного эффекта нет).
- **Soft-deleted документы тоже блокируют:** узлы в состоянии soft-delete — физические строки с `owner_id` ([ADR-059](adr/ADR-059-documents-module.md) — tombstones живут для RAG), поэтому «удалил все свои документы в UI» блокировку **не снимает**. Переназначение авторства/зачистка — прикладная операция, которой пока нет: [TD-077](100-known-tech-debt.md) (сам исход `409` реализован и покрыт тестами).

**Ошибки:** `401 unauthorized`, `403 forbidden`, `404 user_not_found`, **`409 user_in_use`**.
...
| `user_count` | integer | Число пользователей с этой ролью (`COUNT(users) GROUP BY role_id`, [ADR-022](adr/ADR-022-teams-nav-categories.md)). `≥1` → удаление запрещено (`409 role_in_use`). **Системная строка-якорь супер-админа (`is_system`) в счётчик НЕ входит** ([ADR-051](adr/ADR-051-superadmin-db-anchor-personal-state.md) §1.5) — но **гард удаления её учитывает** (зеркало FK `ON DELETE RESTRICT`): роль, которую держит якорь (по умолчанию `admin`), **не удаляется** — `409 role_in_use`, даже если `user_count = 0` |
| `created_at` / `updated_at` | datetime | Метки |
...
> **Дизамбигуация снята ([ADR-044](adr/ADR-044-mail-full-merge-into-crm.md)).** Прежнее различение «CRM-команды vs mail-«команды» (`groups` агрегатора)» **более не актуально**: групп в агрегаторе нет, `GET /api/mail/teams` и схема `MailTeam` **упразднены**. Команда в системе одна — CRM-команда (`/api/teams`); ящик закреплён за ней напрямую (`mail_accounts.team_id`).
...
| `mail_group_id` | integer \| null | **⚠️ ЛЕГАСИ-ОСТАТОК, на почту НЕ влияет** ([ADR-047](adr/ADR-047-mail-fix-pack.md) §0, [TD-051](100-known-tech-debt.md)). Поле физически присутствует в БД (`teams.mail_group_id`, миграция `0018`) и в контракте, но **ни один путь модуля «Почты» его не читает**: владение ящиком определяется исключительно `mail_accounts.team_id`, групп в агрегаторе нет ([ADR-044](adr/ADR-044-mail-full-merge-into-crm.md)). Не используется ни `MailScope`, ни секцией «Почты команды». Подлежит удалению вместе с колонкой — [TD-051](100-known-tech-debt.md). **Не задавать, не читать, новую логику на него не вешать** |
...
| `mailbox_count` | integer | Число почтовых ящиков команды (`COUNT(mail_accounts WHERE team_id = teams.id)`, [Mail](#mail), [ADR-048](adr/ADR-048-teams-mailbox-count-mail-row.md)). Может быть `0`. Считается **на лету** по индексу `ix_mail_accounts_team_id` (не хранится колонкой); агрегат для чипа «N почт» на карточке команды; список почт — `GET /api/teams/{id}/mailboxes`. Гейт — тот же `teams:view` (обоснование видимости — [ADR-048](adr/ADR-048-teams-mailbox-count-mail-row.md) §4) |
...
| `mail_group_id` | integer \| null? | **⚠️ ЛЕГАСИ-ОСТАТОК, на почту НЕ влияет** ([ADR-047](adr/ADR-047-mail-fix-pack.md) §0, [TD-051](100-known-tech-debt.md)). Опц., `ge=1`. Поле **принимается** (`app/schemas/team.py`) и проверяется на уникальность (`409 team_mail_group_taken`), но **никакого эффекта на модуль «Почты» не имеет**: ящик привязывается к команде через `mail_accounts.team_id` ([ADR-044](adr/ADR-044-mail-full-merge-into-crm.md)). **Не задавать.** UI-селектор «Почтовая группа» из формы команды убран |

> **⚠️ `mail_group_id` — мёртвое легаси-поле (нормативно, [ADR-047](adr/ADR-047-mail-fix-pack.md) §0).** Оно физически остаётся в контракте `/api/teams` и в БД (колонка `teams.mail_group_id`, миграция `0018`), но **для почты не значит ничего**: [ADR-044](adr/ADR-044-mail-full-merge-into-crm.md) упразднил группы агрегатора и перевёл владение ящиком на `mail_accounts.team_id`; предписанный им post-ETL-drop колонки **ещё не выполнен**. Ни `MailScope`, ни `GET /api/teams/{id}/mailboxes`, ни один mail-путь его не читают. Задавать/менять `mail_group_id` **не следует**; удаление поля из контракта + миграция — [TD-051](100-known-tech-debt.md).
...
> **⚠️ Нормализация per-channel добавок (нормативно, [ADR-055](adr/ADR-055-per-channel-teams-mail-sms.md) §2.3 — путь 2; security-инвариант).** `POST`/`PATCH /api/teams` — **второй** (наряду с Users CRUD) путь записи в `user_teams`. Сервис обязан **в той же транзакции** удалить строки **`user_channel_teams (user_id ∈ member_ids, channel ∈ {mail,sms}, team_id = <эта команда>)`**: участник команды не хранит её же как **добавку** канала. Без этого возникает **«висящий» доступ**: доп-команда `B` → пользователя добавили в `B` участником на `/teams` → затем исключили из `B` там же ⇒ строка `user_teams` удалена, а **добавка осталась** ⇒ он **продолжает** видеть и обрабатывать почту/СМС команды `B`. Инвариант «снял команду → потерял доступ в обоих каналах» обязан быть **path-independent** ([05-security.md §Per-channel scope](05-security.md#per-channel-scope-команд-нормативно-adr-055)).
...
**Ошибки:** `401 unauthorized`, `403 forbidden`, `400 validation_error`, `422 unprocessable` (невалидный `name`/несуществующий `leader_id`/`member_ids`), `409 team_name_taken`, `409 team_mail_group_taken` (легаси-поле `mail_group_id` занято другой командой — [TD-051](100-known-tech-debt.md); в штатном сценарии поле не передаётся и код не возникает).
...
| `member_ids` | string[]? | опц. Не передано → состав не менять; передано → **полностью заменяет** набор участников (каждый существует → `422`; без дублей). **Добавленные участники теряют эту команду как per-channel добавку** — нормализация `user_channel_teams` в той же транзакции ([ADR-055](adr/ADR-055-per-channel-teams-mail-sms.md) §2.3, путь 2; см. врезку в [`POST /api/teams`](#post-apiteams)) |
| `mail_group_id` | integer \| null? | **⚠️ ЛЕГАСИ-ОСТАТОК, на почту НЕ влияет** ([ADR-047](adr/ADR-047-mail-fix-pack.md) §0, [TD-051](100-known-tech-debt.md)). Опц., presence-семантика, `ge=1`; занят другой командой → `409 team_mail_group_taken`. Эффекта на модуль «Почты» **нет** (владение ящиком — `mail_accounts.team_id`, [ADR-044](adr/ADR-044-mail-full-merge-into-crm.md)). **Не задавать.** UI-селектор «Почтовая группа» из формы команды убран |
...
### DELETE `/api/teams/{id}`
Удаляет команду (hard delete). Гейт `require("teams","delete")`. Строки `user_teams` **и** `user_channel_teams` (per-channel добавки, [ADR-055](adr/ADR-055-per-channel-teams-mail-sms.md)) снимаются `ON DELETE CASCADE` ⇒ удалённая команда исчезает из scope **обоих** каналов автоматически; пользователи не удаляются.
...
Список почтовых ящиков команды — для detail-панели `/teams` (секция «Почты команды», ленивая загрузка). Гейт `require("teams","view")`. Решение — [ADR-044](adr/ADR-044-mail-full-merge-into-crm.md) §4; по образцу `GET /api/teams/{id}/numbers`.

> **Минимальная схема без учётных данных (нормативно, расширена [ADR-048](adr/ADR-048-teams-mailbox-count-mail-row.md) §2).** Держатель `teams:view` видит **состав** ящиков команды и их **слабо-чувствительный идентифицирующий контекст `number`/`app_name`** («Номер»/«Приложение», [ADR-047](adr/ADR-047-mail-fix-pack.md) §3), но **НЕ** IMAP/SMTP-креды/хосты и **НЕ** статус синка (`last_synced_at`/`last_sync_error`/`consecutive_failures`). Поэтому эндпоинт отдаёт схему `TeamMailboxItem` (`id`, `email`, `number`, `app_name`, `display_name`, `is_active`), **не** полный `MailMailbox`. Полный контекст (статус синка, управление) — на эндпоинтах страницы «Почты» под матрицей `mail:*` и `MailScope`. Гейт — `teams:view` (не `mail:view`), `MailScope` не применяется. Трейдофф симметричен [ADR-034](adr/ADR-034-teams-number-login-app.md) (`login`/`app_name` номера под `teams:view`) — обоснование в [ADR-048](adr/ADR-048-teams-mailbox-count-mail-row.md) §4 и [05-security.md](05-security.md#видимость-ящиков-в-get-apiteamsidmailboxes-adr-048).

**Резолв (нормативно, [ADR-044](adr/ADR-044-mail-full-merge-into-crm.md)).** Ящики читаются **из БД CRM**: `mail_accounts WHERE team_id = {id}`. **Ни `teams.mail_group_id`, ни агрегатор в резолве не участвуют** (групп-индирекция упразднена; прежний прокси в `GET /api/external/mailboxes?group_id=…` **отменён**). У команды без ящиков → пустой список (`{ "mailboxes": [] }`, не `404`). Доступность агрегатора на этот эндпоинт не влияет (`502` он не отдаёт).

**Схема `TeamMailboxItem`:**
```json
{ "id": 7, "email": "inbox@example.com", "number": "5108", "app_name": "Klyro Forge (Codex)", "display_name": "5108 Klyro Forge (Codex)", "is_active": true }
```
| Поле | Тип | Примечание |
|------|-----|-----------|
| `id` | integer | ID ящика (`mail_accounts.id`) |
| `email` | string | Адрес ящика |
| `number` | string \| null | **«Номер»** ящика (`mail_accounts.number`, [ADR-047](adr/ADR-047-mail-fix-pack.md) §3); `null` — не задан. Добавлено [ADR-048](adr/ADR-048-teams-mailbox-count-mail-row.md) §2 |
| `app_name` | string \| null | **«Приложение»** ящика (`mail_accounts.app_name`, [ADR-047](adr/ADR-047-mail-fix-pack.md) §3); `null` — не задано. Добавлено [ADR-048](adr/ADR-048-teams-mailbox-count-mail-row.md) §2 |
| `display_name` | string \| null | Производное имя ящика (`"<number> <app_name>"`, [ADR-047](adr/ADR-047-mail-fix-pack.md) §3, [TD-052](100-known-tech-debt.md)); `null` — нет. **В строке detail-панели `/teams` НЕ рендерится** ([ADR-048](adr/ADR-048-teams-mailbox-count-mail-row.md) §2/§3): его составляющие (`number`/`app_name`) показываются явно, дублировать имя не нужно. Поле сохранено в схеме (сужать контракт незачем) |
| `is_active` | boolean | Активен ли ящик (для кружка статуса; критерий статуса в detail-панели `/teams` — **только** `is_active`, полей ошибок синка схема не содержит) |

**Response 200** — `TeamMailboxesResponse`:
```json
{ "mailboxes": [ /* TeamMailboxItem[] */ ] }
```
- Ящики, у которых `mail_accounts.team_id = {id}`. Пагинации нет (NFR-1).

**Ошибки:** `401 unauthorized`, `403 forbidden`, `404 team_not_found` (нет такой CRM-команды).
...
**Видимость (scope, нормативно — [ADR-032](adr/ADR-032-sms-visibility-admin-full-catalog.md) в редакции [ADR-055](adr/ADR-055-per-channel-teams-mail-sms.md) §3).** `SmsScope(sees_all_teams, team_ids, includes_unassigned)`. «Видит все команды» ⇔ **`is_superadmin` ИЛИ роль владеет полным каталогом прав** (предикат **не изменён**); такой актор видит **все** SMS/номера, включая unassigned-номера и SMS удалённых номеров. **Прочие роли** видят SMS/номера по **текущей** принадлежности номера (не по снимку `sms_inbound.team_id`), причём:

- **`team_ids` = `user_teams` ∪ доп-команды канала `sms`** (`user_channel_teams`, [ADR-055](adr/ADR-055-per-channel-teams-mail-sms.md)) — прежнее «только `user_teams`» расширено;
- **`includes_unassigned`** (`users.sms_includes_unassigned`) — даёт доступ к **номерам без команды** (`team_id IS NULL`), которые прежде были доступны **только** admin-уровню;
- **предикат (единый для чтения и мутации):** `number.team_id IN team_ids` **OR** (`includes_unassigned` **AND** `number.team_id IS NULL`).

Запрос к `number_id`/`team_id`/`no_team` вне scope → **пустая страница/список** (анти-энумерация, не `403`/`404`); **мутация** вне scope → `403 forbidden` (`sms_number_service._require_mutation_scope`). Правило симметрично для `GET /api/sms/messages` и `GET /api/sms/numbers`. Требует `Principal.user_id` ([05-security.md](05-security.md#расширение-principal-полем-user_id-нормативно)).
...
| `team_id` | string (uuid)? | Фильтр по команде — по **текущей** принадлежности номера (номера команды → их `to_number`). Несуществующая/вне scope → пустая страница. **Взаимоисключающ с `no_team`** |
| `no_team` | boolean? | **Только SMS номеров БЕЗ команды** (`sms_phone_numbers.team_id IS NULL`) — [ADR-055](adr/ADR-055-per-channel-teams-mail-sms.md) §5.3. Отсутствует/`false` → фильтр не применяется. **Оба (`team_id` и `no_team=true`) заданы → `400 validation_error`.** Вне scope (не-admin **без** `includes_unassigned`) → **пустая страница** (анти-энумерация, не `403`) |
...
- Фильтры `number_id` и `team_id`/`no_team` **комбинируемы** (AND-пересечение множеств видимых `to_number`). Все опциональны.
...
**Ошибки:** `401 unauthorized`, `403 forbidden`, `400 invalid_cursor`, `400 invalid_limit`, **`400 validation_error`** (`team_id` и `no_team=true` одновременно).

### GET `/api/sms/numbers`
Список номеров. Гейт `require("sms","view")` + scope (admin-уровень — `is_superadmin` ИЛИ полный каталог прав — видит все, включая unassigned; прочие роли — номера команд своего **SMS-scope** (базовые ∪ доп-команды, [ADR-055](adr/ADR-055-per-channel-teams-mail-sms.md)) + бесхозные при `includes_unassigned`; [ADR-032](adr/ADR-032-sms-visibility-admin-full-catalog.md)). Пагинации нет (номеров немного, NFR-1); клиентский поиск по номеру.
...
Правка редактируемых полей `login`/`app_name`/`note`. Гейт `require("sms","edit")` + scope (не admin-уровень — номер обязан пройти **предикат scope**: команда номера ∈ `team_ids` (базовые ∪ доп-команды) **ИЛИ** номер бесхозный **И** `sms_includes_unassigned=true` — [ADR-055](adr/ADR-055-per-channel-teams-mail-sms.md) §3; иначе `403 forbidden`. **⚠️ Прежнее «unassigned-номер не-админу недоступен всегда» ОТМЕНЕНО** — бесхозный номер доступен носителю флага «Без команды». Admin-уровень — `is_superadmin` ИЛИ полный каталог прав, [ADR-032](adr/ADR-032-sms-visibility-admin-full-catalog.md)). `label` не редактируется.
...
Назначить/переназначить/снять команду у номера. Гейт `require("sms","transfer")` + scope.
...
| Поле | Тип | Правила |
|------|-----|---------|
| `team_id` | string (uuid) \| null | `null` → снять команду (unassigned); иначе привязать к команде. **Целевая команда проверяется не только на существование, но и по scope** — см. ниже |

**Правила scope (нормативно, [ADR-055](adr/ADR-055-per-channel-teams-mail-sms.md) §3.2; порядок = прецеденция):**

1. **Сам номер** обязан пройти предикат `SmsScope` (`number.team_id ∈ team_ids` **OR** (`includes_unassigned` **AND** `number.team_id IS NULL`)) → иначе **`403 forbidden`**.
2. **`team_id = null` (снять команду):** admin-уровень — всегда; **не-админ — только при `sms_includes_unassigned = true`**, иначе **`403 forbidden`**. Без флага актор безвозвратно выбросил бы номер из собственного scope (прежний [TD-060](100-known-tech-debt.md), **закрыт** этой нормой).
3. **`team_id = <uuid>`:**
   - **не-админ:** целевая команда обязана ∈ `SmsScope.team_ids` (базовые ∪ доп-команды) → иначе **`403 forbidden`**. Проверка scope выполняется **ПЕРВОЙ** ⇒ несуществующая команда не-админу тоже даёт **`403`** (анти-энумерация: «команды нет» неотличимо от «команда чужая»).
   - **admin-уровень** (`sees_all_teams`): существование команды → иначе **`404 sms_team_not_found`**.

> **⚠️ Разворот прежней нормы ([ADR-055](adr/ADR-055-per-channel-teams-mail-sms.md) §3.2, оформлен явно).** Прежде целевая команда валидировалась **только на существование** (`backend/app/services/sms_number_service.py:94-97` — `get_existing_ids` → `sms_team_not_found`), а `team_id=null` был доступен **любому** носителю `sms:transfer`, чей номер попал в scope. Это позволяло вынести номер в **любую** существующую команду (в т.ч. ту, где актор не состоит) — обходной путь, симметричный тому, ради запрета которого перенос **ящика** оставлен admin-only ([ADR-044](adr/ADR-044-mail-full-merge-into-crm.md) §4).

**Response 200** — обновлённый `SmsNumberItem` (при `team_id=null` → `team=null`).

**Ошибки:** `401 unauthorized`, **`403 forbidden`** (номер вне scope; целевая команда вне scope у не-админа; `team_id=null` у не-админа без `sms_includes_unassigned`), `404 sms_number_not_found`, `404 sms_team_not_found` (**admin-уровень**: переданный `team_id` не существует).

### DELETE `/api/sms/numbers/{id}`
Удалить номер. Гейт `require("sms","delete")` **+ тот же предикат scope, что у `PATCH`** ([ADR-055](adr/ADR-055-per-channel-teams-mail-sms.md) §3): не-админ удаляет номер своей/доп-команды, а **бесхозный номер — только при `sms_includes_unassigned=true`** (прежде — `403` всегда); вне scope → `403 forbidden`. ⚠️ Носитель флага «Без команды» вправе **удалять** любой ещё не распределённый номер — объём последствия зафиксирован в [05-security.md](05-security.md#риск-флага-без-команды-в-смс-нормативно-adr-055-31). История SMS сохраняется (`sms_inbound` не затрагивается); SMS удалённого номера остаются видны только actor'у admin-уровня (`sees_all_teams` — `is_superadmin` ИЛИ полный каталог прав, [ADR-032](adr/ADR-032-sms-visibility-admin-full-catalog.md)).
...
Привязка Telegram-аккаунта оператора к его CRM-пользователю. **Auth — только JWT** (любой валидный access-токен; вне матрицы `sms`, [ADR-030](adr/ADR-030-sms-module-full-merge.md) §7). **Супер-админ из `.env` привязать линк не может → `403 forbidden`** ([ADR-051](adr/ADR-051-superadmin-db-anchor-personal-state.md) §1.6: bootstrap-учётка остаётся console-only — Telegram-SSO для неё дал бы беспарольный, неотзываемый из UI путь к admin-уровню в обход `ADMIN_PASSWORD`). Основание «нет `uid`» устарело: с [ADR-051](adr/ADR-051-superadmin-db-anchor-personal-state.md) `Principal.user_id` непуст у **любого** принципала — гейт проверяет `principal.is_superadmin`.
...
**Ошибки:** `401 unauthorized` (нет JWT), `403 forbidden` (супер-админ из `.env` — [ADR-051](adr/ADR-051-superadmin-db-anchor-personal-state.md) §1.6), `401 invalid_init_data`, `401 init_data_expired`, `400 validation_error`.

...
| `documents` | `documents:view` (`/tree`, `/nodes`, `/nodes/{id}`; +per-node фильтр видимости) + `GET /role-refs` и `GET /nodes/{id}/visibility` → `documents:share` | `POST /folders`/`/documents`/`/upload`/`/nodes/{id}/copy` → `documents:create` | `documents:edit` (`PATCH /nodes/{id}`); `PATCH /nodes/{id}/visibility` → `documents:share` | `documents:edit` (`PATCH /order`) | `documents:delete` (soft-delete) ([ADR-059](adr/ADR-059-documents-module.md)) |

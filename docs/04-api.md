...
Фабрика `get_mail_scope` (`api/deps.py`, образец `get_sms_scope`) даёт `MailScope(sees_all_teams, group_ids, team_ids)` ([ADR-043](adr/ADR-043-lazy-mail-group-provisioning.md) §5): `sees_all_teams = is_superadmin OR permissions_subset(full_catalog_permissions(), permissions)`; `group_ids` = **непустые** `teams.mail_group_id` по командам пользователя из `user_teams`; `team_ids` = **все** команды пользователя из `user_teams` (включая с `mail_group_id IS NULL`). Граница безопасности — **backend**:
...
- **Создание ящика по `team_id`** (`POST /mailboxes`) при `sees_all_teams=false`: целевая `team_id` обязана ∈ `MailScope.team_ids` (участник команды), иначе `403 forbidden` ([ADR-043](adr/ADR-043-lazy-mail-group-provisioning.md) §5). Проверка по `team_ids`, а не `group_ids` — команда без группы (`mail_group_id IS NULL`) ещё не имеет `group_id`, но участник вправе завести ей **первую** почту (что и создаст группу лениво). **Осознанный мягкий оракул (принято):** несуществующая `team_id` → `404 team_not_found`, чужая существующая → `403 forbidden` — различимость приемлема, т.к. `team_id` — неперечислимый UUID (write-путь, не лента); анти-энумерация чтения (пустая страница) не ослаблена.
- **Перенос ящика между командами** (`PATCH .../{id}` со сменой `team_id`) — **ТОЛЬКО admin-уровень** (`sees_all_teams=true`), НЕ членство ([ADR-043](adr/ADR-043-lazy-mail-group-provisioning.md) §5; требование владельца «переносить может только админ»): при `sees_all_teams=false` смена `team_id` → `403 forbidden`, даже если пользователь состоит в обеих командах. Отдельного действия `mail:transfer` нет — гейт через admin-предикат (обоснование — [05-security.md](05-security.md#каталог-прав-канон-на-сервере)).
- **Мутация существующего ящика по `id`** (`PATCH` кредов/`is_active`, `DELETE`, `sync`) при `sees_all_teams=false`: целевой ящик обязан принадлежать группе из `group_ids`, иначе `403 forbidden` (не `404`).
...
| `403` | `forbidden` | Мутация/синк существующего ящика вне `MailScope` (не-admin, ящик чужой группы); **создание** ящика для команды вне `MailScope.team_ids` (не-admin, не участник команды); **перенос** ящика между командами при `sees_all_teams=false` (не-admin — перенос только admin-уровень, [ADR-043](adr/ADR-043-lazy-mail-group-provisioning.md) §5); нет действия в правах роли (RBAC `require`) |
...
| `404` | `team_not_found` | Переданная `team_id` (`POST /api/mail/mailboxes` / `PATCH .../{id}` со сменой команды) не существует в CRM ([ADR-043](adr/ADR-043-lazy-mail-group-provisioning.md) §2/§5) |
| `404` | `mail_group_not_found` | **Остаточный edge:** привязка `teams.mail_group_id` указывает на уже удалённую группу агрегатора (stale) — внешний `404 group_not_found` при создании ящика. С ленивым провижинингом ([ADR-043](adr/ADR-043-lazy-mail-group-provisioning.md)) обычный путь `group_id` в теле запроса больше не передаётся (заменён `team_id`), поэтому код возникает редко (только stale-binding). Проброс внешнего `group_not_found`, внешний контракт `ADR-0039` §2 |
...
Создание ящика для **CRM-команды** (прокси к внешнему `POST /api/external/mailboxes`). Требует JWT (`mail:create`). Внешний сервис перед сохранением проверяет соединение и SSRF-guard хостов; владелец ящика в агрегаторе — техпользователь `crm-service` (`ADR-0039` §Q-0039-1). Ответ — `Cache-Control: no-store`.

**Ящик привязывается к команде по `team_id` (UUID CRM), а не по `group_id`** ([ADR-043](adr/ADR-043-lazy-mail-group-provisioning.md) §2). **Ленивый провижининг группы:** если у команды `teams.mail_group_id IS NULL`, backend создаёт группу в агрегаторе (`POST /api/external/teams { name: team.name }`, mail-агрегатор `ADR-0042`), привязывает её к команде и создаёт ящик с этим `group_id`. Резолв — общий хелпер `ensure_team_mail_group(team_id)` через **compare-and-set (CAS) на `teams.mail_group_id`**, БЕЗ удержания row-lock через сетевой вызов (инвариант «одна команда — ровно одна группа», [ADR-043](adr/ADR-043-lazy-mail-group-provisioning.md) §2/§4):
1. прочитать `teams.mail_group_id` (без лока); `IS NOT NULL` → использовать как `group_id`;
2. `IS NULL` → создать группу в агрегаторе → `g` (сетевой вызов **вне** CRM-лока);
3. CAS-запись: `UPDATE teams SET mail_group_id = :g WHERE id = :team_id AND mail_group_id IS NULL RETURNING mail_group_id` — атомарный переход `NULL → значение`:
   - `rowcount = 1` (выиграли гонку) → `group_id = g`;
   - `rowcount = 0` (параллельная ветка уже записала) → перечитать `mail_group_id` победителя (`g_other`), привязать ящик к **его** группе; свою `g` реклеймить best-effort `DELETE /api/external/teams/{g}` (mail-агрегатор `ADR-0042`);
4. создать ящик `POST /api/external/mailboxes { group_id, ...creds }`.

Порядок — **CAS-запись `mail_group_id` ДО создания ящика**: сбой создания ящика не оставляет сироту (повтор реюзит уже привязанную группу). Двойного создания группы из ретрай-слоя нет — write-ретраи только на `Connect*` (ADR-038 §1).
...
| `team_id` | string (uuid) \| null? | опц., CRM-команда (`TeamListItem.id`), к которой привязать ящик. `null` — персональный ящик без команды. Для не-admin: `team_id` обязан ∈ `MailScope.team_ids` (участник команды), иначе `403 forbidden` ([ADR-043](adr/ADR-043-lazy-mail-group-provisioning.md) §5) |

**Response 201** — схема `MailMailbox` (расширенная, с полями статуса, включая `group_id` агрегатора — идентичность ящика; пароля нет).

**Ошибки:** `401`, `403 forbidden` (нет `mail:create` ИЛИ `team_id` вне `MailScope.team_ids`), `400 validation_error`, `404 team_not_found` (переданная `team_id` не существует в CRM), `404 mail_group_not_found` (stale-binding: `mail_group_id` команды указывает на удалённую группу — редко, [коды ошибок](#коды-ошибок-модуля)), `409 mail_conflict` (email уже заведён), `422 unprocessable`, `502 mail_unavailable` (агрегатор недоступен при создании группы или ящика), `503 mail_not_configured`.
...
| `team_id` | string (uuid) \| null? | presence-семантика: передано → сменить команду (перенос); `null` → открепить. **Перенос — ТОЛЬКО admin-уровень** (`MailScope.sees_all_teams=true`, [ADR-043](adr/ADR-043-lazy-mail-group-provisioning.md) §5): при `sees_all_teams=false` присутствие `team_id` → `403 forbidden` (участник, даже нескольких команд, переносить не вправе — требование владельца). Для admin целевая команда резолвится в `group_id` тем же `ensure_team_mail_group` (ленивое создание группы, если у целевой команды `mail_group_id IS NULL`) |

**Response 200** — схема `MailMailbox` (расширенная). **Ошибки:** `401`, `403 forbidden`, `400 validation_error`, `404 mail_mailbox_not_found` (неизвестный `id` ящика) / `404 team_not_found` (целевая `team_id` не существует в CRM), `409 mail_conflict`, `422 unprocessable`, `502 mail_unavailable`, `503 mail_not_configured`.
...
> **Почтовая группа при создании НЕ задаётся ([ADR-043](adr/ADR-043-lazy-mail-group-provisioning.md) §1).** Поле `mail_group_id` **удалено** из `TeamCreateRequest` (модель «команда = почтовая группа»). Новая команда всегда стартует `mail_group_id = NULL`; группа в агрегаторе создаётся **лениво** — по первому добавлению почты (`POST /api/mail/mailboxes` с этой `team_id`, [ADR-043](adr/ADR-043-lazy-mail-group-provisioning.md) §2). Ручная (пере)привязка к **существующей** группе доступна позднее через `PATCH /api/teams/{id}`.
...
**Ошибки:** `401 unauthorized`, `403 forbidden`, `400 validation_error`, `422 unprocessable` (невалидный `name`/несуществующий `leader_id`/`member_ids`), `409 team_name_taken`. **`409 team_mail_group_taken` на создании больше не возникает** (`mail_group_id` не принимается — [ADR-043](adr/ADR-043-lazy-mail-group-provisioning.md) §1; конфликт привязки возможен только на `PATCH`).
...
| `mail_group_id` | integer \| null? | опц. (presence-семантика: передано → изменить). **Ручная (пере)привязка к СУЩЕСТВУЮЩЕЙ группе** ([ADR-043](adr/ADR-043-lazy-mail-group-provisioning.md) §1 — сохранена в `PATCH`, убрана из `POST`): `integer` → привязать/сменить группу mail-агрегатора (свободна, иначе `409 team_mail_group_taken`); `null` → снять привязку к почте. Не передано → не менять. Для **новой** команды привязка обычно возникает автоматически (ленивый провижининг по первой почте, [ADR-043](adr/ADR-043-lazy-mail-group-provisioning.md) §2); этот селектор нужен, чтобы прицепить команду к **уже существующей** группе с ящиками ([ADR-038](adr/ADR-038-mail-headless-integration.md)) |

...
Третья вкладка `AppLayout` (`/mail`). Модуль — [modules/mail](modules/mail/README.md), решение — [ADR-012](adr/ADR-012-mail-read-through-proxy.md), контракт — [04-api.md#mail](04-api.md#mail). Read-through-прокси: фронт получает ленту из `GET /api/mail/messages` и отправляет ответ через `POST /api/mail/messages/{id}/reply`. Токены/шрифты — те же тёмные, что на остальных страницах.

### Лента писем

- **Список писем** (`MailListItem`) в порядке `id` DESC в пределах загруженных батчей ([пагинация модуля](modules/mail/README.md#пагинация-нормативно)). Каждый элемент:
  - **Отправитель:** `from_name` (если есть) + `from_addr` (моношрифт, вторичный цвет).
  - **Тема:** `subject`; при `null` — **«(без темы)»** вторичным цветом. Длинная тема — `truncate` с многоточием (усечение задизайнено; при раскрытии видна целиком).
  - **Дата:** `internal_date` — относительное время по [словарю относительного времени](#относительное-время-n-min-ago); полная дата — в раскрытом виде/подсказке.
  - **Теги:** `Badge` по каждому `tags[]`, фон/акцент — из `tag.color` (HEX). Текст бейджа — `tag.name`.
  - **Аккаунт-получатель:** подпись **«Получено на:»** + `mail_account.email` (или `display_name`) — вторичным цветом ([словарь](#локализация-страницы-почты)). Лейбл «Кому» здесь **не** используется — он только для поля `to` формы ответа.
- **Раскрытие письма** (клик) — панель/аккордеон с телом:
  - `body_html` (не пустой) → **sandbox-iframe**: `<iframe srcDoc={body_html} sandbox>` **без** `allow-scripts` и **без** `allow-same-origin` (изоляция недоверенного HTML — [ADR-012](adr/ADR-012-mail-read-through-proxy.md), [modules/mail](modules/mail/README.md#изоляция-html-тела-нормативно)). Высота iframe — адаптивная/со скроллом; внешние ресурсы письма не подгружаются (CSP + sandbox).
  - иначе → `body_text` в блоке с моношрифтом и сохранением переносов (`white-space: pre-wrap`).
  - `body_truncated=true` → пометка **«Письмо показано не полностью»** вторичным цветом. `body_present=false` → **«Тело письма недоступно»**.

### Форма ответа (reply)

- Кнопка **«Ответить»** на раскрытом письме открывает форму ответа (инлайн-панель или модалка):
  - поле **«Кому»** (`to`, префил — отправитель исходного письма; редактируемо), **«Копия»** (`cc`, опц.), **«Тема»** (`subject`, префил `Re: <subject>`);
  - **«Сообщение»** — примитив **`Textarea`** (`body`, обязательное, непустое);
  - кнопки **«Отмена»** (ghost) / **«Отправить»** (primary, loading-спиннер).
- Отправка → `POST /api/mail/messages/{id}/reply`. Успех → toast **«Ответ отправлен»**, форма закрывается. Ошибки: `404` → **«Письмо не найдено»**; `422`/`400` (пустое сообщение) → подсветка поля/сообщение; `502` → toast **«Почтовый сервис временно недоступен»**; общая → toast.

### Пагинация

- Кнопка **«Загрузить ещё»** внизу ленты — видна, пока `has_more=true`. Клик → запрос с `since_id = next_since_id` предыдущего ответа; новые письма мержатся, порядок отображения — новые сверху в пределах батча ([modules/mail](modules/mail/README.md#пагинация-нормативно)). Loading-состояние на кнопке при догрузке.

### Состояния UI страницы «Почты»

| Состояние | Поведение |
|-----------|-----------|
| **loading** | Skeleton-строки ленты. |
| **empty** | Нет писем — подсказка **«Писем пока нет»**. |
| **не настроено** (`503 mail_not_configured`) | Заглушка **«Сервис почт не настроен»** по центру; лента и форма reply не показываются, toast-спама нет. |
| **error** (`502 mail_unavailable`) | Сообщение **«Почтовый сервис временно недоступен»** + кнопка **«Повторить»**. |
| **reply отправляется** | Спиннер и `disabled` на кнопке «Отправить». |
...
| Аккаунт-получатель (подпись перед `mail_account.email` в ленте) | **Получено на:** |
| Тело обрезано (`body_truncated`) | **Письмо показано не полностью** |
| Тело недоступно (`body_present: false`) | **Тело письма недоступно** |
| Кнопка ответа | **Ответить** |
| Reply — поля | **Кому** (только поле `to` формы ответа), **Копия**, **Тема**, **Сообщение** |
| Reply — кнопки | **Отмена** / **Отправить** |
| Toast успех (reply) | **«Ответ отправлен»** |
| Кнопка догрузки | **Загрузить ещё** |
...
> **Дизамбигуация лейбла (нормативно).** Подпись аккаунта-получателя в ленте писем — **«Получено на:»** (перед `mail_account.email`). Лейбл **«Кому»** используется **только** для поля `to` в форме ответа (reply). Не смешивать: в ленте — «Получено на:», в форме ответа — «Кому».

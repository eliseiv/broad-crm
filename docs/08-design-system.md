...
- Клик по глазу → **on-demand** запрос к reveal-эндпоинту (`GET /api/servers/{id}/ssh-password` · `GET /api/proxies/{id}/password` · `GET /api/ai-keys/{id}/key` · `GET /api/ai-keys/{id}/billing-admin-key` · `GET /api/backends/{id}/api-key` · `GET /api/backends/{id}/admin-api-key`, [04-api.md](04-api.md#reveal-секретов-по-требованию-adr-035)) → показ `value` (моношрифт). Опц. кнопка **копировать** (`Copy`).
...
- После успеха — редирект на **первую доступную по правам вкладку** нового порядка навигации (permission-aware, [ADR-022](adr/ADR-022-teams-nav-categories.md); перечень — [«Плоский порядок листьев навигации»](#навигация-плоская-applayout), **без `dashboard`**; здесь он намеренно не повторяется — прежняя копия была неполной). Прежний дефолт `/dashboard` отменён ([Гейтинг навигации](#гейтинг-навигации-и-действий-по-правам-rbac-нормативно)); `/dashboard` доступен по прямому URL.
...
| 7 | **Юзеры бэков** | `/backend-users` | `backend-users:view` ([ADR-069](adr/ADR-069-backend-users-page-admin-contract.md)) |
| 8 | **Продукты и тарифы** | `/backend-economics` | `backend-economics:view` ([ADR-072](adr/ADR-072-crm-admin-api-v11-economics.md)) |
| 9 | **Пользователи** | `/users` | `me.is_admin_level` (admin-only, вне матрицы; [ADR-078](adr/ADR-078-me-is-admin-level.md)) |
| 10 | **Роли** | `/roles` | `roles:view` |
| 11 | **Команды** | `/teams` | `teams:view` |
| 12 | **Документы** | `/documents` | `documents:view` ([ADR-061](adr/ADR-061-documents-sidebar-two-panel-nav.md)) |
| 13 | **Рассылка** | `/broadcast` | `broadcast:view` ([ADR-076](adr/ADR-076-knowledge-bot-broadcast-and-admin-level.md)) |

> ⚠️ **Строка «Юзеры бэков» была пропущена в этой таблице при вводе страницы** ([ADR-069](adr/ADR-069-backend-users-page-admin-contract.md)) — пункт рендерился в `AppLayout`, но в нормативной таблице отсутствовал, из-за чего и число пунктов ниже было занижено. Дрейф закрыт [ADR-072](adr/ADR-072-crm-admin-api-v11-economics.md); **нормативный лейбл пункта навигации — «Юзеры бэков»** (в матрице каталога прав та же страница подписана «Пользователи бэков» — словарь `PAGE_LABEL`, [Страница «Роли»](#страница-роли); расхождение лейблов осознанное: в хэдере строка короче).

Порядок **сохраняет прежнюю логику группировки** (сначала «агрегатор» Почты+СМС, затем «мониторинг» Серверы/ИИ-ключи/Прокси/Бэки, затем «экономика бэков» Юзеры бэков/Продукты и тарифы, затем «пользователи» Пользователи/Роли/Команды), но **без визуальных заголовков-категорий и без дропдаунов** — просто плоский горизонтальный ряд. Порядок совпадает с плоским порядком листьев `DefaultRoute` (см. ниже).

- **Гейтинг видимости пунктов сохраняется (только UX).** Пункт виден ⇔ у пользователя есть доступ к нему (`<page>:view ∈ permissions`, или `is_superadmin`; `users` — `me.is_admin_level` из `GET /api/auth/me`, [ADR-078](adr/ADR-078-me-is-admin-level.md)). Без права пункт **не рендерится**. Spinner на месте пункта «Пользователи» **не** ставится (прежний `catalogPending` отменён). Граница безопасности — серверный `403 forbidden`.
...
- **Плоский порядок листьев навигации** (для резолва дефолт-маршрута, **без `dashboard`**, [DefaultRoute.tsx](../frontend/src/routes/DefaultRoute.tsx)) — **ЕДИНСТВЕННОЕ нормативное место этого перечня**: `mail, sms, servers, ai-keys, proxies, backends, backend-users, backend-economics, users, roles, teams, documents, broadcast`. **Порядок резолва DefaultRoute НЕ меняется** прежними ADR — `sms` остаётся сразу после `mail` ([ADR-030](adr/ADR-030-sms-module-full-merge.md)); **`documents` после `teams`** ([ADR-061](adr/ADR-061-documents-sidebar-two-panel-nav.md)); **`backend-users` после `backends`** ([ADR-069](adr/ADR-069-backend-users-page-admin-contract.md)); **`backend-economics` сразу после `backend-users`** ([ADR-072](adr/ADR-072-crm-admin-api-v11-economics.md)); **`broadcast` добавлен в конец** ([ADR-076](adr/ADR-076-knowledge-bot-broadcast-and-admin-level.md), существующие приоритеты сохранены).
- **Layout-ветка:** все пункты идут по **не-full-bleed** ветке `AppLayout` (обычный поток документа, `w-full px-6 py-8`), кроме `/mail` **и `/documents`** — **full-bleed** ([ADR-061](adr/ADR-061-documents-sidebar-two-panel-nav.md): `/documents` — двухпанельный сайдбар, [Страница «Документы»](#страница-документы-нормативно-adr-061)). `/dashboard` (по прямому URL) — тоже не-full-bleed.

### Деградация хэдера на узких вьюпортах (нормативно, [ADR-033](adr/ADR-033-flat-nav-theme-toggle-numbers-table.md))

Пунктов — **13** (включая «Юзеры бэков» — [ADR-069](adr/ADR-069-backend-users-page-admin-contract.md), «Продукты и тарифы» — [ADR-072](adr/ADR-072-crm-admin-api-v11-economics.md), «Документы» — [ADR-061](adr/ADR-061-documents-sidebar-two-panel-nav.md) и «Рассылка» — [ADR-076](adr/ADR-076-knowledge-bot-broadcast-and-admin-level.md)); контейнер — на всю ширину (`w-full px-6`). ⚠️ Прежнее утверждение «все пункты помещаются в хэдер на десктопе» относилось к 10 пунктам и **больше не гарантируется**: с ростом числа пунктов ряд может не влезать и на десктопе. Это **не** меняет решение — деградация одна и та же на любом вьюпорте, где ряд шире контейнера. **Выбранная деградация — горизонтальный скролл ряда навигации** при сохранении фиксированной высоты хэдера:
...
Права берутся из `GET /api/auth/me` (`{ role, is_superadmin, is_admin_level, permissions }`, [04-api.md](04-api.md#get-apiauthme)). **UI-гейтинг — только UX; безопасность обеспечивает сервер (`403 forbidden`).**
...
- **Пункты меню (нормативно, [ADR-033](adr/ADR-033-flat-nav-theme-toggle-numbers-table.md) — плоская навигация):** пункт виден ⇔ у пользователя есть доступ к нему: ресурсный/`roles`/`teams`/`broadcast` — `view ∈ permissions[page]` (или `is_superadmin`); `users` — `me.is_admin_level` из `GET /api/auth/me` ([ADR-078](adr/ADR-078-me-is-admin-level.md); фронт **не** пересчитывает покрытие каталога). Пункты — плоский ряд без категорий-дропдаунов ([Навигация](#навигация-плоская-applayout)); скрытый по правам пункт не рендерится. «Дашборд» **в меню отсутствует** (маршрут — по прямому URL под `dashboard:view`).
...
- **Дефолтный маршрут после логина (permission-aware, [ADR-022](adr/ADR-022-teams-nav-categories.md)) — «Дашборд» больше НЕ дефолт.** Резолвится в **первую доступную по правам вкладку** нового порядка листьев навигации (сверху вниз, **без `dashboard`**) — перечень **не дублируется здесь**, единственное нормативное место см. [«Плоский порядок листьев навигации»](#навигация-плоская-applayout) выше (⚠️ прежняя копия перечня в этом абзаце была неполной: в ней отсутствовали `sms` и `backend-users`; расхождение закрыто [ADR-072](adr/ADR-072-crm-admin-api-v11-economics.md)); берётся первый лист, к которому у пользователя есть доступ (`view`, а для `users` — `is_admin_level`). Если у пользователя **нет ни одного доступного листа** (и он не admin/superadmin) — заглушка **«нет ни одного раздела»** (нормативные строки — блок [«Заглушки «Недостаточно прав»»](#гейтинг-навигации-и-действий-по-правам-rbac-нормативно) выше). Прежний дефолт `/dashboard` ([ADR-017](adr/ADR-017-dashboard-client-aggregation-mail-server-filters.md)/[ADR-021](adr/ADR-021-rbac-users-roles.md)) отменён — `/dashboard` доступен только по прямому URL.
- **Page-level view-guard прямого доступа (нормативно, единообразно для ВСЕХ permission-gated страниц):** любая страница под RBAC, открытая **по прямому URL** (закладка/ручной ввод/перезагрузка) или **в результате навигации/редиректа**, при отсутствии у пользователя требуемого для неё права **рендерит страницу-заглушку «Недостаточно прав»** — а не полноценный контент, не редирект и не сброс сессии. Требуемое право по странице: `dashboard` → `dashboard:view`; `mail` → `mail:view`; `servers`/`ai-keys`/`proxies`/`backends` → `<page>:view`; **`backend-users` → `backend-users:view`** ([ADR-069](adr/ADR-069-backend-users-page-admin-contract.md)); **`backend-economics` → `backend-economics:view`** ([ADR-072](adr/ADR-072-crm-admin-api-v11-economics.md)); **`roles` → `roles:view`; `teams` → `teams:view`** ([ADR-022](adr/ADR-022-teams-nav-categories.md)); `users` → `me.is_admin_level` (страница «Пользователи» вне матрицы прав, [ADR-021](adr/ADR-021-rbac-users-roles.md), [ADR-078](adr/ADR-078-me-is-admin-level.md)); **`broadcast` → `broadcast:view`**. Показывается заглушка **«нет доступа к разделу»** (page-scoped подсказка — блок «Заглушки «Недостаточно прав»» выше; **НЕ** строка «нет ни одного раздела», т.к. доступ к другим разделам у пользователя может быть). Правило применяется ко ВСЕМ страницам под правами единообразно (Дашборд, Почты, Роли, Команды гейтятся так же). **`AdminRoute` для `/users` показывает эту заглушку («нет доступа к разделу»), а НЕ редиректит.** Источник гейта — `me.is_admin_level` из `GET /api/auth/me`; каталог ради этого гейта **не** запрашивается (прежние `catalogPending` / `needsPermissionsCatalog` / Spinner «Загрузка…» в `AdminRoute` **отменены**, [ADR-078](adr/ADR-078-me-is-admin-level.md)). Носитель `is_admin_level` видит `/users` (заглушка к нему не применяется). Это UI-гейт (UX); граница безопасности — серверный `403 forbidden` ([ADR-021](adr/ADR-021-rbac-users-roles.md), [ADR-076](adr/ADR-076-knowledge-bot-broadcast-and-admin-level.md)).
...
- **Дефолтный маршрут — permission-aware ([ADR-022](adr/ADR-022-teams-nav-categories.md)):** index-роут `/` и **fallback `*`** (несуществующий путь) резолвятся **той же permission-aware логикой**, что редирект после логина (см. «Дефолтный маршрут после логина» выше): **первая доступная по правам вкладка** нового порядка листьев (перечень — [«Плоский порядок листьев навигации»](#навигация-плоская-applayout), **без `dashboard`**; здесь он намеренно не повторяется — прежняя копия была неполной); если нет ни одного доступного листа — заглушка **«нет ни одного раздела»**. «Дашборд» **не** участвует в дефолт-резолве (убран из меню); `/dashboard` доступен только по прямому URL под `dashboard:view`. Прежняя базовая цель `/dashboard` ([ADR-017](adr/ADR-017-dashboard-client-aggregation-mail-server-filters.md)) отменена.
- **Layout-ветка:** `/dashboard` (по прямому URL), «Роли» (`/roles`), «Команды» (`/teams`) идут по **не-full-bleed** ветке `AppLayout` (обычный поток документа, `min-h-screen`, внутренний `w-full px-6 py-8`) — как «Серверы»/«ИИ - ключи». Full-bleed-режим — у `/mail` **и `/documents`** (`isFullBleed = pathname.startsWith('/mail') || pathname.startsWith('/documents')`, см. [Full-bleed](#full-bleed-layout-нормативно) и [Страница «Документы»](#страница-документы-нормативно-adr-061), [ADR-061](adr/ADR-061-documents-sidebar-two-panel-nav.md)).
...
4. **Блок оценочного остатка** (`AiKeyBalanceDisplay`, [ADR-070](adr/ADR-070-ai-key-estimated-balance-monitor.md)) — **между маской ключа и причиной ошибки**; рендерится **только** при `balance_monitoring_enabled = true`, иначе не занимает места вовсе. Состав — [«Блок оценочного остатка»](#блок-оценочного-остатка-adr-070-нормативно) ниже.
5. **Причина ошибки:** при `check_status = error` — строка с `error_message` (красный акцент).
6. **Действие:** кнопка **Удалить** (как на карточке сервера в ошибке).

> **Клик по карточке → `AiKeyDetailModal` ([ADR-035](adr/ADR-035-detail-view-secret-reveal.md)):** read-only **Название**/**Провайдер**/**Ключ** (`key_masked` + глаз-reveal полного ключа под `ai-keys:edit`); при включённом мониторинге остатка — тот же **блок остатка** и кнопка **«Обновить баланс после пополнения»** (только при `ai-keys:edit`), а в блоке «Информация» — вторая строка секрета **Admin API key**; снизу — сворачиваемая секция **«Бэки»** (бэки, использующие этот ключ; свёрнуто «Бэков: N» по `backend_count`, раскрытие → `GET /api/ai-keys/{id}/backends` — [ADR-040](adr/ADR-040-backend-relations-secrets-reverse-lookup.md)). Карандаш → `AddAiKeyModal mode='edit'`. Паттерн — [Detail-view карточных страниц](#detail-view-карточных-страниц-read-only--карандаш--edit-adr-035).

### Блок оценочного остатка ([ADR-070](adr/ADR-070-ai-key-estimated-balance-monitor.md), нормативно)

Под-карточка `--surface-2`, граница `--border-subtle`, скругление `rounded-sub`. **Отдельная от health-бейджа сущность** — цвет остатка задаёт `balance_alert_level`, а не `check_status`; смешивать их запрещено (сломанный ключ может иметь нормальный остаток и наоборот).

| Элемент | Правило |
|---------|---------|
| Заголовок | **«Оценочный остаток»** (не «Баланс» — величина оценочная, [ADR-070](adr/ADR-070-ai-key-estimated-balance-monitor.md) «Ограничения») |
| Сумма | `balance_remaining_usd` в формате `$0.00`; `null` → `—`. Тон: `depleted` → `--status-red`, `low` → `--status-yellow`, `normal` → `--status-green`, `null` → `--text-primary` |
| Прогресс-бар | Доля `остаток / якорь`, клампится в `0…100 %`; рендерится только при заданных якоре (`> 0`) и остатке. `role="progressbar"` + `aria-label="Доля оставшегося баланса"` + `aria-valuenow/min/max` (a11y, NFR-8: значение доступно не только цветом) |
| Подпись | **«Из $X · порог $Y · синхр. <относительное время>»**; при `balance_last_sync_at = null` вместо времени — **«· ожидание синхронизации»**. Порог `null` отображается как `$10.00` |
| Ошибка синхронизации | При `balance_sync_status = "error"` — строка `balance_sync_error` цветом `--status-red` (текст приходит готовым из API, дополнительной локализации нет) |

**Форма (add + edit), блок `BalanceFormFields`:** чекбокс **«Мониторинг оценочного остатка»** + поясняющий текст; при включении — **«Текущий баланс, $»**, **«Порог уведомления, $»** (префил `10`, подсказка «Алерт при остатке ниже этой суммы») и **«Admin API key»** (password + глаз). В режиме **add** подсказка Admin-поля — «Admin key из Organization → Admin keys (не inference sk-…)», в режиме **edit** — «Оставьте пустым, чтобы не менять Admin key» (секрет не префилится, как и «Ключ»).
...
| Модалка (edit) — кнопки | **Отмена** / **Сохранить** |
| Блок остатка — чекбокс формы | **Мониторинг оценочного остатка** ([ADR-070](adr/ADR-070-ai-key-estimated-balance-monitor.md)) |
| Блок остатка — поля формы | **Текущий баланс, $**, **Порог уведомления, $**, **Admin API key** |
| Блок остатка — заголовок на карточке | **Оценочный остаток** |
| Блок остатка — подпись под суммой | **Из $X · порог $Y · синхр. …** / **· ожидание синхронизации** |
| Detail — кнопка переякорения | **Обновить баланс после пополнения** (только при `ai-keys:edit`) |
| Модалка переякорения — заголовок / поле / кнопки | **Обновить баланс** / **Текущий баланс, $** / **Отмена** / **Сохранить** |
| Detail — строка секрета биллинга | **Admin API key**, маска **Admin key (скрыт)** |
...
## Страница «Продукты и тарифы» (нормативно, [ADR-072](adr/ADR-072-crm-admin-api-v11-economics.md))

Пункт **«Продукты и тарифы»** плоской навигации (`/backend-economics`, #8 ряда), гейт **`backend-economics:view`** (page-level view-guard — [Гейтинг](#гейтинг-навигации-и-действий-по-правам-rbac-нормативно)). Экономика внешнего бэка: сколько токенов даёт продукт и сколько токенов списывается за генерацию. Модуль — [modules/backend-economics](modules/backend-economics/README.md), API — [04-api.md#backend-economics](04-api.md#backend-economics). Layout — **не-full-bleed** (обычный поток документа, `w-full px-6 py-8`, как «Бэки»/«Серверы»). Токены/шрифты — общие.

### Layout

Сверху вниз, одной колонкой:

1. **Селектор приложения** — `Select` ([Компонент `Select`](#компонент-select)), опции из `GET /api/backend-economics/backends` (`{id, code, name}`; `value` опции — `id`).

   **Подпись опции (нормативно, ЕДИНАЯ для обеих страниц): `«{name} — {code}»`** — одна строка обычного текста, без раздельного начертания частей.
   > ⚠️ **Прежняя редакция («подпись — `name`, вторичным моно — `code`») НЕДОСТИЖИМА и снята.** `ui/Select` рендерит **нативный `<option>`** (`frontend/src/components/ui/Select.tsx:66-70` — `<option …>{opt.label}</option>`), а нативный `<option>` принимает только плоский текст: вторичное начертание/моношрифт **для части подписи невозможны в принципе**, а не «не сделаны». Это не долг — норма была ошибочной.
   > **Порядок частей — `name` первым:** он совпадает с сортировкой списка (`name ASC`, tie-break `code`) и повторяет уже принятую идиому страницы «Бэки», где `code` **уникален, а `name` — нет**, поэтому дубли имён стоят рядом и различаются **хвостовым** `code`. ⚠️ Это **разворачивает** порядок, сложившийся на соседней странице «Юзеры бэков» (`frontend/src/pages/BackendUsersPage.tsx:35` — `` `${code} — ${name}` ``): обе страницы обязаны использовать **`{name} — {code}`** и **общий хелпер** (сейчас формат продублирован в двух местах и уже разошёлся). Приведение соседней страницы — **санкционированное этой нормой изменение**, а не drive-by. **Пункта «Все приложения» НЕТ**: правка возможна только в конкретном бэке, а без выбранного источника обе таблицы бессмысленны. Пока бэк не выбран — таблицы не запрашиваются и не рендерятся, вместо них — строка-подсказка (см. словарь).
2. **Таблица «Продукты»**.
3. **Таблица «Тарифы списания»**.

Обёртки и шапки обеих таблиц — по образцу таблицы страницы «Юзеры бэков»: карточка-контейнер `overflow-x-auto rounded-card border border-border-subtle bg-surface-1`, внутри `<table class="w-full min-w-[…] text-left text-sm">`, шапка — `<thead>` со строкой `border-b border-border-subtle text-[12px] uppercase tracking-wide text-text-tertiary`; моноширинный `product_id` (образец — `frontend/src/pages/BackendUsersPage.tsx:239-243`).

> ⚠️ **`sticky`-шапки таблицы НЕТ — ни у образца, ни здесь.** Прежняя редакция этой строки предписывала «`sticky`-шапка по образцу», но у образца её нет (`BackendUsersPage.tsx:240-261` — на `<thead>`/`<th>` классов `sticky` не стоит), то есть требование ссылалось на несуществующий образец. Вводить её здесь **не следует**: страница **не-full-bleed**, а хэдер приложения на таких страницах уже `sticky top-0 z-30` (`frontend/src/components/AppLayout.tsx:100`) — `sticky thead` без собственного z-контекста уехал бы под него. Липкая шапка таблиц — самостоятельное решение для **всех** табличных страниц сразу, а не побочный эффект новой страницы. **Состояния двух таблиц независимы** — отказ одной не гасит другую.

### Таблица «Продукты»

Над таблицей — **переключатель «Показать архивные»** (`ui/Checkbox`, по умолчанию **выключен**, состояние клиентское и **не персистентное**). Выключен ⇒ показываются только активные (`archived !== true`); включён ⇒ активные **и** архивные, архивные помечены (см. колонку «Продукт»). Переключатель **не рендерится**, если ни один элемент не несёт поля `archived` (у бэка нет понятия архива — показывать нечего). Нормативно — [ADR-073](adr/ADR-073-products-archive-and-price-columns.md) §3.

| Колонка | Содержимое | Гейт |
|---------|------------|------|
| **Продукт** | `product_id` (моно) + `name` вторичным; у архивного (`archived: true`) — пилюля **«В архиве»** (`ui/Pill tone="neutral"`, текстом, не только цветом) | — |
| **Токены** | **инлайн-редактируемое** число (`tokens`) — `InlineEditField`, карандаш → ввод → `Check`/`X`. **`null` → `—`, строка read-only:** карандаш **не рендерится ни при каком праве и ни при каких `features`** (бэк не отдал текущее значение — править нечего) | `backend-economics:edit` **И** `products.write_tokens` ∈ `features` **И** `tokens !== null` |
| **Аватар-токены** | **инлайн-редактируемое** число (`avatar_tokens`); **`null` → `—`**, карандаш **не рендерится** (вторая валюта у бэка отсутствует **либо** поле не отдано) | то же, с `avatar_tokens !== null` |
| **Выдаётся** | `grantable`: **«Да»** / **«Нет»** (текстом, не только цветом). **`null` → `—`**, а **НЕ** «Нет»: «не отдано» ≠ «не выдаётся» (тот же принцип, что у себестоимости) | — |
| **Обновлено** | `updated_at` (`DD.MM.YYYY HH:MM`) или **«не менялось»** (`null`/поле не отдано; в этом случае `if_updated_at` в `PATCH` **не отправляется**) | — |
| _(без заголовка)_ | **контрол архива** — компактная иконка-кнопка в последней колонке: у активного «Архивировать» (`Archive`), у архивного «Вернуть из архива» (`ArchiveRestore`); `aria-label` — по словарю. Действие **инлайн и без подтверждения** (обратимо тем же контролом); во время запроса — `disabled` + `Spinner` | `backend-economics:edit` **И** **`products.write_archived` ∈ `features`** **И** элемент несёт `archived` |

> **Колонок «Цена» и «Период» БОЛЬШЕ НЕТ — это решение, а не дефект** ([ADR-073](adr/ADR-073-products-archive-and-price-columns.md) §2). Поля `price`/`period` остаются в контракте и в схеме ответа CRM (они часть v1, универсальный контракт их не теряет), но **не отображаются**: у известных бэков они всегда `null`, а колонка из одних `—` не информирует, а имитирует данные и отнимает ширину у значимых столбцов. Возврат колонок возможен — но только вместе с бэком, который эти значения реально заполняет, и отдельным решением.
>
> **Контрол архива НЕ подчиняется правилу «`tokens: null` ⇒ строка read-only»** ([ADR-073](adr/ADR-073-products-archive-and-price-columns.md) §4): то правило защищает от слепой установки НЕИЗВЕСТНОГО значения, а текущее `archived` известно ⇒ строка с `tokens: null` архивируется штатно. Правилу **`capabilities: null` ⇒ read-only** контрол подчиняется наравне со всеми.

> **Бэк уровня v1 — нормативное поведение таблицы** ([ADR-072](adr/ADR-072-crm-admin-api-v11-economics.md) §1.1, [04-api.md](04-api.md#get-apibackend-economicsbackend_idproducts)). `GET …/products` — **v1-путь**, на бэке без расширения он отвечает **`200`** без новых полей, поэтому «жёлтый блок по `404`» здесь **недостижим**, и содержимое колонок обязано быть задано явно: **(1)** отсутствующее поле нормализуется в `null` и рендерится по правилам колонок выше; **(2)** если `items` **непуст** и **ни один** элемент не несёт `tokens` — таблица целиком переходит в **тот же информационный жёлтый блок**, что и «Тарифы списания» (обе таблицы на v1-бэке выглядят одинаково, потому что причина одна); **(3)** `items: []` под правило (2) **не подпадает** — это empty state «Продуктов нет».
>
> ⚠️ **Асимметрия с таблицей «Тарифы списания» намеренная:** `/pricing` существует только в v1.1, его поля обязательны, и их отсутствие — не `—`, а `502` (contract mismatch).

### Таблица «Тарифы списания»

| Колонка | Содержимое | Гейт |
|---------|------------|------|
| **Тип генерации** | подпись по **`kind`** (словарь ниже); при `kind="other"` — `name`, а при пустом `name` — `tariff_id` (моно) | — |
| **Токенов за генерацию** | **инлайн-редактируемое** число (`tokens`, допускает дробное) | `backend-economics:edit` **И** `pricing.write_tokens` ∈ `features` |
| **Обновлено** | `updated_at` или **«не менялось»** | — |

`tariff_id` — **opaque**: используется как ключ строки и путь `PATCH`, разбирать его на части UI не вправе.

### Контракт инлайн-правки (нормативно)

- **Инлайн, а не модалка** ([ADR-072](adr/ADR-072-crm-admin-api-v11-economics.md) §8): правится один скаляр в ячейке, `PATCH` **идемпотентен**. Модалки соседнего модуля существуют потому, что там операции неидемпотентны; здесь модалка — трение без выигрыша.
- **Карандаш виден постоянно** при наличии права и фичи (не по hover) — как в таблице номеров СМС. **Нет `edit` ИЛИ нет соответствующей фичи в `features` ИЛИ значение ячейки `null` ⇒ карандаш не рендерится вовсе** (ячейка — только просмотр). **`capabilities: null` = «фич не подтверждено»** — это **любой** неуспех подзапроса `/capabilities` (404, таймаут, `5xx`, `401`/`403`, невалидный JSON), а не только «бэк его не реализует» ⇒ **вся страница read-only при любом праве** (fail-closed, [ADR-072](adr/ADR-072-crm-admin-api-v11-economics.md) §7.1). **Держателю `edit` это состояние обязано быть объяснено подсказкой** (см. таблицу состояний и словарь) — исчезнувший карандаш сам по себе неотличим от отсутствия права.
- **Валидация — в обработчике сохранения страницы, не в примитиве:** пусто / не число / отрицательное / нецелое (для `tokens` продукта и `avatar_tokens`) → **`toast.error`** с текстом из словаря, запрос **не уходит**, **значение ячейки не меняется**. **Границы — по `limits` из `capabilities`** (замороженный состав: `product_tokens_max`, `product_avatar_tokens_max`, `tariff_tokens_max`, `tariff_decimal_places` — [04-api.md](04-api.md#get-apibackend-economicsbackend_idproducts)): **отсутствующий ключ ⇒ по нему клиентская проверка НЕ выполняется** (полагаемся на `400` бэка); `limits` нет целиком или `capabilities: null` ⇒ проверки границ нет вовсе, **форма остаётся работоспособной** (правка не блокируется отсутствием лимитов). Проверки «число / целое / `≥ 0`» от `limits` не зависят и действуют всегда. Причина выноса валидации в вызывающий код — у примитива нет пропа ошибки и числового режима ([TD-079](100-known-tech-debt.md)); **сам примитив в этом изменении не трогается**.
  > ⚠️ **Поле после неуспешной валидации ВОЗВРАЩАЕТСЯ В ПРОСМОТР — оно НЕ остаётся в режиме правки.** Прежняя редакция этого пункта требовала «поле остаётся в режиме правки» **одновременно** с «примитив не трогается», и это требование **невыполнимо**: `InlineEditField` закрывает режим правки **безусловно** (`frontend/src/components/InlineEditField.tsx:45-48` — `commit()` вызывает `onSave(...)` и сразу `setEditing(false)`), вызывающий код удержать поле открытым не может. Требование **снято как противоречивое**. Удержание поля открытым при ошибке — часть [TD-079](100-known-tech-debt.md) (проп ошибки у примитива) и вводится **вместе с ним, отдельной задачей**. До закрытия TD-079 норма — выполнимое подмножество: **тост + запрос не ушёл + значение не изменилось + возврат в просмотр**.
- **Успех** → тост с **дельтой** и с **задержкой применения** из `effective_after_seconds`. Дельта обязательна: правка глобальна и необратима, подтверждающей модалки нет (решение владельца). **Строка тоста выбирается по тому, КАКИЕ поля были отправлены** — три нормативных варианта в [словаре](#локализация-страницы-продукты-и-тарифы-нормативный-словарь): только `tokens`, только `avatar_tokens`, оба сразу.
  > ⛔ **`previous_tokens` относится ИСКЛЮЧИТЕЛЬНО к `tokens` — подставлять его в дельту аватар-токенов ЗАПРЕЩЕНО.** В замороженном контракте авторитетного «прежнего» значения для `avatar_tokens` **нет** (`previous_avatar_tokens` не существует — [04-api.md](04-api.md#patch-apibackend-economicsbackend_idproductsproduct_id), [ADR-072](adr/ADR-072-crm-admin-api-v11-economics.md) §8). Подстановка даёт **ложное утверждение о другой денежной величине**: правка аватар-токенов `50 → 60` при `tokens = 1000` показала бы «Токены продукта: 1000 → 60», после чего оператор «исправляет» токены продукта и реально меняет начисление всем будущим покупкам.
  > **Прежнее значение аватар-токенов берётся из КЛИЕНТСКОГО СНИМКА** — из значения `avatar_tokens`, которое было отрисовано в ячейке **до** правки (именно его оператор и видел; новое приходит в ответе). Это честный источник для утверждения «что вы изменили», но **не авторитетный** — [TD-080](100-known-tech-debt.md). Если прежнее значение клиенту недоступно (перерисовка, прямой переход) — показывается **вариант без дельты** (см. словарь), а **не** дельта с подставленным `previous_tokens`.
- **`changed: false`** (повтор того же значения) → **нейтральный** тост «Значение не изменилось», **не** ошибка.
- **Двойной сабмит** блокируется на время запроса (`saving`), как в существующих инлайн-полях.

### Состояния (нормативно — реализовать ВСЕ)

| Состояние | UI |
|-----------|-----|
| **Список бэков ПУСТ** (`GET …/backends` → `items: []` — ни у одного бэка не задан Admin API Key) | **Строка-подсказка из словаря вместо таблиц и вместо селектора**: селектор с единственным плейсхолдером **не рендерится** (выбирать нечего — контрол без единой опции вводит в заблуждение). Запросов к спискам нет. **Это НЕ ошибка** (кнопки повтора нет): состояние чинится в другом месте — в карточке бэка на странице «Бэки» |
| Бэк не выбран (список бэков **непуст**) | Строка-подсказка вместо таблиц; запросов нет |
| loading | Skeleton строк таблицы (обе — независимо) |
| error (общий) | Инлайн-блок ошибки + кнопка **«Повторить»** |
| **`backend_admin_extension_not_supported`** | **Информационный ЖЁЛТЫЙ блок** (`--status-yellow`, тонировка как у существующих warn-блоков) с текстом из словаря. **Кнопки повтора НЕТ** — повтор ничего не изменит, требуется обновление бэка. **Это НЕ красная ошибка** и **НЕ** «пользователь не найден». Так этот код обрабатывается **только когда он пришёл на `GET`** — то есть для таблицы «Тарифы списания» (у v1-бэка пути `/pricing` нет вовсе). Для «Продуктов» на `GET` он не приходит (`/products` — v1-путь, отвечает `200`; состояние выводится из данных — см. следующую строку) |
| **Бэк уровня v1: `200`, `items` непуст, НИ ОДИН элемент без `tokens`** (только таблица «Продукты») | **ТОТ ЖЕ информационный жёлтый блок**, что строкой выше — состояние выводится из **данных**, а не из кода ошибки ([ADR-072](adr/ADR-072-crm-admin-api-v11-economics.md) §1.1 п.4). Обе таблицы на v1-бэке выглядят одинаково. **`items: []` сюда НЕ относится** (empty state) |
| **Бэк уровня v1, ЧАСТИЧНО** (часть элементов несёт `tokens`, часть — нет) | Таблица рендерится; элементы без `tokens` — ячейки **`—`** и **read-only** строка (карандаша нет ни при каком праве). Жёлтого блока **нет** |
| **`backend_admin_extension_not_supported` пришёл на `PATCH`** (список уже загружен, `tokens` непусты, карандаш был показан) | **ТОСТ-ошибка с сообщением кода** («Бэк не поддерживает расширение контракта»), строка возвращается в просмотр, **значение не меняется**. **Подменять уже загруженную таблицу информационным блоком ЗАПРЕЩЕНО:** блок утверждает «Бэк не отдаёт продукты и тарифы», что на непустой таблице — **заведомая ложь** (продукты он отдал, не реализована только запись). Карандаши при этом **остаются на месте** — скрывать их до следующего `GET` **не требуется**: состояние означает, что бэк объявил `products.write_tokens` в `features` и не реализовал `PATCH`, то есть **нарушил собственный контракт** ([ADR-072](adr/ADR-072-crm-admin-api-v11-economics.md) §7); заводить ради этого клиентское состояние — усложнение из-за дефекта контрагента. Повторная попытка просто повторит тост |
| **`capabilities: null`** (любой неуспех подзапроса `/capabilities`: 404, таймаут, `5xx`, `401`/`403`, битый JSON) | Списки **рендерятся нормально** (это **не** ошибка страницы), но **все контролы правки скрыты** при любом праве (fail-closed). **Обязательна нейтральная подсказка** — строка из словаря, **рендерится ТОЛЬКО при `capabilities === null` И наличии `backend-economics:edit`**: без неё состояние неотличимо от «у меня нет права» и от «бэк не умеет писать» (карандаши просто исчезли, а лог виден разработчику, не оператору). **Не** ошибка, **без** кнопки повтора, нейтральный тон (`--text-secondary`, не `--status-red`). Держателю только `view` подсказка **НЕ показывается** — ему нечего объяснять. Техническая причина уходит в лог backend событием `backend_admin_capabilities_unavailable` ([ADR-072](adr/ADR-072-crm-admin-api-v11-economics.md) §7.1) |
| `backend_admin_key_not_set` | Инлайн-блок с подсказкой «задайте Admin API Key в карточке бэка»; кнопки повтора нет |
| **`backend_admin_conflict`** | **Тост** «Значение изменил другой оператор — обновите страницу» + **автоматический refetch** списка; строка возвращается в режим просмотра |
| `backend_admin_bad_request` | Тост с текстом бэка как есть (не подменять его на «расширение не поддерживается») |
| empty (`items: []`) | «Продуктов нет» / «Тарифов нет» |
| **ВСЕ продукты архивные** (`items` непуст, `tokens` есть, но у **всех** `archived: true`, переключатель выключен) | **Строка-подсказка из словаря с предложением включить переключатель** — **НЕ** empty state «Продуктов нет» и **НЕ** жёлтый блок «расширение не поддерживается» ([ADR-073](adr/ADR-073-products-archive-and-price-columns.md) §6). Спутать значит назвать оператору **ложную причину** пустого экрана: «продуктов нет» вместо «все скрыты вами же». Переключатель при этом **виден** (иначе состояние неисправимо) |
| нет `backend-economics:view` | Заглушка **«Недостаточно прав»** («нет доступа к разделу», [Заглушки](#гейтинг-навигации-и-действий-по-правам-rbac-нормативно)) |

### История запросов пользователя (страница «Юзеры бэков») — расширение той же волной

В таблице истории запросов (`RequestsTab` карточки пользователя бэка) добавляются две колонки:

| Колонка | Содержимое |
|---------|------------|
| **Списано токенов** | `tokens_spent`; **`null` → `—`** (`--text-tertiary`, с `title`) |
| **Себестоимость** | `provider_cost_usd` в USD, **до 4 знаков** после запятой; **`null` → `—`**; измеренный `0` → **`$0.00`** |

- **`null` — это «не измерено», а не ноль** ([ADR-072](adr/ADR-072-crm-admin-api-v11-economics.md) §5): рендерить `$0.00` вместо `—` **запрещено**; `value || 0`/`?? 0`/`coalesce` на уровне строки **запрещены**.
- **Формат — 4 знака**: глобальный формат с двумя знаками схлопнул бы `$0.0004` в `$0.00` — то же враньё с другой стороны.
- **`refunded: true`** — строка получает пометку возврата (текстом/бейджем, **не только цветом**); `tokens_spent` при этом **остаётся показанным** (возврат не обнуляет стоимость). **`false` → пометки нет. `null` (поле не отдано, бэк уровня v1) → пометки тоже нет** — и это не вводит в заблуждение: в такой строке `tokens_spent` и `provider_cost_usd` равны `—`, то есть о списании не утверждается ничего. Отдельного индикатора «неизвестно, был ли возврат» **не вводится** (он был бы шумом на строке, где и списание неизвестно).
- **Если хоть одна строка содержит `—`** — под таблицей рендерится **легенда** «— — себестоимость не измерена» (a11y: различие не передаётся только цветом).
- Минимальная ширина таблицы увеличивается под две новые колонки; **значимые числовые значения не усекаются** (`truncate`/`overflow-hidden` на них запрещены — [Усечение контента](#усечение-контента--общее-правило-нормативно-claudemd)); переполнение решается размером.

### Локализация страницы «Продукты и тарифы» (нормативный словарь)

| Ключ | Русский |
|------|---------|
| Пункт навигации / заголовок страницы | **Продукты и тарифы** |
| Селектор приложения (плейсхолдер) | **Выберите приложение** |
| Подсказка «бэк не выбран» (список бэков непуст) | **«Выберите приложение, чтобы увидеть продукты и тарифы»** |
| Подсказка «список бэков пуст» (`items: []`) — селектор при этом не рендерится | **«Нет приложений с Admin API Key — задайте ключ в карточке бэка на странице «Бэки»»** |
| Заголовок таблицы продуктов | **Продукты** |
| Заголовок таблицы тарифов | **Тарифы списания** |
| Колонки таблицы продуктов ([ADR-073](adr/ADR-073-products-archive-and-price-columns.md) §2 — «Цена» и «Период» **убраны**) | **Продукт** / **Токены** / **Аватар-токены** / **Выдаётся** / **Обновлено** + последняя колонка без заголовка (контрол архива) |
| Переключатель над таблицей продуктов | **Показать архивные** |
| Пометка архивного продукта (пилюля в колонке «Продукт») | **В архиве** |
| `aria-label` контрола у активного продукта | **Архивировать** |
| `aria-label` контрола у архивного продукта | **Вернуть из архива** |
| Toast успех — архивирование | **«Продукт скрыт с витрины. Начисление по нему продолжает работать. Применится в течение {N} с»** |
| Toast успех — возврат из архива | **«Продукт возвращён на витрину. Применится в течение {N} с»** |
| Обе архивные строки — хвост о задержке | Хвост **« Применится в течение {N} с»** добавляется **при наличии** `effective_after_seconds` в ответе и **опускается**, если поле не пришло (общее правило толерантности — [ADR-073](adr/ADR-073-products-archive-and-price-columns.md) §8). Прежняя редакция обеих строк была **безусловным** утверждением о состоявшемся переходе и окно применения **скрывала** — см. обоснование в §7 ADR-073 |
| Подсказка «все продукты архивные» (переключатель выключен) | **«Все продукты в архиве. Включите «Показать архивные», чтобы увидеть их»** |
| Пометка архивной опции в форме **«Установить план»** (страница «Юзеры бэков»; форма архивные **не фильтрует** — [ADR-073](adr/ADR-073-products-archive-and-price-columns.md) §5) | суффикс подписи опции — **« (в архиве)»** |
| Колонки таблицы тарифов | **Тип генерации** / **Токенов за генерацию** / **Обновлено** |
| `kind` → подпись | `chat` → **Чат**, `photo` → **Фото**, `video` → **Видео**, `other` → **Другое** |
| `grantable` | `true` → **Да**, `false` → **Нет** |
| Пустое `updated_at` | **не менялось** |
| Пустое значение ячейки (`null`) | **—** |
| Toast успех — правка **только `tokens`** продукта (`{previous}` = `previous_tokens` ответа) | **«Токены продукта: {previous} → {current}. Применится в течение {N} с»** |
| Toast успех — правка **только `avatar_tokens`** (`{previous}` = значение, отрисованное до правки; **НЕ `previous_tokens`**) | **«Аватар-токены: {previous} → {current}. Применится в течение {N} с»** |
| Toast успех — правка **обеих величин одним `PATCH`** (тело это допускает) | **«Токены продукта: {previous} → {current}; аватар-токены: {previous_avatar} → {current_avatar}. Применится в течение {N} с»** |
| Toast успех — **прежнее значение аватар-токенов клиенту недоступно** (фолбэк; дельту НЕ выдумывать) | **«Аватар-токены: {current}. Применится в течение {N} с»** |
| Toast успех (тариф) | **«Тариф: {previous} → {current}. Применится в течение {N} с»** |
| Toast «без изменений» (`changed:false`) | **«Значение не изменилось»** |
| Toast конфликта (`backend_admin_conflict`) | **«Значение изменил другой оператор — обновите страницу»** |
| Ошибка валидации ввода | **«Введите целое число ≥ 0»** (для дробных тарифов — **«Введите число ≥ 0»**); вне лимита — **«Значение вне допустимого диапазона»** |
| Блок «расширение не поддерживается» — **ОДНА строка на оба триггера и обе таблицы** (код `backend_admin_extension_not_supported` **и** «`200`, но ни один элемент не несёт `tokens`») | **«Бэк не отдаёт продукты и тарифы — требуется обновление до расширенного CRM Admin API»** |
| Подсказка при `capabilities === null` — **только держателю `backend-economics:edit`** | **«Правка недоступна: бэк не подтвердил поддержку изменения значений»** (нейтральный тон, без кнопки повтора; держателю только `view` не рендерится) |
| Блок «нет admin-ключа» (`backend_admin_key_not_set`) | **«У бэка не задан Admin API Key — задайте его в карточке бэка»** |
| Empty state (продукты) | **«Продуктов нет»** |
| Empty state (тарифы) | **«Тарифов нет»** |
| Ошибка загрузки + кнопка | **«Не удалось загрузить»** / **Повторить** |
| Колонки истории запросов | **Списано токенов** / **Себестоимость** |
| Пометка возврата (`refunded`) | **Возврат** |
| Легенда под таблицей запросов | **«— — себестоимость не измерена»** |
| `title` у прочерка себестоимости | **«Не измерено»** |
...
> **Дефолт SMTP формы (нормативно, [ADR-071](adr/ADR-071-mail-smtp-default-587-starttls.md)):** при создании ящика поля SMTP предзаполняются **портом `587` и `STARTTLS`** (не `465`/`SSL`). Причина — host этого деплоя не имеет исходящего доступа на `465` (фильтр upstream Hetzner: `:465` таймаутит, `:587` коннектится), а `587`/STARTTLS поддерживают все перечисленные провайдеры и все прод-ящики. `465`/SSL не запрещён — снят с дефолта и меняется вручную, если провайдер его требует. IMAP-дефолт (`993`/`SSL`) не меняется.

**Подсказки по провайдерам** (каждый — вложенный свёрнутый пункт):

| Провайдер (домены) | Нормативная подсказка |
|--------------------|-----------------------|
| **Gmail** (`@gmail.com`) | 1) Включить 2FA: [myaccount.google.com/security](https://myaccount.google.com/security) → «2-Step Verification». 2) Включить IMAP: [Gmail](https://mail.google.com) → «See all settings» → «Forwarding and POP/IMAP» → «Enable IMAP». 3) Создать app password: [myaccount.google.com/apppasswords](https://myaccount.google.com/apppasswords) → скопировать 16-символьный пароль *(показывается один раз)*. 4) В форме: адрес `you@gmail.com`, пароль — этот 16-символьный *(без пробелов)*. Хосты: `imap.gmail.com:993 SSL`, `smtp.gmail.com:587 STARTTLS`. **⚠️ Корпоративный Workspace Gmail: app passwords часто отключены администратором домена → подключиться не получится (OAuth не поддерживается).** |
| **Яндекс** (`@yandex.ru`, `@yandex.com`, `@ya.ru`) | 1) 2FA: [id.yandex.ru/security](https://id.yandex.ru/security). 2) Включить IMAP: [mail.yandex.ru](https://mail.yandex.ru) → «Все настройки» → «Почтовые программы» → «С сервера imap.yandex.ru по протоколу IMAP». 3) «Пароли приложений» → «Создать» → «Почта». 4) Хосты: `imap.yandex.ru:993 SSL`, `smtp.yandex.ru:587 STARTTLS`. |
| **Mail.ru и семейство** (`@mail.ru`, `@inbox.ru`, `@bk.ru`, `@list.ru`) | 1) 2FA: [account.mail.ru/user/2-step-auth](https://account.mail.ru/user/2-step-auth/). 2) «Пароли для внешних приложений»: [account.mail.ru/user/2-step-auth/passwords](https://account.mail.ru/user/2-step-auth/passwords) → «Добавить». 3) IMAP включён по умолчанию. 4) Хосты: `imap.mail.ru:993 SSL`, `smtp.mail.ru:587 STARTTLS`. |
...
| **Yahoo Mail** (`@yahoo.com`, `@yahoo.ru`, `@ymail.com`) | 1) 2FA + app password: [login.yahoo.com/account/security](https://login.yahoo.com/account/security) → «Generate and manage app passwords». 2) Хосты вручную: `imap.mail.yahoo.com:993 SSL`, `smtp.mail.yahoo.com:587 STARTTLS`. |
| **Другой провайдер / свой сервер** | Найдите настройки на сайте провайдера («IMAP settings»). Обычно: IMAP `imap.<домен>:993 SSL`, SMTP `smtp.<домен>:587 STARTTLS` (некоторые провайдеры — `465 SSL`); логин — полный email (иногда нужен отдельный SMTP-логин). **⚠️ ProtonMail без платной подписки не работает** (IMAP/SMTP — только через локальный ProtonMail Bridge, недоступный серверу). |
...
- **Хэдер** — снова `sticky top-0 z-30` (закреплён при скролле `body`; в потоке документа `sticky` эквивалентен прежнему закреплённому хэдеру). Внутренняя разметка хэдера (лого, вкладки, кнопка «Выйти», внутренний `w-full px-6`) — без изменений; контейнер хэдера — на всю ширину (решение владельца, 2026-07-23).
- **`<main>`** — **простой контейнер БЕЗ** `flex-1`/`min-h-0`/`overflow-y-auto`/`h-*`/`w-full`-как-скролл-области и **БЕЗ** `mx-auto`/`max-w-[1400px]`/`px-6`/`py-8`. `<main>` **не является скролл-контейнером** — он не порождает собственный скроллбар. Внутри `<main>` — **внутренний `<div className="w-full px-6 py-8">`** как обёртка вокруг `<Outlet/>`: именно он задаёт внешние паддинги; контент занимает всю ширину вьюпорта (прежний предел `1400px` упразднён решением владельца, 2026-07-23).
...
### Layout (Gmail-like, [ADR-071](adr/ADR-074-mail-gmail-layout-folders-batch.md))

**Трёхколоночный:** сайдбар (~220px) · список (~38%) · деталь. Горизонтальные вкладки «Сообщения / Почты / Теги» **убраны** — CRUD «Почты» и «Теги» внизу сайдбара. По умолчанию при открытии выбрано **самое свежее** письмо.

- **Сайдбар:** «Написать» (v1 — disabled, tooltip «Скоро»), папки (Входящие с badge из `unread-count`, Отправленные, Удалённые, **С тегами**), блок «Команды» (при ≥2 вариантах канала), список тегов, ссылки «Почты»/«Теги».
- **Список:** чекбоксы, компактные строки, тулбар bulk (Прочитано · Архивировать · Удалить · Обновить).
- **Адаптив.** На узких вьюпортах сайдбар — drawer; список/деталь — как master-detail с «Назад».
- **Пустое состояние:** «Писем пока нет» / «Отправленных писем нет».

### Список писем (средняя панель)

- **Порядок** `internal_date DESC, id DESC`. Активный элемент — акцентная граница.
- **`MailListItem` (компактная строка):** checkbox, `#id`, отправитель, inline tag chip (первый тег), тема + snippet `body_text` (~80 символов), дата **`YYYY-MM-DD HH:mm`** (absolute). «Получено на:» в строке **не показывается** (только в детали).
...
**Супер-админ из `.env`** (`me.is_superadmin === true`): индикатор непрочитанного и кнопка «Отметить непрочитанным» рендерятся **любому** носителю `mail:view`, включая супер-админа ([ADR-051](adr/ADR-051-superadmin-db-anchor-personal-state.md)). **Badge «Входящие»** — из `GET /api/mail/unread-count` ([ADR-071](adr/ADR-074-mail-gmail-layout-folders-batch.md)). **Контрол «Уведомления»** остаётся скрытым при `me.is_superadmin === true` ([ADR-051 §1.6](adr/ADR-051-superadmin-db-anchor-personal-state.md)).

### Навигация и фильтры ([ADR-071](adr/ADR-074-mail-gmail-layout-folders-batch.md))

Фильтры — в **сайдбаре**, не в тулбаре списка. Серверные: папка (`folder`), «С тегами» (`has_tags=true`), клик по тегу (`tag_id`), команда (`team_id`/`no_team`). Тулбар списка — только bulk-действия и «Обновить». Прежняя норма «фильтры только в тулбаре левой панели» **отменена**.

> **ОТМЕНЕНО в UI v1 Gmail-layout:** тумблер «Непрочитанные», дропдауны «Почта»/«Команда» в тулбаре списка. Команды — блок в сайдбаре (≥2 варианта канала, [ADR-055](adr/ADR-055-per-channel-teams-mail-sms.md)).
...
**Где применяется (исчерпывающе):** дропдаун **«Почта»** ленты, вкладка [«Сообщения»](#дропдауны-почта-и-команда-серверные) (`mode='select'`); **поле поиска/выбора почты**, вкладка [«Почты»](#вкладка-почты-нормативно) (`mode='search'`); фильтр **«Все номера»** на `/sms` вкладка [«Сообщения»](#вкладка-сообщения) (`mode='select'`, опции отсортированы по `phone_number`).
...
Пункт **«Пользователи»** плоской навигации (`/users`), **admin-only** (виден при `is_admin_level`: `is_superadmin || "admin" ∈ roles` **или** полный каталог по **union** прав всех ролей, [ADR-076](adr/ADR-076-knowledge-bot-broadcast-and-admin-level.md) §4 в редакции [ADR-079](adr/ADR-079-users-m2m-roles-full-name-telegram-table.md) §2). Со Спринта B ([ADR-022](adr/ADR-022-teams-nav-categories.md)) управление **ролями** вынесено на отдельную страницу [«Роли»](#страница-роли), а **командами** — на [«Команды»](#страница-команды); эта страница содержит **только пользователей** — **таблицей** ([ADR-079](adr/ADR-079-users-m2m-roles-full-name-telegram-table.md) §10). Решения — [ADR-021](adr/ADR-021-rbac-users-roles.md), [ADR-022](adr/ADR-022-teams-nav-categories.md), [ADR-029](adr/ADR-029-ui-login-password-nav-team-form.md), **[ADR-079](adr/ADR-079-users-m2m-roles-full-name-telegram-table.md)** (supersedes [ADR-065](adr/ADR-065-users-flat-list-team-chips.md)); API — [04-api.md](04-api.md#users). Токены/шрифты — те же тёмные. Layout — **не-full-bleed** (`w-full px-6 py-8`).
...
### Таблица пользователей (нормативно, [ADR-079](adr/ADR-079-users-m2m-roles-full-name-telegram-table.md) §10)

> **⚠️ Прежняя норма «плоский СПИСОК карточек, один элемент на пользователя, сортировка по `username`» ([ADR-065](adr/ADR-065-users-flat-list-team-chips.md)) — ОТМЕНЕНА [ADR-079](adr/ADR-079-users-m2m-roles-full-name-telegram-table.md).** Раскладка — **таблица**. Ещё раньше [ADR-065](adr/ADR-065-users-flat-list-team-chips.md) отменил группировку по командам с дублированием пользователя в каждой секции ([ADR-021](adr/ADR-021-rbac-users-roles.md)/[ADR-022](adr/ADR-022-teams-nav-categories.md)) — **это остаётся в силе**: один пользователь = одна строка, `groupByTeams` в раскладке не применяется. **Backend-контракт изменён** (в отличие от ADR-065): `roles`/`role_ids`, ФИО, обязательный `telegram` — [04-api.md](04-api.md#users).

**Примитивы — только существующие. Новых компонентов ДС не вводится.**

- **Сводные плашки** — существующий `SummaryCell` и его grid-обёртка (образец — `frontend/src/pages/BackendUsersPage.tsx:184-198`, `:323`, сверка 2026-08-19). Четыре ячейки, считаются **на клиенте** из `GET /api/users` (серверных счётчиков не заводится): **«Всего»** (число строк), **«Активны»** (`status === "active"`), **«Ожидают входа»** (`status === "pending"`), **«Активны в боте»** (`bot_started === true`). Неактивные (`status === "inactive"`) отдельной плашки не получают — их видно в колонке «Статус».
- **Обёртка таблицы** — тот же паттерн, что у «Юзеров бэков» и «Продуктов и тарифов»: карточка-контейнер `overflow-x-auto rounded-card border border-border-subtle bg-surface-1`, внутри `<table class="w-full text-left text-sm">`, шапка `<thead>` со строкой `border-b border-border-subtle text-[12px] uppercase tracking-wide text-text-tertiary`.
- **Сортировка (нормативно, Спринт B).** Кликабельные заголовки **ФИО**, **Роли**, **Команды**; повторный клик по активной колонке разворачивает направление, смена колонки начинает с возрастания. Сравнение — `localeCompare` с локалью `'ru'`. Роли/команды сравниваются по **первому** чипу — по тому, что видно в колонке. **Пустой набор («Без роли»/«Без команды») всегда уезжает в конец** независимо от направления: иначе при сортировке по возрастанию голову списка занимали бы строки без значения. Равные значения — вторичный ключ ФИО (иначе порядок «дрожал» бы между рендерами). Направление дублируется в `aria-sort`, заголовок — `<button>` внутри `<th>`.
- **Поиск** — **клиентский**, одно поле над таблицей; серверного параметра нет ([03-data-model.md](03-data-model.md#индексы-и-обоснование) — индексов по ФИО не заводится). Область поиска (нормативно): **ФИО-строка + `username` + `telegram`**, подстрочно, регистронезависимо. ⚠️ **`username` входит в область поиска, но НЕ в подпись плейсхолдера** («Поиск по ФИО или Телеграму»): поля «Логин» в UI больше нет, обещать его в подсказке нельзя, а искать по нему необходимо — у исторических учёток без ФИО именно `username` и стоит в колонке «ФИО».
- **Сортировка** — по ФИО-строке, `localeCompare` с локалью `ru`. Прежняя сортировка по `username` отменена.

**Колонки (нормативно, порядок фиксирован):**

| Колонка | Содержимое |
|---|---|
| **ФИО** | ФИО-строка `«{last_name} {first_name} {middle_name}»` со схлопыванием пустых; **все три пусты → `username`**. ⚠️ **Второй строки с `username` НЕТ** (норма пересмотрена, Спринт B): технический логин дублировал видимое значение у исторических учёток и шумел у остальных. Поиском `username` по-прежнему находится |
| **Роли** | **Набор** ролей (`user.roles`) чипами — тот же примитив, что у чипов команд (`ui/Pill` нейтрального/`accent` tone либо существующая чип-идиома строки). Пустой набор (возможен только при правке БД в обход API) → вторичная подпись **«Без роли»** |
| **Команды** | Чипы `user.teams`; пустой массив → единичный фолбэк **«Без команды»** (вторичным цветом) — норма [ADR-065](adr/ADR-065-users-flat-list-team-chips.md) сохранена |
| **Telegram** | `telegram` **моноширинным**, с ведущим `@` при отображении; `null` (историческая строка) → `—` |
| **Статус** | `StatusBadge` по производному `status` ([ADR-028](adr/ADR-028-user-status-first-login.md)): `"active"` → **«Активен»**, `"pending"` → **«Ожидает входа»**, `"inactive"` → **«Неактивен»**. Рядом — опц. индикатор **«Без пароля»** при `has_password=false` ([ADR-025](adr/ADR-025-passwordless-users-login-identifier-open-first-login.md)) |
| **Бот** | `Badge` запуска ИИ-бота ([ADR-076](adr/ADR-076-knowledge-bot-broadcast-and-admin-level.md)): `bot_started=true` → **«Бот»** (`tone="green"`), `false` → **«Бот не запущен»** (`tone="red"`) |
| **Действия** | Кнопка **«Открыть»** → модалка редактирования (`PATCH /api/users/{id}`). Клик по строке целиком делает то же |

- `useTeams` на странице **остаётся** — он нужен модалкам (мультивыбор команд), **не** раскладке.
- Супер-админ (`.env`) в таблице **не отображается**: его системная строка-якорь исключена из `GET /api/users` на сервере ([ADR-051](adr/ADR-051-superadmin-db-anchor-personal-state.md) §1.4).
- **Кнопка «Добавить пользователя»** → модалка создания (`POST /api/users`). Поля формы (нормативно):
  - **Фамилия** (`last_name`) — **обязательна**; **Имя** (`first_name`) — **обязательна**; **Отчество** (`middle_name`) — опционально. Placeholder-примеров с конкретными именами нет ([ADR-022](adr/ADR-022-teams-nav-categories.md), правило сохранено).
  - **Телеграм** (`telegram`) — **обязателен** ([ADR-079](adr/ADR-079-users-m2m-roles-full-name-telegram-table.md) §8), `Input` с плейсхолдером `@username`. Невалидный формат → инлайн-подсказка (`422`); дубль → `409 telegram_taken`; **`409 username_taken` показывается НА ЭТОМ ЖЕ поле** (выведенный логин совпал с историческим — поля «Логин» в форме нет).
  - **Пароль** (`password`, **опционально**, 8–128 при заполнении) — маскируется. Подсказка под полем — **«Оставьте пустым — пользователь задаст пароль при первом входе»** ([ADR-025](adr/ADR-025-passwordless-users-login-identifier-open-first-login.md)). Беспарольный сценарий **сохранён**.
  - **Роли** (**мультивыбор**, [MultiSelect](#компонент-мультивыбор-multiselect) из `GET /api/roles`) — **обязателен минимум один**. Пустой выбор блокируется формой; серверный отказ — `422` (`details[].field="role_ids"`). Прежнее поле «Роль» (`Select`, одиночный) **упразднено**.
  - **Команды** (**мультивыбор** из `GET /api/teams`) — **опционально** (0..N). Это базовое членство (`team_ids`).
  - **Блоки «СМС» и «Почты»** — внизу формы, **свёрнуты**, без изменений (см. ниже).
  - ⛔ **Поля «Логин» в форме НЕТ** ([ADR-079](adr/ADR-079-users-m2m-roles-full-name-telegram-table.md) §9). `username` выводится сервисом из телеграм-ника и наружу показывается только как фолбэк-подпись в таблице.
- **Модалка редактирования** (`PATCH /api/users/{id}`): **Фамилия / Имя / Отчество**, **Телеграм** (сменить можно, **очистить нельзя** — пустое значение даёт `422`, форма обязана блокировать сабмит и объяснять причину), **Роли** (мультивыбор, ≥1), **Команды**, **блоки «СМС»/«Почты»** (свёрнуты), **Статус** (тумблер «Активен»), **Сброс/установка пароля** (поле «Новый пароль» — пустое = не менять). Логина в форме нет. Удаление — кнопка «Удалить» (`DELETE`).

> **Экран входа НЕ меняется** ([ADR-041](adr/ADR-041-login-theme-session-ux.md)): label **«Логин или Телеграм»** и плейсхолдер **«Логин или @username»** остаются — существующие пользователи входят по историческому `username`, и убрать эту возможность нельзя (у части строк `telegram IS NULL`).
...
- Ошибки формы: `409 telegram_taken` → «Пользователь с таким Телеграмом уже существует»; **`409 username_taken` → «Этот Телеграм уже занят как логин другого пользователя»** — показывается **на поле «Телеграм»** ([ADR-079](adr/ADR-079-users-m2m-roles-full-name-telegram-table.md) §9: выведенный `username` совпал с историческим логином; поля «Логин» в форме нет, поэтому прежняя строка «Пользователь с таким логином уже существует» **отменена** — она указывала бы на несуществующий контрол); `422` невалидные ФИО/телеграм/пароль → инлайн-подсказка на соответствующем поле; **`422` по `role_ids`** (пустой набор) → подсказка у мультивыбора «Роли». Удаление пользователя-лидера **не блокируется** (лидерство авто-передаётся, [ADR-026](adr/ADR-026-teams-optional-leader-auto-transfer.md)) — специального сообщения об ошибке нет.

### Гейтинг и `403`

Пункт и страница доступны только при `me.is_admin_level` из `GET /api/auth/me` ([ADR-078](adr/ADR-078-me-is-admin-level.md); иначе пункт скрыт, прямой переход на `/users` → заглушка **«нет доступа к разделу»** без сброса сессии; `AdminRoute` показывает эту заглушку, **не редиректит**). Spinner на месте пункта / в `AdminRoute` «пока грузится каталог» **не** ставится. Нормативные строки заглушки и общее правило page-level гейтинга — [Гейтинг навигации](#гейтинг-навигации-и-действий-по-правам-rbac-нормативно). Любой `403` от API — inline/toast «Недостаточно прав», сессия сохраняется.
...
- **Редактор роли** (`POST`/`PATCH /api/roles/{id}`): поле **Название** (`name`) + **матрица прав**. Матрица строится из `GET /api/permissions/catalog`:
  - **строки** — страницы каталога **в порядке ответа** (`GET /api/permissions/catalog`): Дашборд, Серверы, ИИ - ключи, Прокси, Бэки, Пользователи бэков, Продукты и тарифы, Почты, **СМС**, **Роли**, **Команды**, **Документы**, **Рассылка** (`broadcast → «Рассылка»`, [ADR-076](adr/ADR-076-knowledge-bot-broadcast-and-admin-level.md)). Порядок — как в каталоге ([05-security.md](05-security.md#каталог-прав-канон-на-сервере));
  - **столбцы** — **объединение действий серверного каталога**, не зашитый `ACTION_ORDER`. Ячейка существует только если действие есть у страницы. Подписи — `ACTION_LABEL`: `view`→«Просмотр», `create`→«Создание», `edit`→«Изменение», `delete`→«Удаление», `share`→**«Видимость»**, `send`→**«Отправка»**, `sync`→**«Синк»**, `tags`→**«Теги»**, `transfer`→**«Перенос»**. Фолбэк на сырой ключ — только авария ([ADR-063](adr/ADR-063-documents-editor-cache-lifecycle-focus.md) §D). **[TD-068](100-known-tech-debt.md) закрыт в коде frontend ([ADR-076](adr/ADR-076-knowledge-bot-broadcast-and-admin-level.md), сверка 2026-08-17)**; машинный гейт подписей — [TD-071](100-known-tech-debt.md);
  - каждая ячейка — примитив **`Checkbox`**; отмеченные складываются в `permissions` `{page:[actions]}` и отправляются в запросе.
  - Страница «Пользователи» (`users`) в матрице **отсутствует** (вне каталога — доступ по `is_admin_level`).
...
- Тулбар: [`ui/Combobox`](#компонент-uicombobox-нормативно-adr-052) **`mode='select'`** **«Все номера»** (тот же контракт, что дропдаун «Почта» на `/mail` «Сообщения», [ADR-052](adr/ADR-052-mail-mailbox-combobox.md) §2) — опции из `useSmsNumbers`, `value=id`, лейбл `phone_number` + `label` если есть; первая опция **«Все номера»** — `pinned` (фильтром не отсекается); опции **отсортированы** по `phone_number` (`localeCompare`, `numeric`); `keywords` = `[phone_number, label?]`. **Ввод фильтрует только выпадающий список**; ленту меняет **только выбор** (`number_id`). Рендерится **всем** держателям `sms:view` (у не-admin — только его номера по scope). `Select` **«Все команды»** — **правило рендера и источник опций единые с почтой**: см. [Фильтр «Команда» — единое правило на пяти экранах](#фильтр-команда--единое-правило-на-пяти-экранах-нормативно-adr-055) (порог **«≥ 2 доступных варианта канала»**, опции — из `me.sms_teams` у **любого** актора, опция **«Без команды»** при `me.sms_includes_unassigned` → серверный **`no_team=true`**). ⚠️ Прежняя норма «**рендерится только при `me.sees_all_sms_teams === true`**» ([ADR-036](adr/ADR-036-sms-team-filter-admin-only.md)) — **ОТМЕНЕНА** [ADR-055](adr/ADR-055-per-channel-teams-mail-sms.md) §6. Когда оба фильтра видны — они **комбинируемы** (AND, выбор одного не сбрасывает другой).
...
| Заголовки колонок таблицы ([ADR-079](adr/ADR-079-users-m2m-roles-full-name-telegram-table.md) §10) | **ФИО** / **Роли** / **Команды** / **Telegram** / **Статус** / **Бот** / **Действия** |
| Подписи сводных плашек ([ADR-079](adr/ADR-079-users-m2m-roles-full-name-telegram-table.md) §10) | **Всего** / **Активны** / **Ожидают входа** / **Активны в боте** |
| Плейсхолдер поиска по таблице | **Поиск по ФИО или Телеграму** — слово «логин» в подписи **не употребляется** ([ADR-079](adr/ADR-079-users-m2m-roles-full-name-telegram-table.md) §9: поля «Логин» в UI больше нет, и называть его в плейсхолдере значило бы обещать контрол, которого нет). ⚠️ **Область поиска при этом ШИРЕ подписи:** фильтр накрывает ещё и `username` — это нужно для исторических учёток, у которых ФИО не заполнено и в колонке «ФИО» отображается именно `username` (см. [норматив колонок](#таблица-пользователей-нормативно-adr-079)). Расхождение осознанное: подпись называет то, что оператор **вводит осмысленно**, а `username` ищется как фолбэк того, что он **видит на экране**. |
| Кнопка открытия карточки в строке | **Открыть** |
| Чипы ролей в строке (`user.roles`) | названия ролей чипами (тот же примитив, что чипы команд) |
| Фолбэк при пустом наборе ролей | **Без роли** (вторичным цветом) |
| Чипы команд в строке (`user.teams`, [ADR-065](adr/ADR-065-users-flat-list-team-chips.md)) | названия команд чипами. ⚠️ **«список — плоский, сортировка по `username`» ОТМЕНЕНО** [ADR-079](adr/ADR-079-users-m2m-roles-full-name-telegram-table.md): раскладка — **таблица**, сортировка — по **ФИО** (`localeCompare`, `ru`) |
| Фолбэк при пустых командах в строке ([ADR-065](adr/ADR-065-users-flat-list-team-chips.md)) | **Без команды** (вторичным цветом) |
| ~~Поле логина~~ | ⛔ **УПРАЗДНЕНО** [ADR-079](adr/ADR-079-users-m2m-roles-full-name-telegram-table.md) §9 — строки **«Логин»** в форме пользователя больше нет (поле удалено из обеих модалок; `username` выводится сервисом из телеграм-ника). ⚠️ Не путать со строкой **«Логин или Телеграм»** на экране входа — она **в силе** |
| Поле фамилии ([ADR-079](adr/ADR-079-users-m2m-roles-full-name-telegram-table.md) §7) | **Фамилия** (обязательно; без placeholder-примера) |
| Поле имени | **Имя** (обязательно; без placeholder-примера) |
| Поле отчества | **Отчество** (опционально) |
| Поле телеграма | **Телеграм** (**обязательно**, плейсхолдер `@username`) — прежняя пометка «опционально» отменена [ADR-079](adr/ADR-079-users-m2m-roles-full-name-telegram-table.md) §8 |
...
| Поле ролей (мультивыбор, [ADR-079](adr/ADR-079-users-m2m-roles-full-name-telegram-table.md) §1) | **Роли** (обязательно, минимум одна). Прежняя строка «Поле роли — **Роль**» (одиночный `Select`) **отменена** |
| Подсказка при пустом наборе ролей (`422`) | **«Выберите хотя бы одну роль»** |
...
| Бейдж ИИ-бота (`bot_started`, [ADR-076](adr/ADR-076-knowledge-bot-broadcast-and-admin-level.md)) | `true` → **Бот** (`tone="green"`) · `false` → **Бот не запущен** (`tone="red"`) |
| Телеграм занят (`409 telegram_taken`) | **«Пользователь с таким Телеграмом уже существует»** |
| Выведенный логин занят (`409 username_taken`, [ADR-079](adr/ADR-079-users-m2m-roles-full-name-telegram-table.md) §9) | **«Этот Телеграм уже занят как логин другого пользователя»** — показывается **на поле «Телеграм»**. ⚠️ Прежняя строка «Пользователь с таким логином уже существует» **отменена**: она указывала бы на контрол, которого в форме больше нет |
| Попытка очистить телеграм (`422`, [ADR-079](adr/ADR-079-users-m2m-roles-full-name-telegram-table.md) §8) | **«Телеграм нельзя удалить — это единственный способ входа»** |
...
| Столбцы действий | Объединение действий каталога. Словарь `ACTION_LABEL`: **Просмотр** / **Создание** / **Изменение** / **Удаление** / **Видимость** (`share`) / **Отправка** (`send`) / **Синк** (`sync`) / **Теги** (`tags`) / **Перенос** (`transfer`). Фолбэк на сырой ключ — только авария. [TD-068](100-known-tech-debt.md) закрыт в коде frontend ([ADR-076](adr/ADR-076-knowledge-bot-broadcast-and-admin-level.md)) |
| Строки страниц матрицы (`PAGE_LABEL`, `features/users/labels.ts`) | **Дашборд** / **Серверы** / **ИИ - ключи** / **Прокси** / **Бэки** / **Пользователи бэков** (`backend-users`, [ADR-069](adr/ADR-069-backend-users-page-admin-contract.md)) / **Продукты и тарифы** (`backend-economics`, [ADR-072](adr/ADR-072-crm-admin-api-v11-economics.md)) / **Почты** / **СМС** (`sms → «СМС»`, [ADR-039](adr/ADR-039-ui-server-inline-edit-backends-search-empty-sms-label.md)) / **Роли** / **Команды** / **Документы** (`documents → «Документы»`, [ADR-063](adr/ADR-063-documents-editor-cache-lifecycle-focus.md) §D) / **Рассылка** (`broadcast → «Рассылка»`, [ADR-076](adr/ADR-076-knowledge-bot-broadcast-and-admin-level.md)) |
...
| Подзаголовок empty-state токена | **Обратитесь к администратору для настройки ИИ-бота базы знаний.** |
| Ошибка загрузки audience | **Не удалось загрузить аудиторию** |
| 422 на POST | **Проверьте текст и аудиторию** |
...
Маршрут **`/documents`** — Notion/Kaiten-подобный менеджер знаний ([modules/documents](modules/documents/README.md), [ADR-059](adr/ADR-059-documents-module.md)). **Второй full-bleed маршрут** после `/mail` (`isFullBleed = startsWith('/mail') || startsWith('/documents')`, см. [Навигация](#навигация-плоская-applayout)/[Full-bleed](#full-bleed-layout-нормативно)). Пункт хэдера «Документы» — плоская навигация ([ADR-033](adr/ADR-033-flat-nav-theme-toggle-numbers-table.md)), **#12** (номер — по [нормативной таблице навигации](#навигация-плоская-applayout); формулировка «пункт 10» / «последний в ряду» из [ADR-061](adr/ADR-061-documents-sidebar-two-panel-nav.md) описывала ряд на момент его принятия и устарела: после «Документы» специфицирован пункт **«Рассылка»**, [ADR-076](adr/ADR-076-knowledge-bot-broadcast-and-admin-level.md)), гейт `documents:view`.
...
- **На строке узла — [kebab-меню](#компонент-kebab-меню-dropdownmenu-нормативно-adr-061)** (3 точки): «Переименовать» (`edit`) / «Создать копию» (`create`) / «Сменить видимость» (`share`) / «Удалить» (`delete`) — пункт рендерится ⇔ есть право (UX-гейт). «Сменить видимость» — ⇔ `share ∈ me.permissions.documents` (`useCan('documents','share')`) **или** `me.is_admin_level` (минимальный фолбэк, [ADR-078](adr/ADR-078-me-is-admin-level.md)). Если `/me.permissions` содержит `share` — пункт **обязан** быть. Источник — только `/me`, не каталог.
...
- Сохранение — `PATCH /api/documents/nodes/{id}/visibility` под `documents:share`; контрол доступен ⇔ `useCan('documents','share')` **или** `me.is_admin_level` ([ADR-078](adr/ADR-078-me-is-admin-level.md); безопасность — сервер).

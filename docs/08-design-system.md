...
- **Дефолт при первом входе — СИСТЕМНАЯ тема ОС** (решение пользователя, [ADR-033](adr/ADR-033-flat-nav-theme-toggle-numbers-table.md), [Q-UI-2](99-open-questions.md)). При отсутствии сохранённого выбора тема следует `prefers-color-scheme`: `matchMedia('(prefers-color-scheme: dark)').matches` → `data-theme="dark"`, иначе `"light"`.
- **Явный выбор пользователя ПЕРЕОПРЕДЕЛЯЕТ систему.** Клик по переключателю пишет выбор в `localStorage['crm-theme']` (`"light"`/`"dark"`) и меняет `data-theme` на `<html>`. С этого момента приоритет: **сохранённый выбор > системная тема** (явный выбор «залипает» между сессиями, ОС больше не влияет).
- **Следование за сменой темы ОС — только пока нет явного выбора (нормативно).** Если пользователь ещё НЕ делал явный выбор (в `localStorage` пусто), приложение **подписывается на `matchMedia('(prefers-color-scheme: dark)')` change** и переключает `data-theme` вслед за ОС в реальном времени. После первого явного выбора подписка перестаёт влиять (сохранённое значение имеет приоритет — слушатель либо снимается, либо игнорирует событие при наличии `localStorage['crm-theme']`).
- **Персистентность выбора — `localStorage`, ключ `crm-theme`** (значения `"light"`/`"dark"`). Пусто = «выбор не сделан» → действует системный дефолт.
- **Без FOUC.** В `index.html` в `<head>` — **синхронный inline-скрипт** (до рендера приложения), который вычисляет тему по правилу: **сохранённый выбор → иначе `matchMedia('(prefers-color-scheme: dark)')`** — и сразу проставляет `document.documentElement.dataset.theme`. Псевдокод: `const t = localStorage.getItem('crm-theme') || (matchMedia('(prefers-color-scheme: dark)').matches ? 'dark' : 'light'); document.documentElement.dataset.theme = t;`. Так первая отрисовка идёт уже в правильной теме, без вспышки.
...
- Заголовок — **«Просмотр»** (или имя сущности). Вверху справа — иконка-кнопка **`Pencil`** (aria-label «Редактировать»), видна **только при `<page>:edit`** (гейт `useCan('<page>','edit')`) → закрывает detail и открывает **существующую** edit-модалку `Add<Entity>Modal mode='edit'` (форма не меняется). Без `edit` карандаша нет — модалка чисто просмотровая.
...
|----------|------------------|--------|
| Серверы | **Название** (`name`), **IP** (`ip`, моно), **Пользователь** (`ssh_user`) | **Пароль** — reveal |
...
| ИИ-ключи | **Название** (`name`), **Провайдер** (`provider` → OpenAI/Anthropic), **Ключ** (`key_masked`, моно) | **Ключ** — reveal (полное значение) |
| Бэки | **Код** (`code`, моно/чип), **Название** (`name`), **Домен** (`domain`, моно) | секрета нет — reveal не показывается |

**Reveal секрета (нормативно, [ADR-035](adr/ADR-035-detail-view-secret-reveal.md)):**
- Секретное поле показывается как **`••••••••`** (`****`). Рядом — иконка-кнопка **глаз** (`Eye`/`EyeOff`, aria-label «Показать пароль» / «Показать ключ»), видна **только при `<page>:edit`** (для прокси — дополнительно только при `has_password=true`).
- Клик по глазу → **on-demand** запрос к reveal-эндпоинту (`GET /api/servers/{id}/ssh-password` · `GET /api/proxies/{id}/password` · `GET /api/ai-keys/{id}/key`, [04-api.md](04-api.md#reveal-секретов-по-требованию-adr-035)) → показ `value` (моношрифт). Опц. кнопка **копировать** (`Copy`).
...
| Метки полей бэка | **«Код»** / **«Название»** / **«Домен»** |
...
- **Клик по карточке → detail-view ([ADR-035](adr/ADR-035-detail-view-secret-reveal.md)).** Короткий клик по `ServerCard` открывает `ServerDetailModal` (read-only: **Название**, **IP**, **Пользователь** `ssh_user`, **Пароль** `••••` + глаз-reveal под `servers:edit`). Карандаш вверху справа → `AddServerModal mode='edit'` (правит **только `name`** — `ip`/`ssh_user`/`ssh_password` не редактируются: репровижининг вне scope, [modules/servers](modules/servers/README.md#out-of-scope)). Паттерн — [Detail-view карточных страниц](#detail-view-карточных-страниц-read-only--карандаш--edit-adr-035).
...
> **Клик по карточке → `AiKeyDetailModal` ([ADR-035](adr/ADR-035-detail-view-secret-reveal.md)):** read-only **Название**/**Провайдер**/**Ключ** (`key_masked` + глаз-reveal полного ключа под `ai-keys:edit`). Карандаш → `AddAiKeyModal mode='edit'`. Паттерн — [Detail-view карточных страниц](#detail-view-карточных-страниц-read-only--карандаш--edit-adr-035).
...
> **Клик по карточке → `BackendDetailModal` ([ADR-035](adr/ADR-035-detail-view-secret-reveal.md)):** read-only **Код**/**Название**/**Домен** (секрета нет — reveal не применяется). Карандаш → `AddBackendModal mode='edit'` (правит `code`/`name`/`domain`). Паттерн — [Detail-view карточных страниц](#detail-view-карточных-страниц-read-only--карандаш--edit-adr-035).
...
- **Перестановка** — единый `SortableContext` по всему списку (клик=detail→карандаш=edit / зажатие ~200 мс=drag, @dnd-kit — [Перестановка карточек](#перестановка-карточек-drag-and-drop)); на `onDragEnd` — оптимистично + `PATCH /api/backends/order {ids}`; при ошибке — откат и инвалидация `GET /api/backends`.
...
- **Имя тега** — сам тег отображается существующим `ui/Pill.tsx`, цвет — `tag.color`; рядом свотч цвета (редактируется).

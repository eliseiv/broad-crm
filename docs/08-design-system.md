...
> **Статус реализации:** код к этой норме **НЕ приведён** — примитивы выставляют `aria-describedby` только при ошибке (`frontend/src/components/ui/Input.tsx:34`, `Select.tsx:43`, `Textarea.tsx:36`, `MultiSelect.tsx:65`; в `Combobox.tsx` атрибута нет вовсе), подсказки рендерятся несвязанными `<p>` у потребителей. Приведение существующего кода ведёт **[TD-061](100-known-tech-debt.md)**. Норма выше действует для **новых** полей и подсказок с момента фиксации.

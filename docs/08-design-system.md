...
Вкладка **«Прокси»** (`/proxies`) добавлена пятой ([ADR-019](adr/ADR-019-proxies-availability-monitor.md)); идёт по **не-full-bleed** ветке `AppLayout` (обычный поток документа, как «Серверы»/«ИИ - ключи»).
...
Иконка сервиса (favicon вкладки браузера) — **SVG в тёмной теме сервиса**, единый визуальный язык с UI (мониторинг серверов).

- **Файл:** `frontend/public/favicon.svg` (векторный SVG; папку `frontend/public/` создать при отсутствии — Vite копирует её содержимое в корень билда как есть).
...
- **Мотив:** мониторинг серверов — **сервер/стойка + пульс-линия** (heartbeat/monitoring). Тёмный фон иконки — токен `--bg-base` (`#0A0C10`); мотив/линия — акцент `--accent` (`#6366F1`) и/или `--status-green` (`#22C55E`) для «пульса». Скругление подложки в духе чипов ДС (радиус ~`8px` по пропорции viewBox). Форма читаемая на 16×16 (без мелких деталей, толщина штриха достаточная).
- **`viewBox`** квадратный (например, `0 0 32 32`), без внешних ссылок/шрифтов/растровых вставок — самодостаточный SVG (совместимо с CSP `img-src 'self'`; иконка отдаётся с того же origin).
- **Заголовок страницы (`<title>`) не меняется** — остаётся **«Мониторинг серверов»**.
- **Тема:** favicon одноцветно-тёмный (у сервиса тёмная тема по умолчанию); отдельного light-варианта не требуется.
...
> Favicon сервиса (`frontend/public/favicon.svg`) — отдельный SVG, не из `lucide-react` (см. [Брендинг и favicon](#брендинг-и-favicon-нормативно)).

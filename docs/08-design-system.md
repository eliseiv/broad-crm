...
- **Шаг 2**: показывается введённый идентификатор (с кнопкой «назад»/сменить) + поле **«Пароль»** + кнопка «Войти». Запрос `POST /api/auth/login`. **Поле «Пароль» не является обязательным на клиенте** — беспарольный пользователь оставляет его пустым и жмёт «Войти» (см. «Открытый первый вход» ниже).
- **Открытый первый вход (нормативно, [ADR-025](adr/ADR-025-passwordless-users-login-identifier-open-first-login.md)).** Если ответ `POST /api/auth/login` содержит `password_setup_required: true` (у пользователя нет пароля), вместо ошибки показывается **модалка «Задайте пароль»**: поле **«Новый пароль»** (`type=password`, toggle видимости, ≥ 8 символов) + кнопка **«Сохранить»**. Отправка → `POST /api/auth/set-password` (Bearer setup-token из ответа login). Успех → пользователь сразу залогинен (получен access-token) → редирект на первую доступную вкладку. Ошибки: `422` (короткий пароль) → подсветка поля; `409 password_already_set` → сообщение «Пароль уже задан, войдите с паролем» + возврат к шагу 2; `401` (просрочен setup-token) → «Сессия установки истекла, начните заново».
...
> **Раскладка панели дропдауна (нормативно, [ADR-023](adr/ADR-023-ui-nav-dropdown-proxy-ip-single-delete.md)).** Пункты (страницы) в панели располагаются **вертикально — по одному в строку** (стек/колонка, `flex-col`), **не** в горизонтальный ряд. Каждый пункт — полноширинная кликабельная строка панели. Панель — **«квадратная» (менее скруглённая):** радиус скругления **6px** (см. [Сетка, отступы, скругления](#сетка-отступы-скругления)), заметно меньше скруглений карточек/кнопок.
...
- Стиль навигации: категории-триггеры в шапке (подпись + `chevron-down`), активная категория (содержит текущий маршрут) / активный пункт — акцентная подсветка (`--accent`), неактивные — `--text-secondary`, hover → `--text-primary`. Видимый focus-ring (`--accent`, 2px); панель дропдауна — тёмная поверхность `--surface-1` с тонкой границей, **скругление 6px** («квадратная»), пункты — **вертикальным стеком, по одному в строку** ([ADR-023](adr/ADR-023-ui-nav-dropdown-proxy-ip-single-delete.md)).
...
- Строка/карточка пользователя: `username`, **Телеграм** (`telegram`, если задан, вторичным цветом — с ведущим `@` при отображении), название роли (`role_name`), статус (`is_active` → бейдж **«Активен»**/**«Неактивен»**); опц. индикатор **«Без пароля»** при `has_password=false` (беспарольный пользователь ещё не завершил первый вход, [ADR-025](adr/ADR-025-passwordless-users-login-identifier-open-first-login.md)). Супер-админ (`.env`) в списке **не отображается** (его нет в таблице).
...
- **Создание/редактирование** (`AddTeamModal`, add+edit; `POST`/`PATCH /api/teams`): поля **Название** (`Input`), **Лидер** (`Select` пользователя из `GET /api/users`, **опционально** — первая опция **«Без лидера»**), **Участники** ([мультивыбор](#компонент-мультивыбор-multiselect) пользователей, опционально). **Если лидер выбран — он автоматически считается участником** (UI фиксирует его в списке участников, снять нельзя). **Лидер можно не выбирать** — тогда лидером станет первый добавленный участник (авто-назначение), а пустая команда допустима. Отправляются только изменённые поля (edit).
- **Отображение «Без лидера» (нормативно, [ADR-026](adr/ADR-026-teams-optional-leader-auto-transfer.md)):** `leader_id=null` → в списке и форме показывается «Без лидера». При исключении текущего лидера из участников лидерство авто-передаётся следующему по дате добавления (UI отражает нового лидера после ответа сервера); если участников не осталось — команда становится «Без лидера».
...
| Статус активен | **Активен** / **Неактивен** |
...
| Окно установки пароля первого входа — заголовок | **Задайте пароль** |
...
| Поле лидера | **Лидер** (опционально; первая опция **«Без лидера»**) |
| Команда без лидера (`leader_id=null`) | **Без лидера** |

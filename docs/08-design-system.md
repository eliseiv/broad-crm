...
| 5 | **Прокси** | `/proxies` |
| 6 | **Бэки** | `/backends` |
| 7 | **Пользователи** | `/users` (admin-only) |

Вкладка **«Прокси»** (`/proxies`) добавлена пятой ([ADR-019](adr/ADR-019-proxies-availability-monitor.md)); идёт по **не-full-bleed** ветке `AppLayout` (обычный поток документа, как «Серверы»/«ИИ - ключи»). Вкладка **«Бэки»** (`/backends`) добавлена шестой ([ADR-020](adr/ADR-020-backends-healthcheck-monitor.md)); та же **не-full-bleed** ветка. Вкладка **«Пользователи»** (`/users`) добавлена седьмой ([ADR-021](adr/ADR-021-rbac-users-roles.md)); та же **не-full-bleed** ветка, но видна **только** супер-админу / роли `admin` (RBAC, см. [Страница «Пользователи»](#страница-пользователи)).
...
- **Вкладки:** ресурсная вкладка (`servers`/`ai-keys`/`proxies`/`backends`/`mail`) видна ⇔ `view` ∈ `permissions[page]` (или `is_superadmin`). Вкладка **«Дашборд»** — по `dashboard:view`. Вкладка **«Пользователи»** — только `is_superadmin || role=="admin"`.
- **Кнопки действий** на ресурсной странице: «Добавить» (создать) — по `create`; клик-редактирование/перестановка карточки — по `edit`; «Удалить» — по `delete`. Отсутствует право → соответствующий контрол **не рендерится** (карточки остаются в режиме просмотра; drag-and-drop недоступен без `edit`).
...
- **Дефолтный маршрут после логина (permission-aware)** — `/dashboard`, если есть `dashboard:view` (или пользователь admin/superadmin). Если `dashboard:view` отсутствует, но есть другие `view` — редирект на **первую доступную по правам вкладку**, где «первая» определяется **порядком вкладок навигации сверху вниз** (таблица [Порядок вкладок](#навигация-верхние-вкладки-applayout) выше: Дашборд → Почты → Серверы → ИИ-ключи → Прокси → Бэки → Пользователи); берётся первая вкладка из этого порядка, для которой у пользователя есть доступ. Если у пользователя **нет ни одного `view`** (и он не admin/superadmin) — вместо редиректа показывается заглушка **«нет ни одного раздела»** (нормативные строки — блок [«Заглушки «Недостаточно прав»»](#гейтинг-навигации-и-действий-по-правам-rbac-нормативно) выше).
- **Page-level view-guard прямого доступа (нормативно, единообразно для ВСЕХ permission-gated страниц):** любая страница под RBAC, открытая **по прямому URL** (закладка/ручной ввод/перезагрузка) или **в результате навигации/редиректа**, при отсутствии у пользователя требуемого для неё права **рендерит страницу-заглушку «Недостаточно прав»** — а не полноценный контент, не редирект и не сброс сессии. Требуемое право по странице: `dashboard` → `dashboard:view`; `mail` → `mail:view`; `servers`/`ai-keys`/`proxies`/`backends` → `<page>:view`; `users` → `is_superadmin || role=="admin"` (страница «Пользователи» вне матрицы прав, [ADR-021](adr/ADR-021-rbac-users-roles.md)). Показывается заглушка **«нет доступа к разделу»** (page-scoped подсказка — блок «Заглушки «Недостаточно прав»» выше; **НЕ** строка «нет ни одного раздела», т.к. доступ к другим разделам у пользователя может быть). Правило применяется ко ВСЕМ страницам под правами единообразно (не только ресурсным `servers`/`ai-keys`/`proxies`/`backends` и не только `/users`) — Дашборд и Почты гейтятся так же. **`AdminRoute` для `/users` показывает эту заглушку («нет доступа к разделу»), а НЕ редиректит на `/dashboard`.** Супер-админ и `role=="admin"` видят все страницы (заглушка к ним не применяется). Это UI-гейт (UX); граница безопасности — серверный `403 forbidden` ([ADR-021](adr/ADR-021-rbac-users-roles.md)).
...
- **Дефолтный маршрут — permission-aware:** index-роут `/` и **fallback `*`** (несуществующий путь) резолвятся **той же permission-aware логикой**, что редирект после логина (см. «Дефолтный маршрут после логина» выше): `/dashboard`, если есть `dashboard:view` (или admin/superadmin); иначе редирект на **первую доступную по правам вкладку** (порядок вкладок сверху вниз); если нет ни одного `view` — заглушка **«нет ни одного раздела»**. Это исключает ситуацию, когда пользователь без `dashboard:view`, но с другими правами, попадая на `/` или несуществующий путь, упирается в `/dashboard`→заглушку вместо своей доступной вкладки. Базовая цель редиректа — **`/dashboard`** (ранее `/mail`): «Дашборд» стал обзорной стартовой страницей ([ADR-017](adr/ADR-017-dashboard-client-aggregation-mail-server-filters.md)); прежний дефолт `/mail` ([ADR-013](adr/ADR-013-mail-newest-first-master-detail-inline-reply.md)) заменён.
...
Седьмая вкладка `AppLayout` (`/users`), **admin-only** (видна только `is_superadmin || role=="admin"`). Управление пользователями и ролями (RBAC). Решение — [ADR-021](adr/ADR-021-rbac-users-roles.md); API — [04-api.md](04-api.md#users), [04-api.md](04-api.md#roles), [04-api.md](04-api.md#permissions). Токены/шрифты — те же тёмные. Layout — **не-full-bleed** (`mx-auto max-w-[1400px] px-6 py-8`).

Страница делится на два блока (табы или две секции — на усмотрение frontend, единый визуальный язык карточек):

### Блок «Пользователи»

- **Список пользователей** (`GET /api/users`): строки/карточки с `username`, названием роли (`role_name`), статусом (`is_active` → бейдж **«Активен»**/**«Неактивен»**). Супер-админ (`.env`) в списке **не отображается** (его нет в таблице).
- **Кнопка «Добавить пользователя»** → модалка создания (`POST /api/users`): поля **Логин** (`username`, допускает кириллицу), **Пароль** (`password`, 8–128), **Роль** (`Select` из `GET /api/roles`). Пароль вводится открыто в поле (маскируется как `<input type="password">`), в списке/ответах не показывается.
- **Клик по пользователю** → модалка редактирования (`PATCH /api/users/{id}`): **Роль** (`Select`), **Статус** (`Checkbox`/тумблер «Активен»), **Сброс пароля** (поле «Новый пароль» — пустое = не менять). Логин не редактируется (read-only). Удаление — кнопка «Удалить» в модалке (`DELETE`).
- Ошибки формы: `409 username_taken` → «Пользователь с таким логином уже существует»; `422` невалидный логин/пароль → инлайн-подсказка.

### Блок «Роли»

- **Список ролей** (`GET /api/roles`): `name` + сводка прав. Кнопка «Добавить роль» → редактор (`POST /api/roles`).
- **Редактор роли** (`POST`/`PATCH /api/roles/{id}`): поле **Название** (`name`) + **матрица прав**. Матрица строится из `GET /api/permissions/catalog`:
  - **строки** — страницы каталога (в порядке ответа: Дашборд, Серверы, ИИ - ключи, Прокси, Бэки, Почты);
  ...
  - Страница «Пользователи» в матрице **отсутствует** (вне каталога — доступ по роли `admin`).
- Удаление роли — кнопка «Удалить»; при `409 role_in_use` → «Роль назначена пользователям — удаление невозможно».
- `409 role_name_taken` → «Роль с таким названием уже существует».

### Гейтинг и `403`

Вкладка и обе секции доступны только `is_superadmin || role=="admin"` (иначе вкладка скрыта, прямой переход на `/users` → заглушка **«нет доступа к разделу»** без сброса сессии; `AdminRoute` показывает эту заглушку, **не редиректит** на `/dashboard`). Нормативные строки заглушки и общее правило page-level гейтинга — [Гейтинг навигации](#гейтинг-навигации-и-действий-по-правам-rbac-нормативно). Любой `403` от API — inline/toast «Недостаточно прав», сессия сохраняется (см. [Гейтинг навигации](#гейтинг-навигации-и-действий-по-правам-rbac-нормативно)).
...
## Локализация страницы «Пользователи»

| Ключ | Русский |
|------|---------|
| Вкладка/заголовок | **Пользователи** |
| Секция пользователей | **Пользователи** |
| Секция ролей | **Роли** |
...
| Строки страниц | **Дашборд** / **Серверы** / **ИИ - ключи** / **Прокси** / **Бэки** / **Почты** |
...
> Подписи страниц/действий матрицы — **на стороне frontend** (сервер отдаёт технические ключи `page`/`action`; локализация — этот словарь). `lucide-react`: `users`/`user` (вкладка «Пользователи»), `shield`/`shield-check` (роли), `check` (Checkbox).

...
Страница `/mail` — **full-bleed**: занимает **всю ширину** вьюпорта и примыкает вплотную к нижней границе sticky-хэдера (без внешнего `max-w-[1400px]`-контейнера и без верхнего отступа `py-8`). Список писем начинается **сразу под хэдером**. Страницы «Серверы»/«ИИ - ключи» остаются в ограниченном контейнере — **не менять** (нет регрессии их вёрстки).

- **Подход (нормативно).** `AppLayout` применяет классы `<main>` **условно по активному маршруту**. Для `/mail` (например, `useLocation().pathname.startsWith('/mail')`) — `<main className="w-full">` (**без** `mx-auto max-w-[1400px]`, **без** `px-6 py-8`); внутренние отступы, если нужны, задаёт сама `MailPage`. Для остальных маршрутов — прежний контейнер `mx-auto max-w-[1400px] px-6 py-8`. Sticky-хэдер (ряд вкладок, `top-0`) сохраняется для **всех** страниц.
- **Эквивалентная реализация.** Допустим per-route layout (отдельный layout-роут для `/mail` с full-bleed-`<main>`) вместо условных классов — критерий один: **только** `/mail` full-bleed, ограниченные страницы не затронуты. Выбор — за frontend; целевое поведение — как описано.
- **Скролл.** Страница по горизонтали не скроллится: широкий контент (HTML/iframe тела, длинные строки) скроллится **внутри своих контейнеров** (см. [Layout](#layout-master-detail)); вертикальный скролл — внутри панелей master-detail, хэдер остаётся sticky.

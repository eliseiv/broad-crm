...
- **Содержимое карточки:** иконка раздела в чипе (`--surface-3`) + заголовок раздела (Inter 20/700) + ряд счётчиков (**`Badge`-тона**): «активная» группа — `--status-green`, «неактивная» — `--status-red` (или нейтральный тон для семантически-нейтральных). Счётчики моношрифтом (JetBrains Mono). Не полагаться только на цвет — счётчик всегда подписан текстом (a11y).

### Блоки (Этап 1)

| Карточка | Переход | Источник | Счётчики (клиентский подсчёт) |
|----------|---------|----------|-------------------------------|
| **Почты** | `/mail` | `GET /api/mail/mailboxes` | **Активные** = ящики с `is_active=true` (green) / **Неактивные** = `is_active=false` (red/neutral) |
| **Серверы** | `/servers` | `GET /api/servers` | **В сети** = `items` с `online=true` (green) / **Не в сети** = `online=false` (red) |
| **ИИ - ключи** | `/ai-keys` | `GET /api/ai-keys` | **Работает** = `check_status='working'` (green) / **Не работает** = `check_status='error'` (red); опц. **Проверяется** = `check_status='pending'` (neutral) |
...
| **empty** | Источник вернул пустой список → счётчики `0 / 0`, карточка остаётся кликабельной. |
...
| Карточка «Почты» — счётчики | **Активные** / **Неактивные** |
| Карточка «Серверы» — счётчики | **В сети** / **Не в сети** |
| Карточка «ИИ - ключи» — счётчики | **Работает** / **Не работает** (опц. **Проверяется**) |

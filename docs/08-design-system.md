...
| Колонка | Содержимое |
|---|---|
| **ФИО** | ФИО-строка `«{last_name} {first_name} {middle_name}»` со схлопыванием пустых; **все три пусты → `username`**. Второй строкой вторичным цветом — `username`, **только если он отличается** от ФИО-строки (у новых пользователей `username` = telegram-ник и почти всегда отличается; дублировать одно и то же значение дважды нельзя) |

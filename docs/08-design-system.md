...
|---------|------------|------|
| **Продукт** | `product_id` (моно) + `name` вторичным | — |
| **Цена** | `price` или **`—`** (`null`) | — (не редактируется) |
| **Период** | `period` или **`—`** (`null`) | — (не редактируется) |
...
| Колонки таблицы продуктов | **Продукт** / **Цена** / **Период** / **Токены** / **Аватар-токены** / **Выдаётся** / **Обновлено** |

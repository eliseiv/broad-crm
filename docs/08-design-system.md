...
- Стиль навигации: категории-триггеры в шапке (подпись + `chevron-down`), активная категория (содержит текущий маршрут) / активный пункт — акцентная подсветка (`--accent`), неактивные — `--text-secondary`, hover → `--text-primary`. Видимый focus-ring (`--accent`, 2px); панель дропдауна — тёмная поверхность `--surface-1` с тонкой границей.
...
- Строка/карточка пользователя: `username`, **email** (если задан, вторичным цветом), название роли (`role_name`), статус (`is_active` → бейдж **«Активен»**/**«Неактивен»**). Супер-админ (`.env`) в списке **не отображается** (его нет в таблице).
- **Кнопка «Добавить пользователя»** → модалка создания (`POST /api/users`). Поля формы (нормативно):
  ...
  - **Почта** (`email`, **опционально**) — `Input type="email"`; пусто → без email. Невалидный формат → инлайн-подсказка (`422`), дубль → `409 email_taken`.
  - **Пароль** (`password`, 8–128) — маскируется (`<input type="password">`), в списке/ответах не показывается.
...
- **Клик по пользователю** → модалка редактирования (`PATCH /api/users/{id}`): **Почта** (`email`), **Роль** (`Select`), **Команды** (мультивыбор), **Статус** (`Checkbox`/тумблер «Активен»), **Сброс пароля** (поле «Новый пароль» — пустое = не менять). Логин не редактируется (read-only). Удаление — кнопка «Удалить» в модалке (`DELETE`).
- Ошибки формы: `409 username_taken` → «Пользователь с таким логином уже существует»; `409 email_taken` → «Пользователь с такой почтой уже существует»; `409 user_is_team_leader` (удаление пользователя-лидера) → «Пользователь — лидер команды: сначала смените лидера»; `422` невалидный логин/почта/пароль → инлайн-подсказка.
...
- **Список команд** (`GET /api/teams`): по команде — **Название**, **Лидер** (`leader_username`), «**N участников**» (`member_count`, формы мн.ч.). Кнопка «Добавить команду» — по `useCan('teams','create')`.
- **Создание/редактирование** (`AddTeamModal`, add+edit; `POST`/`PATCH /api/teams`): поля **Название** (`Input`), **Лидер** (`Select` пользователя из `GET /api/users`), **Участники** ([мультивыбор](#компонент-мультивыбор-multiselect) пользователей). **Лидер автоматически считается участником** — UI фиксирует его в списке участников (нельзя снять). Отправляются только изменённые поля (edit).
...
| Поле почты | **Почта** (опционально) |
| Поле пароля | **Пароль** |
| Поле нового пароля (сброс) | **Новый пароль** (пусто — не менять) |
| Поле роли | **Роль** |
...
| Почта занята (`409 email_taken`) | **«Пользователь с такой почтой уже существует»** |
| Лидер команды при удалении (`409 user_is_team_leader`) | **«Пользователь — лидер команды: сначала смените лидера»** |
...
| Счётчик участников (`member_count`) | **N участник / участника / участников** (формы мн.ч. по правилу) |

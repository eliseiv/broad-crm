...
Форма ответа — **инлайн под телом письма** в правой панели (без модалки; прежняя `ReplyModal` **удалена/заменена** — [ADR-013](adr/ADR-013-mail-newest-first-master-detail-inline-reply.md)).

- **Строка ответа (chat-like):** многострочный примитив **`Textarea`** (`body`, обязательное, непустое; `rows` ~3, растёт) + кнопка **«Ответить»** (primary, loading-спиннер) **рядом** — «написал и сразу отправил».
- **Расширенные поля** (`to`/`cc`/`subject`) — предзаполнены из письма (`to` = отправитель, `subject` = `Re: <subject>`) и **свёрнуты** за раскрытием **«Расширенно»** (disclosure). По умолчанию скрыты — при отправке без изменений внешний сервис подставит отправителя/`Re:` сам. Раскрытые: **«Кому»** (`to`), **«Копия»** (`cc`, опц.), **«Тема»** (`subject`).
- Отправка → `POST /api/mail/messages/{id}/reply`. Успех → toast **«Ответ отправлен»** + **очистка поля** (`body` очищается, форма остаётся на месте для следующего ответа). Ошибки: `404` → **«Письмо не найдено»**; `422`/`400` (пустое сообщение) → подсветка поля/сообщение; `502` → toast **«Почтовый сервис временно недоступен»**; общая → toast.
...
| Раскрытие расширенных полей reply | **Расширенно** |
| Reply — поля (расширенно) | **Кому** (только поле `to` формы ответа), **Копия**, **Тема**, **Сообщение** |

...
| 9 | **Пользователи** | `/users` | `is_admin_level` (admin-only, вне матрицы; [ADR-076](adr/ADR-076-knowledge-bot-broadcast-and-admin-level.md)) |
...
- **Гейтинг видимости пунктов сохраняется (только UX).** Пункт виден ⇔ у пользователя есть доступ к нему (`<page>:view ∈ permissions`, или `is_superadmin`; `users` — `is_admin_level`: `is_superadmin || role=="admin"` **или** полное покрытие серверного каталога). Без права пункт **не рендерится**. Пока для `users` грузится каталог (`catalogPending`, см. ниже) — вместо пункта Spinner, не скрытие и не ссылка. Граница безопасности — серверный `403 forbidden`.
...
- **Пункты меню (нормативно, [ADR-033](adr/ADR-033-flat-nav-theme-toggle-numbers-table.md) — плоская навигация):** пункт виден ⇔ у пользователя есть доступ к нему: ресурсный/`roles`/`teams`/`broadcast` — `view ∈ permissions[page]` (или `is_superadmin`); `users` — `is_admin_level` (`is_superadmin || role=="admin"` **или** полное покрытие серверного каталога из `/api/permissions/catalog` ∩ `permissions` из `/api/auth/me`). Пункты — плоский ряд без категорий-дропдаунов ([Навигация](#навигация-плоская-applayout)); скрытый по правам пункт не рендерится. «Дашборд» **в меню отсутствует** (маршрут — по прямому URL под `dashboard:view`).
...
- **Page-level view-guard прямого доступа (нормативно, единообразно для ВСЕХ permission-gated страниц):** любая страница под RBAC, открытая **по прямому URL** (закладка/ручной ввод/перезагрузка) или **в результате навигации/редиректа**, при отсутствии у пользователя требуемого для неё права **рендерит страницу-заглушку «Недостаточно прав»** — а не полноценный контент, не редирект и не сброс сессии. Требуемое право по странице: `dashboard` → `dashboard:view`; `mail` → `mail:view`; `servers`/`ai-keys`/`proxies`/`backends` → `<page>:view`; **`backend-users` → `backend-users:view`** ([ADR-069](adr/ADR-069-backend-users-page-admin-contract.md)); **`backend-economics` → `backend-economics:view`** ([ADR-072](adr/ADR-072-crm-admin-api-v11-economics.md)); **`roles` → `roles:view`; `teams` → `teams:view`** ([ADR-022](adr/ADR-022-teams-nav-categories.md)); `users` → `is_admin_level` (страница «Пользователи» вне матрицы прав, [ADR-021](adr/ADR-021-rbac-users-roles.md), [ADR-076](adr/ADR-076-knowledge-bot-broadcast-and-admin-level.md)); **`broadcast` → `broadcast:view`**. Показывается заглушка **«нет доступа к разделу»** (page-scoped подсказка — блок «Заглушки «Недостаточно прав»» выше; **НЕ** строка «нет ни одного раздела», т.к. доступ к другим разделам у пользователя может быть). Правило применяется ко ВСЕМ страницам под правами единообразно (Дашборд, Почты, Роли, Команды гейтятся так же). **`AdminRoute` для `/users` показывает эту заглушку («нет доступа к разделу»), а НЕ редиректит.** Пока `catalogPending` (каталог ещё не получен, а `is_admin_level` без него не вычислить) — `AdminRoute` рендерит Spinner «Загрузка…», **не** заглушку (`frontend/src/routes/AdminRoute.tsx`). Каталог запрашивается только если `!is_superadmin && role !== "admin" && roles:view ∈ permissions` (`needsPermissionsCatalog`). Ошибка загрузки каталога → не admin → заглушка. Носитель `is_admin_level` видит `/users` (заглушка к нему не применяется). Это UI-гейт (UX); граница безопасности — серверный `403 forbidden` ([ADR-021](adr/ADR-021-rbac-users-roles.md), [ADR-076](adr/ADR-076-knowledge-bot-broadcast-and-admin-level.md)).
...
Пункт и страница доступны только при `is_admin_level` (иначе пункт скрыт, прямой переход на `/users` → заглушка **«нет доступа к разделу»** без сброса сессии; `AdminRoute` показывает эту заглушку, **не редиректит**). Пока `catalogPending` — в ряду навигации на месте пункта «Пользователи» Spinner, на странице — Spinner «Загрузка…» (не заглушка). Нормативные строки заглушки и общее правило page-level гейтинга — [Гейтинг навигации](#гейтинг-навигации-и-действий-по-правам-rbac-нормативно). Любой `403` от API — inline/toast «Недостаточно прав», сессия сохраняется.

...
**Тулбар вкладки «Почты» (нормативно, [ADR-050](adr/ADR-050-mail-search-team-filter-personal-read-state.md) §1).** В **одной строке** с сегментом активности — **рядом с кнопками «Все / Активные / Неактивные»** — ещё два контрола. При нехватке ширины контролы **переносятся** (`flex-wrap`), значимый текст **не обрезается** (правило CLAUDE.md).

**1. Поле поиска — КЛИЕНТСКОЕ.**

- Контрол — существующий **`ui/Input`** с `trailing` иконкой **`Search`** (lucide), по образцу [поиска по бэкам](#поиск-по-бэкам-adr-039). **Новый примитив не вводится.**
- Нормативные строки: плейсхолдер **«Поиск по почтам…»** (символ `…` — **один** символ, не три точки), `aria-label` **«Поиск по почтам»**.
- **Поля поиска — ровно три:** **`number`** («Номер»), **`app_name`** («Приложение»), **`email`** (адрес почты). Совпадение — **подстрока, регистронезависимо**, по `trim()`-нутому запросу; пустой запрос → фильтр не применяется. `display_name` в поиск **не входит** (производная склейка `number`+`app_name` — искать по ней значит искать по тем же данным дважды).
...
- **Пустой результат:** активный непустой запрос без совпадений → **«Ничего не найдено»** (не путать с «Почт пока нет» — пустой каталог).

**2. Фильтр по команде — КЛИЕНТСКИЙ, гейт `sees_all_mail_teams`.**

- Контрол — **`ui/Select`**, `aria-label` **«Команда»**. Первая опция — **«Все команды»** (сброс); далее команды из **`GET /api/teams`** (`value = team.id` (UUID), лейбл — `name`); последняя опция — **«Без команды»** (ящики с `team_id = null` — они существуют и иначе были бы нефильтруемы).
- **⚠️ Рендерится ТОЛЬКО при `me.sees_all_mail_teams === true`** — **та же норма, что у фильтра «Все команды» на вкладке «Сообщения»** ([ADR-036](adr/ADR-036-sms-team-filter-admin-only.md)). Для прочих ролей **не рендерится вовсе** (не пустой, не disabled — отсутствует). Причина: опции берутся из `GET /api/teams` под гейтом **`teams:view`**, которого у mail-оператора может не быть → дропдаун рендерился бы **пустым** (тот самый мусорный контрол, который [ADR-036](adr/ADR-036-sms-team-filter-admin-only.md) и упразднил); плюс не-admin и так видит только ящики своих команд (`MailScope`). Остаточное ограничение (не-admin в нескольких командах) — [TD-058](100-known-tech-debt.md).
...
**Порядок применения фильтров (нормативно):** серверный `is_active` → **клиентский поиск** → **клиентский фильтр по команде** → рендер строк. Все три комбинируются (AND); ни один не сбрасывает другие.
...
**Супер-админ из `.env`** (`me.is_superadmin === true`): индикатор непрочитанного, фильтр «Непрочитанные» и кнопка «Отметить непрочитанным» **не рендерятся**, `POST`/`DELETE …/read` **не вызываются** (у него нет строки в `users` ⇒ личного состояния нет; backend вернул бы `403` — [04-api.md](04-api.md#личная-прочитанность-писем-нормативно-adr-050)). Это UX; граница — backend.

### Фильтры ленты («С тегами» · «Непрочитанные» · «Почта» · «Команда»)

Над списком писем (левая панель) — компактный **тулбар** с четырьмя контролами: тумблер **«С тегами»** (**клиентский**) + тумблер **«Непрочитанные»** (**СЕРВЕРНЫЙ**, [ADR-050](adr/ADR-050-mail-search-team-filter-personal-read-state.md)) + два дропдауна (`Select`) **«Почта»** и **«Команда»** (серверные). Фильтрация выполняется **в БД CRM** ([ADR-044](adr/ADR-044-mail-full-merge-into-crm.md); прежняя проксирующая модель ADR-017/ADR-038 отменена).
...
- **⚠️ Он СЕРВЕРНЫЙ** — в отличие от соседнего клиентского «С тегами»: включение → **`unread=true`** в `GET /api/mail/messages` → **сброс пагинации** и новый первый запрос **без `before`** (та же механика, что у [«Почта»/«Команда»](#дропдауны-почта-и-команда-серверные-select)). **Клиентская фильтрация непрочитанных ЗАПРЕЩЕНА** — лента курсорная, фильтр над загруженным набором ломает догрузку (это и есть известный дефект «С тегами», тиражировать его нельзя).
...
- **Не рендерится** у супер-админа из `.env` (`me.is_superadmin === true`) — у него нет личного состояния прочитанности.
...
| Фильтр непрочитанных (кнопка-тумблер, лента) — [ADR-050](adr/ADR-050-mail-search-team-filter-personal-read-state.md) | **Непрочитанные** (**СЕРВЕРНЫЙ** — `unread=true`, сбрасывает пагинацию; не рендерится у супер-админа из `.env`) |
...
| Поиск на вкладке «Почты» — [ADR-050](adr/ADR-050-mail-search-team-filter-personal-read-state.md) | плейсхолдер **«Поиск по почтам…»**, `aria-label` **«Поиск по почтам»** (**клиентский**, по `number`/`app_name`/`email`) |
| Поиск на вкладке «Почты» без совпадений | **«Ничего не найдено»** |
| Фильтр по команде на вкладке «Почты» — [ADR-050](adr/ADR-050-mail-search-team-filter-personal-read-state.md) | **Команда** (**клиентский**; первая опция **«Все команды»**, последняя — **«Без команды»**; опции из `GET /api/teams`; рендерится **только** при `sees_all_mail_teams`) |
| Дропдаун серверного фильтра по ящику (лента) | **Почта** (первая опция **«Все почты»**) |
| Дропдаун серверного фильтра по команде (лента) | **Команда** (первая опция **«Все команды»**; опции из `GET /api/teams`, значение — `team_id` UUID; рендерится только при `sees_all_mail_teams`) |
...
| Фильтр «Непрочитанные» / кнопка «Отметить непрочитанным» | **отсутствуют** (в Mini App нет тулбара фильтров; возврат в «непрочитано» — функция десктопной `/mail`) |

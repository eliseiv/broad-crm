...
- **Плоский порядок листьев навигации** (для резолва дефолт-маршрута, **без `dashboard`**): `mail, sms, servers, ai-keys, proxies, backends, users, roles, teams`. **`sms` — сразу после `mail`** (обе вкладки — категория «Агрегатор», `sms` идёт следом за `mail` в порядке пунктов категории, [ADR-030](adr/ADR-030-sms-module-full-merge.md)): пользователь с единственным правом `sms:view` при заходе на `/` резолвится на `/sms` (после проверки/отказа `mail`), а не на заглушку «нет ни одного раздела». Порядок резолва в `DefaultRoute.tsx` обязан включать `sms` в этой позиции.
...
- **Detail-панель команды (аккордеон, нормативно, [ADR-030](adr/ADR-030-sms-module-full-merge.md)).** Клик по карточке команды **раскрывает/сворачивает** `TeamDetailPanel` (аккордеон, `aria-expanded`, стейт `expandedId`) — **вместо** прежнего открытия edit-модалки. Панель (только просмотр): **Название**, **Лидер** (`leader_username`/«Без лидера»), **Участники** (`team.members` — уже в `TeamListItem`), **Список номеров команды** (ленивый `GET /api/teams/{id}/numbers`, свой `useQuery` + состояния loading/empty «Номеров нет»/error внутри панели; строка номера — **только `phone_number`** (схема `TeamNumberItem`), **без** пилюль `Логин/Приложение/Примечание` и `label` — авторизационное сужение [ADR-030](adr/ADR-030-sms-module-full-merge.md) §8: чувствительный учётный контекст номеров доступен только на [странице «СМС»](#страница-смс) под матрицей `sms:*`). Вверху справа — иконка `Pencil` (`stopPropagation`) → **существующий `AddTeamModal mode='edit'`** (модалка не меняется). Гейт `canEdit` сохраняется. Layout существующих карточек не регрессирует (CLAUDE.md: без сужения/переносов существующего контента).
...
- Тулбар: два `Select` — **«Все номера»** (опции из `useSmsNumbers`, `value=id`, лейбл `phone_number` + `label` если есть) и **«Все команды»** (опции из `useTeams`, `value=id`, лейбл `name`). Фильтры **комбинируемы** (AND) — выбор одного не сбрасывает другой (в отличие от взаимоисключения «Почта↔Команда» на странице «Почты»).
...
- Таблица `SmsNumberRow` (пагинации нет — номеров немного): колонки **Номер** (`phone_number` + системный `label` вторичным), **Логин/Приложение/Примечание** (инлайн-редактируемые: `Pencil` → `Input`/`Textarea` + `Check`/`X`, по `useCan('sms','edit')`), **Команда** (`Select` команд), **Действия** (**«Перенести»** по `useCan('sms','transfer')` + **«Удалить»** с confirm по `useCan('sms','delete')`).

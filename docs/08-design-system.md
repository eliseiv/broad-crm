...
- **Mini App `/tg/sms`** — вне `AppLayout`, наследует **тему Telegram** через `themeParams` ([Операторская Telegram Mini App](#нативный-вид-telegram-themeparams)), а не токены админ-SPA. Переключатель там **не показывается**; экран красит собственный фон (`--tg-*`) независимо от `data-theme` на `<html>`. Правило Mini App не меняется.

...
Вкладка и обе секции доступны только `is_superadmin || role=="admin"` (иначе вкладка скрыта, прямой переход на `/users` → заглушка «Недостаточно прав» без сброса сессии). Любой `403` от API — inline/toast «Недостаточно прав», сессия сохраняется (см. [Гейтинг навигации](#гейтинг-навигации-и-действий-по-правам-rbac-нормативно)).

...
**2. Фильтр по команде — КЛИЕНТСКИЙ `ui/Select`, гейт `sees_all_mail_teams`.**
...
|---------|----------------------|
| `<input>` | `role="combobox"`, `aria-expanded={open}`, `aria-controls={listboxId}`, `aria-haspopup="listbox"`, `aria-autocomplete="list"`, `aria-activedescendant={activeOptionId \| undefined}`, `autoComplete="off"`, `aria-label` (или связанный `<label htmlFor>`) |
...
| Фильтр по команде на вкладке «Почты» — [ADR-050](adr/ADR-050-mail-search-team-filter-personal-read-state.md) | **Команда** (**`ui/Select`**, **клиентский**; первая опция **«Все команды»**, последняя — **«Без команды»**; опции из `GET /api/teams`; рендерится **только** при `sees_all_mail_teams`) |

...
Пункт **«Почты»** плоской навигации (`/mail`, [ADR-033](adr/ADR-033-flat-nav-theme-toggle-numbers-table.md); первый лист permission-aware дефолт-порядка). Модуль — [modules/mail](modules/mail/README.md), решения — [ADR-012](adr/ADR-012-mail-read-through-proxy.md) (read-through-прокси) + [ADR-013](adr/ADR-013-mail-newest-first-master-detail-inline-reply.md) (newest-first, master-detail, inline-reply), контракт — [04-api.md#mail](04-api.md#get-apimailmessages). Фронт получает ленту из `GET /api/mail/messages` (`order=desc`) и отправляет ответ через `POST /api/mail/messages/{id}/reply`. Токены/шрифты — те же тёмные, что на остальных страницах.

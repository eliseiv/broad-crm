...
| Модалка — поля | **Название**, **IP**, **Пользователь**, **Пароль** |
| Модалка — кнопки | **Отмена** / **Добавить** |
...
`lucide-react`: `server` (шапка), `cpu` (CPU), `memory-stick` (RAM), `hard-drive` (SSD), `clock` (Обновлено), `plus` (добавить), `activity`/`line-chart` (низ под-карточки), `loader` (провижининг).

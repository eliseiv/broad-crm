...
- **Ширина (нормативно).** `AppLayout` применяет классы `<main>` **условно по активному маршруту**. Для `/mail` (например, `useLocation().pathname.startsWith('/mail')`) — `<main>` **без** `mx-auto max-w-[1400px]` и **без** `px-6 py-8`; внутренние отступы, если нужны, задаёт сама `MailPage`. Для остальных маршрутов — прежний контейнер `mx-auto max-w-[1400px] px-6 py-8`.
- **Высота — flex-fill, НЕ магический `calc` (нормативно).** Высота двухпанельного блока «Почты» **вычисляется раскладкой**, а не константой. **Запрещено** задавать высоту панели магическим `h-[calc(100vh-Nрем)]` (константа `N` подобрана под конкретный хэдер/паддинги и ломается при их изменении — именно так возникло пустое место ~87px снизу после снятия `py-8` у full-bleed). Целевая схема:
  ...
  - **`<main>`** — `flex-1 min-h-0` (заполняет остаток высоты под хэдером; `min-h-0` обязателен, чтобы flex-ребёнок мог сжиматься и отдавать внутренний скролл, а не растягивать shell). Далее по маршруту:
    - **`/mail`** — `<main>` — `w-full` (без `mx-auto max-w-[1400px]`, `px-6`, `py-8`; существующий assert `AppLayout.test.tsx` на `w-full` у `<main>` остаётся валиден) + `overflow-hidden` (внешнего скролла нет); `MailPage` растит двухпанельный блок на всю высоту через `h-full` (вместо прежнего `h-[calc(...)]`). Вертикальный скролл — **внутри** панелей master-detail (`overflow-y-auto`), не на странице.
    - **остальные маршруты** — `<main>` дополнительно `overflow-y-auto` + прежние классы контейнера **на самом `<main>`**: `mx-auto max-w-[1400px] px-6 py-8` (как сегодня — чтобы существующие assert'ы `AppLayout.test.tsx` на `max-w-[1400px]`/`py-8` у `<main>` оставались валидны). Длинный контент «Серверы»/«ИИ-ключи» скроллится **внутри `<main>`**, хэдер (вне скролл-области) остаётся закреплён.
...
- **Скролл (нормативно).** Ни по горизонтали, ни по вертикали страница целиком не скроллится (нет двойного скролла). Широкий контент (HTML/iframe тела, длинные строки) скроллится **внутри своих контейнеров** (см. [Layout](#layout-master-detail)); вертикальный скролл `/mail` — внутри панелей master-detail; вертикальный скролл «Серверы»/«ИИ-ключи» — внутри `<main>`. Хэдер закреплён сверху на всех страницах.

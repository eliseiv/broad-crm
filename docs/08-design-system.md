...
- `email` (+ `display_name` вторичным цветом), команда (`ui/Select` в строке — привязка к CRM-команде, как в `SmsNumberRow`), время последнего синка (`last_synced_at`, относительное), ошибка синка (`last_sync_error`, если есть — вторичным/red).
...
**Создание/редактирование** — модалка (`ui/Modal`) с полями IMAP/SMTP (host/port/ssl/starttls, email, пароль, опц. SMTP-логин/пароль, `display_name`, команда) и кнопкой **«Проверить соединение»** (`POST /api/mail/mailboxes/test`). Пароль — поле ввода, наружу не показывается.
...
| Поле почтовой группы (`AddTeamModal`) | **Почтовая группа** (первая опция **«Без привязки»**) |
...
- **Создание/редактирование** (`AddTeamModal`, add+edit; `POST`/`PATCH /api/teams`): поля **Название** (`Input`), **Участники** ([мультивыбор](#компонент-мультивыбор-multiselect) пользователей, опционально), **Лидер**, **Почтовая группа** (`Select`, опц. — опции из `GET /api/mail/teams`, лейбл `MailTeam.name`, `value=MailTeam.id`, первая опция «Без привязки»; маппится в `mail_group_id`, [ADR-038](adr/ADR-038-mail-headless-integration.md)). Отправляются только изменённые поля (edit).

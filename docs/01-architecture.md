...
- **mail** — **система-запись почты в CRM** ([ADR-044](adr/ADR-044-mail-full-merge-into-crm.md); модель «read-through-прокси без хранения» отменена): письма/теги/каталог ящиков/Telegram-уведомления — **в БД CRM** (`mail_*`, миграции `0021`–`0024`). Агрегатор `postapp.store` — тонкий IMAP/SMTP-connector: ему делегируются только жизненный цикл ящика (креды **транзитом**, `X-API-Key`) и SMTP-отправка reply; он же **push'ит** новые письма и статус синка в CRM (`POST /api/mail/ingest` / `/mailbox-status`, HMAC). Плюс фоновый `MailDispatcherService` (Telegram-доставка, 5 ботов) и Mini App `/tg/mail` ([modules/mail](modules/mail/README.md)). Требует исходящий HTTPS-egress backend → `postapp.store` и `api.telegram.org`, **входящий** — от агрегатора и Telegram.
- **broadcast** — рассылка текста в Telegram **ИИ-бота базы знаний** (соседний репозиторий `ba-knowledge-base`, тот же `BOT_TOKEN` = CRM `KNOWLEDGE_BOT_TOKEN`). CRM сама вызывает Bot API; HTTP процесса бота не открывается. Линки запуска — таблица `knowledge_bot_links`. Решение — [ADR-076](adr/ADR-076-knowledge-bot-broadcast-and-admin-level.md), [modules/broadcast](modules/broadcast/README.md). Статус модуля `implemented` (сверка состава кода 2026-08-17; прогон qa architect'ом не выполнялся).
...
### Notifier (Telegram)
Фоновая asyncio-задача в процессе backend ([ADR-009](adr/ADR-009-in-backend-notifier-vs-alertmanager.md)). Периодически опрашивает online-серверы через `monitoring`/`MonitoringService`, ведёт **персистентную** (в БД, таблица `notifier_server_state`) state-машину и шлёт в Telegram-группу сообщения **только при эскалации** зоны нагрузки или потере доступности (`up` 1→0). Состояние переживает рестарт/деплой, а сервер, впервые увиденный уже под нагрузкой, получает ровно один catch-up-алерт ([ADR-014](adr/ADR-014-persist-notifier-state-alert-on-first-elevated.md), закрыт [TD-019](100-known-tech-debt.md)). Опционален (env `TELEGRAM_BOT_TOKEN`+`TELEGRAM_CHAT_ID`). Пороги — те же `usage_to_zone()`, что у UI/`monitoring`. Спецификация — [modules/notifier](modules/notifier/README.md).

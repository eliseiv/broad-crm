...
> **Модуль «Почты» — вкладка `/mail`, read-through без новых зависимостей** ([ADR-012](adr/ADR-012-mail-read-through-proxy.md), [modules/mail](modules/mail/README.md)). Добавляются: маршрут `/mail` (третья вкладка **«Почты»** в `AppLayout`), фича `features/mail` (`api.ts`/`hooks.ts` на TanStack Query) и страница `MailPage`. Новый UI-примитив **`Textarea`** (нативный стилизованный `<textarea>` в `components/ui`, для поля ответа) — **без новой зависимости**. **HTML-тело письма изолируется sandbox-iframe** (`<iframe srcDoc sandbox>` без `allow-scripts`/`allow-same-origin`) — **без DOMPurify и без иных новых зависимостей** ([08-design-system.md](08-design-system.md#компонент-textarea), [08-design-system.md](08-design-system.md#страница-почты)). Backend модуля почты **не добавляет** зависимостей и БД: переиспользует `httpx` (клиент к `postapp.store`).

...
| python-multipart | 0.0.12 (`>=0.0.12,<0.1`) | Парсинг `multipart/form-data` для загрузки `.md` (`POST /api/documents/upload`, `UploadFile`) — модуль «Документы» ([ADR-059](adr/ADR-059-documents-module.md), [modules/documents](modules/documents/README.md)). Обязательна: FastAPI без неё бросает `RuntimeError` на роуте с `UploadFile`. Первый multipart-приёмник в проекте |
...
| Radix UI (primitives) | `@radix-ui/react-dialog` 1.1.x, `@radix-ui/react-tooltip` 1.1.x, `@radix-ui/react-dropdown-menu` 2.1.x | Headless-примитивы (Dialog, Tooltip, DropdownMenu). `@radix-ui/react-dropdown-menu` — **уже в `package.json`** (`frontend/package.json`, `2.1.2`; исходно под retired `NavMenu`, переиспользуется kebab-меню документов — [ADR-061](adr/ADR-061-documents-sidebar-two-panel-nav.md)) |
...
| TipTap | `@tiptap/react` 2.27.2, `@tiptap/starter-kit` 2.27.2, `@tiptap/extension-link` 2.27.2, `@tiptap/extension-image` 2.27.2, **`@tiptap/pm` 2.27.2**, `tiptap-markdown` 0.8.10 (`frontend/package.json`) | **WYSIWYG-редактор документов** ([ADR-062](adr/ADR-062-documents-wysiwyg-tiptap.md), [modules/documents](modules/documents/README.md)). Первая rich-text библиотека — осознанный отход от NFR-1; хранение остаётся **markdown** в `content_md` (редактор сериализует ProseMirror ↔ markdown). `@tiptap/extension-link` (официальное расширение TipTap, [поправка ADR-062 §2](adr/ADR-062-documents-wysiwyg-tiptap.md) от 2026-07-18) — версия из линии установленного TipTap 2.27.x (2.x-расширения версионируются в lockstep с ядром); нужен, т.к. `@tiptap/starter-kit` Link-расширения НЕ содержит, а без него markdown-ссылки `[text](url)` теряют URL при round-trip ProseMirror↔markdown. **`@tiptap/extension-image` 2.x** ([ADR-068](adr/ADR-068-documents-image-attachments.md)) — тот же класс (официальное расширение линии 2.x, версия в lockstep с ядром), нужен для изображений в документе; регистрируется с **`inline: false`** и **`allowBase64: false`** (последнее — требование безопасности/модели: `data:`-картинки в `content_md` запрещены, любая картинка проходит через upload вложения). **`@tiptap/pm` 2.27.2** ([ADR-068](adr/ADR-068-documents-image-attachments.md), добавлен при реализации 2026-07-22) — **не новая библиотека в дереве**: это ProseMirror-бандл TipTap, который и раньше присутствовал транзитивно как peer-зависимость `@tiptap/core` (версия в `frontend/package-lock.json` — та же `2.27.2`, дерево не изменилось). В манифест вынесен явно, потому что появились **прямые** импорты ProseMirror-примитивов из `@tiptap/pm/*` (`frontend/src/features/documents/imageExtension.ts` — `@tiptap/pm/markdown`, `@tiptap/pm/model`; `imageUploadPlaceholder.ts` — `@tiptap/pm/state`, `@tiptap/pm/view`): импортируемое напрямую обязано быть в `dependencies`, иначе версия управляется чужим деревом. Пин `2.27.2` — тот же lockstep, что у остальных пакетов линии. **Итого TipTap — шесть пакетов** (см. врезку-амендмент в [ADR-062](adr/ADR-062-documents-wysiwyg-tiptap.md)) |
...
> **Модуль «Документы» — `/documents`** ([ADR-059](adr/ADR-059-documents-module.md)/[ADR-061](adr/ADR-061-documents-sidebar-two-panel-nav.md)/[ADR-062](adr/ADR-062-documents-wysiwyg-tiptap.md), [modules/documents](modules/documents/README.md)). **Backend:** новые таблицы `document_nodes`/`document_node_roles` (SQLAlchemy async + Alembic — уже в стеке); внутренний JWT-API + внешний read-only `/api/external/documents/*` (`X-API-Key`, `hmac.compare_digest` — stdlib). **Новая backend-зависимость — `python-multipart`** (multipart-парсинг для `POST /api/documents/upload`, `UploadFile`; см. backend-таблицу выше) — первый multipart-приёмник в проекте; иных backend-зависимостей нет. **Frontend:** **единственная новая зависимость — TipTap** (WYSIWYG, см. таблицу выше; markdown остаётся форматом хранения ⇒ редактор заменяем). kebab-меню — на **уже установленном** `@radix-ui/react-dropdown-menu`; `TreeView`/сайдбар-shell — своя реализация на существующих примитивах; модалка видимости — существующие Radix-Dialog + `MultiSelect`. `/documents` — второй **full-bleed** маршрут после `/mail` (частичный разворот плоской навигации [ADR-033](adr/ADR-033-flat-nav-theme-toggle-numbers-table.md), [ADR-061](adr/ADR-061-documents-sidebar-two-panel-nav.md)).
>
> **Изображения в документах** ([ADR-068](adr/ADR-068-documents-image-attachments.md)). **Backend:** таблица `document_attachments` + байты **файлами на volume** `documents-attachments` (stdlib `os`/`hashlib` — новых зависимостей нет; `python-multipart` уже добавлен ради `POST /upload`). Определение типа — **собственная проверка magic bytes** по whitelist из 4 сигнатур; библиотека вроде `python-magic`/`filetype` **не вводится** (тянет `libmagic`/лишний слой ради четырёх констант — NFR-1). **Frontend:** новых библиотек в дереве нет — в манифест добавлены `@tiptap/extension-image` (официальное расширение уже принятой линии TipTap 2.x) и `@tiptap/pm` (ProseMirror-бандл, ранее транзитивный peer `@tiptap/core`; явный пин под прямые импорты `@tiptap/pm/*`), см. таблицу выше; резолвер `blob:`-URL — свой NodeView на `fetch`, без библиотек. **CSP:** `img-src` расширяется `blob:` ([05-security.md](05-security.md#csp)).

...
| TipTap | `@tiptap/react` 2.x, `@tiptap/starter-kit` 2.x, `@tiptap/extension-link` 2.x, `tiptap-markdown` 0.8.x | **WYSIWYG-редактор документов** ([ADR-062](adr/ADR-062-documents-wysiwyg-tiptap.md), [modules/documents](modules/documents/README.md)). Первая rich-text библиотека — осознанный отход от NFR-1; хранение остаётся **markdown** в `content_md` (редактор сериализует ProseMirror ↔ markdown). `@tiptap/extension-link` (официальное расширение TipTap, [поправка ADR-062 §2](adr/ADR-062-documents-wysiwyg-tiptap.md) от 2026-07-18) — версия из линии установленного TipTap 2.27.x (2.x-расширения версионируются в lockstep с ядром); нужен, т.к. `@tiptap/starter-kit` Link-расширения НЕ содержит, а без него markdown-ссылки `[text](url)` теряют URL при round-trip ProseMirror↔markdown |

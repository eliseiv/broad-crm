...
| React Router | 6.27.x | Роутинг (`/login`, `/servers`, `/ai-keys`; `/servers` и `/ai-keys` — под общим `AppLayout` со вкладками) |
...
├── frontend/           # React + Vite
│   └── src/
│       ├── pages/      # LoginPage, ServersPage, AiKeysPage
│       ├── layouts/    # AppLayout (вкладки Servers/AI Keys, общий для /servers и /ai-keys)
...
│       ├── features/   # auth, servers, ai-keys (api+hooks; reorder-мутации)

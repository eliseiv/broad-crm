...
> **Страница «Пользователи» — вкладка `/users` (admin-only), RBAC** ([ADR-021](adr/ADR-021-rbac-users-roles.md), [08-design-system.md](08-design-system.md#страница-пользователи)). Седьмая вкладка `AppLayout`, **не-full-bleed**, видна только супер-админу / роли `admin`. Добавляются: маршрут `/users`, фича `features/users` (`api.ts`/`hooks.ts` на TanStack Query: users/roles/permissions-catalog/me), страница со списком пользователей (модалка create/edit: username, пароль, `Select` роли) и редактором ролей (матрица прав страницы×действия). Новый UI-примитив **`Checkbox`** — нативный стилизованный `<input type="checkbox">` в `components/ui` (**без новой зависимости**, по образцу нативного `Select`; `@radix-ui/react-checkbox` НЕ добавляется). UI-гейтинг вкладок/кнопок — по правам из `GET /api/auth/me`; безопасность — на сервере (`403`). Backend RBAC зависимостей не добавляет (bcrypt — уже в backend-таблице выше).

> **Модуль «Почты» — вкладка `/mail`, read-through без новых зависимостей** ([ADR-012](adr/ADR-012-mail-read-through-proxy.md), [modules/mail](modules/mail/README.md)). Добавляются: маршрут `/mail` (вкладка **«Почты»** в `AppLayout` — **вторая**, после «Дашборд»; дефолтный маршрут теперь `/dashboard` — [ADR-013](adr/ADR-013-mail-newest-first-master-detail-inline-reply.md), [ADR-017](adr/ADR-017-dashboard-client-aggregation-mail-server-filters.md)), фича `features/mail` (`api.ts`/`hooks.ts` на TanStack Query, `useInfiniteQuery` для бесконечной ленты) и страница `MailPage` (master-detail, inline-reply, тулбар фильтров «С тегами» + дропдауны «Почта»/«Команда»). Новый UI-примитив **`Textarea`** (нативный стилизованный `<textarea>` в `components/ui`, для поля ответа) — **без новой зависимости**; дропдауны фильтров используют существующий примитив **`Select`**. **HTML-тело письма изолируется sandbox-iframe** (`<iframe srcDoc sandbox>` без `allow-scripts`/`allow-same-origin`) — **без DOMPurify и без иных новых зависимостей** ([08-design-system.md](08-design-system.md#компонент-textarea), [08-design-system.md](08-design-system.md#страница-почты)). Backend модуля почты **не добавляет** зависимостей и БД: переиспользует `httpx` (клиент к `postapp.store`); прокси-эндпоинты `teams`/`mailboxes` и фильтры `mail_account_id`/`group_id` — [ADR-017](adr/ADR-017-dashboard-client-aggregation-mail-server-filters.md).

> **Страница «Дашборд» — вкладка `/dashboard`, клиентская агрегация без backend** ([ADR-017](adr/ADR-017-dashboard-client-aggregation-mail-server-filters.md), [08-design-system.md](08-design-system.md#страница-дашборд)). Первая вкладка `AppLayout` и **дефолтный маршрут после логина**; идёт по **не-full-bleed** ветке (обычный поток документа). Фича `features/dashboard` (карточки-разделы на TanStack Query) считает счётчики клиентски из существующих `GET /api/mail/mailboxes`, `GET /api/servers`, `GET /api/ai-keys` — **без нового backend-эндпоинта и без новых зависимостей**. Клик по карточке — навигация в раздел (`react-router`).

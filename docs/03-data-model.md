...
erDiagram
    SERVERS {
        ...
        inet ip
        text ssh_user
        text auth_method "password|key (ADR-067)"
        bytea ssh_password_encrypted "NULL при auth_method=key"
        bytea ssh_private_key_encrypted "NULL при auth_method=password"
        bytea ssh_key_passphrase_encrypted "NULL (опц., только с ключом)"
        int exporter_port
...
| `auth_method` | `text` | `NOT NULL`, `DEFAULT 'password'`, CHECK `IN ('password','key')` | **Способ входа** ([ADR-067](adr/ADR-067-server-ssh-key-auth.md)). Не секрет — отдаётся в API. |
| `ssh_password_encrypted` | `bytea` | **`NULL`** (было `NOT NULL` до [ADR-067](adr/ADR-067-server-ssh-key-auth.md)) | Fernet-ciphertext SSH-пароля. Заполнен ⇔ `auth_method='password'`. Plaintext никогда не хранится и не логируется. |
| `ssh_private_key_encrypted` | `bytea` | `NULL` | Fernet-ciphertext **приватного SSH-ключа** (нормализованный OpenSSH/PEM). Заполнен ⇔ `auth_method='key'` ([ADR-067](adr/ADR-067-server-ssh-key-auth.md)). |
| `ssh_key_passphrase_encrypted` | `bytea` | `NULL` | Fernet-ciphertext **парольной фразы** ключа. Опциональна; допустима **только** при `auth_method='key'`. |
...
CREATE TABLE servers (
    ...
    ssh_key_passphrase_encrypted bytea,
    ...
    CONSTRAINT ck_servers_auth_method CHECK (auth_method IN ('password','key')),
    -- ADR-067: ровно ОДИН способ входа на запись; passphrase — только вместе с ключом.
    CONSTRAINT ck_servers_auth_material CHECK (
...
- **`ck_servers_auth_material` — граница целостности, а не дубль валидации сервиса** ([ADR-067](adr/ADR-067-server-ssh-key-auth.md)): «половинчатая» строка (`key` без ключа, `password` без пароля, пароль **и** ключ одновременно, passphrase без ключа) не появится даже при ошибке сервиса, ручном `UPDATE` или data-миграции. На нём же держится отказ от флагов `has_password`/`has_key` в API: `auth_method` определяет наличие материала **однозначно**.
...
## Шифрование SSH-материала (`ssh_password_encrypted` / `ssh_private_key_encrypted` / `ssh_key_passphrase_encrypted`)

- Алгоритм: **Fernet** (`cryptography`), симметричный AES-128-CBC + HMAC.
- Ключ: `FERNET_KEY` из `.env` (base64, 32 байта). Никогда не в коде/БД/логах. **Все три материала шифруются ОДНИМ примитивом и ОДНИМ ключом** ([ADR-007](adr/ADR-007-shifrovanie-fernet.md), [ADR-067](adr/ADR-067-server-ssh-key-auth.md)) — отдельного ключа для SSH-ключей не вводится; ротация — [TD-006](100-known-tech-debt.md).
- Шифрование при `POST /api/servers` (после нормализации и разбора ключа — [04-api.md](04-api.md#post-apiservers)), расшифровка только в памяти провижининг-сервиса непосредственно перед запуском Ansible **либо** в обработчике reveal-эндпоинта (только для пароля).
- В ответах API пароль/ключ/парольная фраза (ни в каком виде) НЕ возвращаются. **Приватный ключ и парольная фраза — write-only: reveal-эндпоинтов для них не существует** ([ADR-067](adr/ADR-067-server-ssh-key-auth.md) §4). Детали — [05-security.md](05-security.md#защита-ssh-кредов-целевых-серверов).

## Миграция `0031_servers_ssh_key_auth` (концепт, [ADR-067](adr/ADR-067-server-ssh-key-auth.md) §2)

> `revision = "0031_servers_ssh_key_auth"` — **25 символов** ≤ 32 ([правило id](#1-revision-id--не-длиннее-32-символов)). `down_revision` = фактическая голова цепочки на момент реализации (backend сверяет по `alembic_version`; сейчас — `"0030_document_node_roles"`).

**`upgrade()`** — строго в этом порядке:

1. `ADD COLUMN auth_method text NOT NULL` **со `server_default='password'`**;
2. `ADD COLUMN ssh_private_key_encrypted bytea` и `ssh_key_passphrase_encrypted bytea` (обе `NULL`);
3. `ALTER COLUMN ssh_password_encrypted DROP NOT NULL`;
4. `CREATE` обоих CHECK (`ck_servers_auth_method`, `ck_servers_auth_material`) — **последним шагом**.

**Backfill данных НЕ требуется — это следствие порядка, а не совпадение.** `server_default` проставляет `'password'` всем существующим строкам **в момент `ADD COLUMN`**, а `ssh_password_encrypted` у них уже заполнен ⇒ к шагу 4 каждая существующая строка удовлетворяет `ck_servers_auth_material`. Отдельного `UPDATE` нет. **Совместимость со старыми записями полная:** старый сервер = `auth_method='password'`, ведёт себя ровно как прежде (тот же провижининг, тот же reveal пароля).
...
2. **`DELETE FROM servers WHERE auth_method = 'key'`** — записи key-серверов в старой схеме **непредставимы** (`ssh_password_encrypted NOT NULL`, а пароля у них нет и взять неоткуда). Подстановка пароля-заглушки **ЗАПРЕЩЕНА**: после отката провижининг пошёл бы на хост с заведомо неверным паролем, а оператор видел бы «пароль задан». Каскады штатные (`notifier_server_state` — `CASCADE`; `notifier_alert_log`/`backends.server_id` — `SET NULL`). Перед `DELETE` — запись в лог числа удаляемых строк (без секретов): потеря не должна быть молчаливой;
3. `ALTER COLUMN ssh_password_encrypted SET NOT NULL`;
4. `DROP COLUMN` трёх новых колонок.

> **⚠️ Откат этой миграции ЛОССИ по строкам, а не только по колонкам.** Обязателен pre-deploy `pg_dump` ([07-deployment.md §Откат миграций БД](07-deployment.md#откат-миграций-бд)); осиротевшие `targets/<id>.json` удалённых серверов убираются регенерацией file_sd из БД ([09-provisioning.md](09-provisioning.md#восстановление-file_sd-из-бд)).
...
| `leader_id` | `uuid` | **`NULL`**, FK → `users(id)` **`ON DELETE SET NULL`** | Лидер команды ([ADR-026](adr/ADR-026-teams-optional-leader-auto-transfer.md)). **`NULL` — команда без лидера.** Удаление пользователя-лидера **не блокируется КОМАНДОЙ** (лидерство авто-передаётся сервисом, остаток — `SET NULL`; `409 user_is_team_leader` **упразднён**). ⚠️ Это не значит «удаление пользователя всегда проходит»: независимое основание отказа — авторство документов/вложений (`409 user_in_use`, [04-api.md](04-api.md#delete-apiusersid), [TD-077](100-known-tech-debt.md)); при нём авто-передача откатывается вместе с транзакцией. **Инвариант: если `leader_id` задан — он ∈ участники** (в `user_teams` есть строка `(leader_id, id)`) — обеспечивает сервис. |
| `created_at` | `timestamptz` | `NOT NULL`, `DEFAULT now()` | Дата создания. |
| `updated_at` | `timestamptz` | `NOT NULL`, `DEFAULT now()` | Дата последнего изменения. |
...
- **Пользователь** — hard delete; членства снимаются `ON DELETE CASCADE`. Если пользователь — лидер хотя бы одной команды, удаление **не блокируется КОМАНДОЙ** ([ADR-026](adr/ADR-026-teams-optional-leader-auto-transfer.md)): сервис **авто-передаёт лидерство** следующему участнику по `user_teams.created_at` (или `leader_id → NULL`, если участников не осталось; `ON DELETE SET NULL` — предохранитель), затем удаляет пользователя. Код `409 user_is_team_leader` **упразднён**. ⚠️ **Но само удаление может быть отклонено по независимому основанию:** пользователя держит FK `ON DELETE RESTRICT` от `document_nodes.owner_id` / `document_attachments.created_by` → **`409 user_in_use`** ([04-api.md](04-api.md#delete-apiusersid), [TD-077](100-known-tech-debt.md)); в этом случае авто-передача лидерства **откатывается вместе с транзакцией** — команды остаются со старым лидером.
...
### Политика удаления
- **Пользователь** — hard delete; добавки снимаются `ON DELETE CASCADE`. ⚠️ **Но удаление пользователя может быть заблокировано другим FK `RESTRICT`:** автор документов (`document_nodes.owner_id`) или загрузивший вложение (`document_attachments.created_by`) → **`409 user_in_use`** ([04-api.md](04-api.md#delete-apiusersid), [TD-077](100-known-tech-debt.md)).
...
- **Пользователь** — hard delete; `sms_telegram_links`/`sms_deliveries` (`user_id`) — `ON DELETE CASCADE`; `sms_phone_numbers.added_by_user_id` — `SET NULL`; `sms_inbound` не затрагивается. ⚠️ **Но удаление пользователя может быть заблокировано другим FK `RESTRICT`:** автор документов (`document_nodes.owner_id`) или загрузивший вложение (`document_attachments.created_by`) → **`409 user_in_use`** ([04-api.md](04-api.md#delete-apiusersid), [TD-077](100-known-tech-debt.md)).
...
- **Пользователь-автор** — `ON DELETE RESTRICT`: пока у пользователя есть узлы, удаление заблокировано (переназначение авторства/зачистка — прикладная операция). **Прикладной исход — `409 user_in_use`** ([04-api.md §DELETE /api/users/{id}](04-api.md#delete-apiusersid)), а не `500`: `UserService.delete_user` перехватывает `IntegrityError` (зеркало FK — тот же принцип, что `409 role_in_use`). **Soft-deleted узлы блокируют так же, как живые** — tombstone остаётся физической строкой с `owner_id`. Текущее состояние реализации — [TD-077](100-known-tech-debt.md).
...
## Таблица `document_attachments` ([ADR-068](adr/ADR-068-documents-image-attachments.md))

Метаданные **изображений**, вставленных в документ. Решение — [ADR-068](adr/ADR-068-documents-image-attachments.md) (решение владельца: **байты — файлом на volume**, метаданные — в БД; ссылка в markdown — `![alt](/api/documents/attachments/{id})`). Модуль — [modules/documents](modules/documents/README.md), контракт — [04-api.md#attachments](04-api.md#вложения-изображения-документов-adr-068), безопасность — [05-security.md](05-security.md#вложения-изображения-документов-нормативно-adr-068). Миграция — **`0032_document_attachments`**.

**Разделение носителей (нормативно):** строка — источник истины о **существовании и свойствах** вложения; файл — только байты. Строки без файла быть не может (порядок записи — [ADR-068](adr/ADR-068-documents-image-attachments.md) §2); файл без строки возможен как аномалия и никем не адресуем (убирается GC — [TD-076](100-known-tech-debt.md)).

| Поле | Тип | Ограничения | Описание |
|------|-----|-------------|----------|
| `id` | `uuid` | PK, `DEFAULT gen_random_uuid()` | Идентификатор вложения. **Единственный источник пути на диске** и адреса в markdown. |
| `document_node_id` | `uuid` | `NOT NULL`, FK → `document_nodes(id)` **`ON DELETE CASCADE`** | Узел-владелец. Обязан быть `node_type='document'` (проверка — в сервисе: CHECK через FK невыразим). |
| `filename` | `text` | `NOT NULL`, CHECK `char_length BETWEEN 1 AND 255` | Исходное имя файла — **только** для `Content-Disposition` и alt-текста. **В пути на диске НЕ участвует** ([ADR-068](adr/ADR-068-documents-image-attachments.md) §1). |
| `mime` | `text` | `NOT NULL`, CHECK `IN ('image/png','image/jpeg','image/webp','image/gif')` | Тип, определённый **по содержимому** (magic bytes) при загрузке. Управляет `Content-Type` отдачи и расширением файла. |
| `size_bytes` | `integer` | `NOT NULL`, CHECK `> 0` | Размер в байтах. Верхняя граница — `DOCUMENTS_MAX_IMAGE_BYTES` (env, проверка в сервисе: в CHECK её вшивать нельзя — менялась бы миграцией). |
| `checksum` | `text` | `NOT NULL`, CHECK `char_length = 64` | **sha256 содержимого**, hex lowercase. Используется как **`ETag`** отдачи и для контроля целостности после восстановления volume. UNIQUE **не вводится** (дедупликация — [ADR-068](adr/ADR-068-documents-image-attachments.md) §1). |
| `created_by` | `uuid` | `NOT NULL`, FK → `users(id)` **`ON DELETE RESTRICT`** | Кто загрузил (для отображения/аудита) — симметрично `document_nodes.owner_id`; **не гейт** (enforcement permission-based). Действия консольного супер-админа — `SUPERADMIN_USER_ID` ([ADR-051](adr/ADR-051-superadmin-db-anchor-personal-state.md)). |
| `created_at` | `timestamptz` | `NOT NULL`, `DEFAULT now()` | Дата загрузки. |

> **`updated_at` нет намеренно:** вложение **иммутабельно** — перезаписи файла не существует, «замена картинки» = новая загрузка (новый `id`) + правка `content_md` (`content_version += 1`). Именно на этой иммутабельности стоит отказ расширять внешний RAG-контракт ([ADR-068](adr/ADR-068-documents-image-attachments.md) §6).
...
CREATE TABLE document_attachments (
    id                uuid PRIMARY KEY DEFAULT gen_random_uuid(),
    document_node_id  uuid NOT NULL REFERENCES document_nodes(id) ON DELETE CASCADE,
    ...
    CONSTRAINT ck_document_attachments_filename_len CHECK (char_length(filename) BETWEEN 1 AND 255),
    CONSTRAINT ck_document_attachments_mime CHECK (
        mime IN ('image/png','image/jpeg','image/webp','image/gif')),
    CONSTRAINT ck_document_attachments_size CHECK (size_bytes > 0),
    CONSTRAINT ck_document_attachments_checksum CHECK (char_length(checksum) = 64)
);

CREATE INDEX ix_document_attachments_node_id ON document_attachments (document_node_id);
CREATE INDEX ix_document_attachments_created_by ON document_attachments (created_by);
```

### Индексы и обоснование
- `ix_document_attachments_node_id` — вложения узла (копирование поддерева, GC, обслуживание `ON DELETE CASCADE`).
- `ix_document_attachments_created_by` — обслуживание `ON DELETE RESTRICT` при удалении пользователя (иначе seq-scan на каждой попытке удаления учётки) — та же роль, что `ix_document_nodes_owner_id`.
- Индекс по `checksum` **не заводится**: дедупликации нет, поиск по контрольной сумме — не сценарий.

### Расположение файла на диске (нормативно)

```
${DOCUMENTS_ATTACHMENTS_DIR}/<id[0:2]>/<id[2:4]>/<id>.<ext>
```

- `<ext>` — **производное от `mime`** по константному whitelist-маппингу (`image/png→png`, `image/jpeg→jpg`, `image/webp→webp`, `image/gif→gif`), **не** от `filename`;
- пользовательские строки в построении пути **не участвуют вовсе** ⇒ path traversal невозможен конструктивно ([ADR-068](adr/ADR-068-documents-image-attachments.md) §1);
- два уровня шардинга (`aa/bb`) — против вырождения каталога в десятки тысяч записей;
- запись — **атомарная**: temp-файл в **корне** `DOCUMENTS_ATTACHMENTS_DIR` (та же файловая система, что финальный шард-путь ⇒ `os.replace` не упрётся в `EXDEV`) + `os.replace` на финальное имя, как у file_sd-таргетов. Шард-каталоги создаются (`0700`) перед `replace`;
- **корневой каталог создаётся в backend-образе** (`mkdir -p` + `chown app:app` + `0700`): backend работает под non-root `USER app`, а пустой named-volume Docker создаёт `root:root` — без этого не создастся даже первый шард-каталог ([07-deployment.md](07-deployment.md#каталоги-создаваемые-в-образе-нормативно));
- **права: файлы `0600`, каталоги `0700`** — намеренно **строже**, чем `0644`/`0755` у file_sd ([09-provisioning.md](09-provisioning.md#регистрация-таргета-file_sd)): там файл читает **второй процесс под другим uid** (Prometheus), здесь читатель ровно один — backend. «Унификация» прав до world-readable = дефект.

### Политика удаления

- **Вложение** — hard delete строки (в транзакции) + удаление файла **после `commit`** (best-effort; сбой → лог + GC [TD-076](100-known-tech-debt.md)).
- **Узел (soft-delete)** — строки и файлы **не трогаются**; отдача вложения узла с `deleted_at IS NOT NULL` → `404 document_attachment_not_found`. Физическая уборка — вместе с GC tombstones ([TD-067](100-known-tech-debt.md), расширен на файлы).
- **Узел (физический GC/hard delete)** — строки уходят `ON DELETE CASCADE`; файлы обязан снять тот же GC (БД о файлах ничего не знает).
- **Пользователь** — `ON DELETE RESTRICT`: учётку с загруженными вложениями нельзя удалить, пока они существуют (симметрично `document_nodes.owner_id`). Прикладной исход — **`409 user_in_use`** ([04-api.md §DELETE /api/users/{id}](04-api.md#delete-apiusersid)); перехват реализован и покрыт тестами (2026-07-23), остаток долга (передача владения) — [TD-077](100-known-tech-debt.md).

## Миграция `0032_document_attachments` (концепт, [ADR-068](adr/ADR-068-documents-image-attachments.md))

> `revision = "0032_document_attachments"` — **25 символов** ≤ 32 ([правило id](#1-revision-id--не-длиннее-32-символов)). `down_revision` = фактическая голова цепочки на момент реализации (при последовательной реализации обеих фич — `"0031_servers_ssh_key_auth"`; при иной очерёдности backend сверяет по `alembic_version` и ставит фактическую голову).

...
| [ADR-022](ADR-022-teams-nav-categories.md) | CRM-команды (`teams`+`user_teams` M2M, лидер+участники, дизамбигуация vs mail-`groups`) + `users.email` + каталог прав `roles`/`teams` (амендмент ADR-021 §1, роли под матрицей `roles:*`, security-инвариант эскалации) + категоризированная навигация (3 дропдауна, «Дашборд» вне меню, permission-aware дефолт) | accepted | 2026-07-08 |

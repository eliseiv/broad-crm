...
> **Амендмент [ADR-032](ADR-032-sms-visibility-admin-full-catalog.md):** «видит все команды» расширено с одного `is_superadmin` до **`is_superadmin` ИЛИ роль владеет полным каталогом прав** (admin-роль). «Не-админ» ниже читается как «актор не admin-уровня» (`sees_all_teams=False`).
...
> **Амендмент [ADR-034](ADR-034-teams-number-login-app.md) (2026-07-09):** сужение частично развёрнуто — `TeamNumberItem` теперь **включает** `login` и `app_name` (под `teams:view`), но `note`/`label` остаются сужёнными (только под `sms:*`). Ниже — исходная формулировка сужения; актуальная схема `TeamNumberItem` — [04-api.md](../04-api.md#teams) и [ADR-034](ADR-034-teams-number-login-app.md).

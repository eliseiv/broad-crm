...
- **Амендирован:** [ADR-039](ADR-039-ui-server-inline-edit-backends-search-empty-sms-label.md) (editable-scope §6 **сервера**: карандаш открывает не отдельную `AddServerModal`, а **inline-edit** поля `name` прямо в `ServerDetailModal`) · [ADR-040](ADR-040-backend-relations-secrets-reverse-lookup.md) (у **бэка** появились секреты `api_key`/`admin_api_key` → reveal-эндпоинты `GET /api/backends/{id}/api-key` и `/admin-api-key` под `backends:edit`; строка «Бэк — секрета нет» в таблицах ниже более не действует) · **[ADR-049](ADR-049-servers-backends-card-first-detail.md) §3 (2026-07-13)** · **[ADR-067](ADR-067-server-ssh-key-auth.md) §4/§6 (2026-07-22)**
- > **⚠️ Амендмент 2026-07-22 ([ADR-067](ADR-067-server-ssh-key-auth.md) §4/§6) — СУЖЕНИЕ для сервера с `auth_method='key'`.** Строка «Сервер» в таблице §1 и пункт §2 про `GET /api/servers/{id}/ssh-password` описывают **только** `auth_method='password'`. Для key-сервера: (1) приватный ключ и парольная фраза — **write-only, reveal-эндпоинтов НЕ существует** (радиус поражения ключа — весь парк хостов); (2) `GET /api/servers/{id}/ssh-password` отдаёт **`404 secret_not_set`**; (3) в detail-view — строка **«SSH-ключ»** с маской `••••••••` и **БЕЗ кнопки-глаза** — первый не-раскрываемый секрет в системе: **глаз не рендерится ни при каком праве**, включая `servers:edit` (правило §3 «глаз при `<page>:edit`» здесь не применяется). Плюс добавлена строка **«Способ входа»**. Действующий канон detail-view — [08-design-system.md](../08-design-system.md#состав-detail-view-нормативно-adr-046-2в-в-редакции-adr-049-0). Для `password`-серверов, прокси, ИИ-ключей и бэков решение ADR-035 — **в силе без изменений**.
- > **⚠️ Для `/backends` паттерн «клик по карточке → read-only detail-модалка → карандаш в её шапке» ОТМЕНЁН** ([ADR-049](ADR-049-servers-backends-card-first-detail.md) §3): `BackendDetailModal` **упразднена**, вся информация — на карточке (свёрнутый блок «Информация»), карандаш (`backends:edit`) — **в блоке действий карточки**. Для **`/servers`, `/proxies`, `/ai-keys`** паттерн **в силе без изменений**. **Гарантии on-demand reveal секретов (гейт `<page>:edit`, секрет не преднагружается, `Cache-Control: no-store`, аудит `secret_revealed`, значение только в локальном стейте) НЕ ослаблены нигде** — [ADR-049](ADR-049-servers-backends-card-first-detail.md) §4.

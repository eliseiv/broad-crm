...
> **Амендмент (ADR-076, 2026-08-17):** утверждение «`documents:share` выдаётся только через каталог/PATCH» в связке с UI-матрицей более не действует как пробел — матрица `/roles` рендерит extra-действия каталога, включая `share`. Само действие `share` и его гейты **в силе**. См. [ADR-076](ADR-076-knowledge-bot-broadcast-and-admin-level.md) §4.

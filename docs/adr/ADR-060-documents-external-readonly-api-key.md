...
> **Амендмент (ADR-076, 2026-08-17):** утверждение «внешний контур CRM под `X-API-Key` — только GET» сужается до **роутера `/api/external/documents/*`**. Write-регистрация запуска ИИ-бота живёт на **отдельном** префиксе `/api/external/knowledge-bot/*` (тот же ключ `DOCUMENTS_API_KEY`). Инвариант «документы — только GET» **в силе**. См. [ADR-076](ADR-076-knowledge-bot-broadcast-and-admin-level.md) §2.

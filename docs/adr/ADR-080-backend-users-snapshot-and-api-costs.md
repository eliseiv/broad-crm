...
**Воркер `BackendUsersSnapshotService`** — канон `BackendMonitorService`: немедленный `refresh_once()` при старте, затем `while True: refresh_once(); await asyncio.sleep(interval)`; исключение итерации логируется и задачу не валит. Регистрация — `asyncio.create_task` в `lifespan` (`app/main.py`), рядом с существующими мониторами. Fan-out по бэкам с ключом, `Semaphore(5)` (тот же лимит, что у сегодняшнего fan-out). Алгоритм на бэк:
...
7. **Любое исключение** → прошлый снимок **не трогается**, в строку источника пишутся `error_message` / `failed_at`. Устаревшие данные лучше пустого экрана; их возраст виден оператору из `snapshot_at`.
   > ⚠️ **Открытый вопрос [Q-BU-2](../99-open-questions.md):** при **постоянном** отказе источника на одной и той же странице (жёсткий rate-limit) цикл падает там же каждый раз ⇒ `refreshed_at` не проставляется **никогда** и `snapshot_at` остаётся `null` при накапливающихся строках. Backoff на `429`/`5xx` внутри страницы сейчас нет, ограничитель обхода — только `_MAX_PAGES`.

...
## Амендмент (прод-инцидент 2026-08-20, вторая волна)

Снимок собирался (68 262 строки, 20 из 30 источников обновлялись каждый цикл), но
страница показывала «Снимок формируется…» бессрочно. Разобрано по данным прода — логам
воркера и сырым ответам апстрима.

1. **`snapshot_at` больше не гаснет из-за одного сломанного источника.** Прежняя норма
   («хоть один не собран → `null`») на практике означала: три бэка, которые не соберутся
   никогда (у двух отвергнут admin-ключ, третий не отдаёт контракт v1), навсегда скрывали
   готовые данные всех остальных. Метка = `MIN(refreshed_at)` по **собранным** источникам;
   `null` — только когда не собран ни один. Несобранные названы в `errors[]` — там и место
   для «этот бэк в выдачу не попал».

2. **Отказ карточки пользователя не роняет цикл.** Логи показали: `500` приходит на
   `get_user` отдельных пользователей, тогда как список, `/stats` и `/products` у тех же
   бэков исправны (проверено сырым запросом — 200 с первой попытки). Фаза экономики
   вторична по отношению к списку: её отказ означает «расходы неполны», а не «список
   недостоверен». Цикл завершается, `revenue_backfill_done` не выставляется ⇒ `partial`
   в UI горит. Отказ **обхода списка** по-прежнему роняет цикл: неполный список нельзя
   пускать в `delete_rows`.

3. **Терпение к `429` увеличено** (`attempts` 5 → 8, `base` 1 → 2 с): у бэков с десятками
   тысяч пользователей прежний бюджет (~10 с) исчерпывался до открытия окна лимита.

⚠️ Диагноз первой волны хотфикса («воркер сам выбивает источники в 429») подтвердился
лишь частично: 429 действительно возникает на обходе крупных бэков, но **большинство
отказов были `500` на карточке** — их первая волна не объясняла и не лечила.

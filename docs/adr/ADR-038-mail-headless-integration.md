...
- **Enforcement (граница безопасности — backend, не UI):**
  - **Чтение/список** (`GET /api/mail/messages`, `/mailboxes`, `/tags` в части ленты): вне scope → **пустой результат** (анти-энумерация), не `403`. При `sees_all_teams=false` сервис пробрасывает во внешний API `group_id` = `MailScope.group_ids` (пересечение с пользовательским фильтром `group_id`, если задан); пустой `group_ids` → пустая страница без вызова внешнего API.

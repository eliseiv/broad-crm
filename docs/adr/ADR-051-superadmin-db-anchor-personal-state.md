...
> **Амендмент ([ADR-079](ADR-079-users-m2m-roles-full-name-telegram-table.md), 2026-08-19):** утверждение «зеркало FK **`users.role_id` → `roles.id` ON DELETE RESTRICT**» более не действует буквально — колонка `users.role_id` **дропнута**, связь пользователь↔роль живёт в M2M-таблице **`user_roles`** (миграция `0038_user_roles_m2m`, FK ролевой стороны — по-прежнему **`ON DELETE RESTRICT`**). Обе нормы §1.5 сохранены и переносятся на `user_roles`: `is_in_use` читает `user_roles` **без** фильтра `is_system` (иначе `DELETE` роли якоря дал бы `500` вместо `409 role_in_use`), а `user_count` — **`COUNT(DISTINCT u.id)`** через `user_roles` с `WHERE NOT users.is_system`. Строку якоря в `user_roles` создаёт тот же идемпотентный bootstrap (`INSERT … ON CONFLICT DO NOTHING`); миграция `0038` переносит существующую связь якоря вместе со всеми остальными. Значение `role_id` в таблице нормативных полей якоря (§1.1) читать как «**роль(и) якоря**» — резолв-цепочка `admin` → самая ранняя роль → создать `admin` **не меняется**.
...
> **Амендмент (ADR-076, 2026-08-17):** утверждение «`require_admin` = `is_superadmin or role == "admin"`» более не действует — предикат `is_admin_level` (добавлена ветка «полный каталог»). Для супер-админа исход **не меняется** (первая ветка истинна). Якорь по-прежнему не участвует.

...
> **Амендмент (ADR-077, 2026-08-17):** утверждение не-full-bleed ветки `AppLayout` **в силе**. Внутренняя раскладка страницы: основной блок композера / loading / error / empty **горизонтально и вертикально центрируется в рабочей области** под хэдером (не в навбаре), на пустом фоне — [ADR-077](ADR-077-broadcast-page-visual-redesign.md) §«Раскладка в рабочей области» (статус **implemented**, сверка `BroadcastPage.tsx:24-28`). `/broadcast` в `isFullBleed` не входит.
...
> **Амендмент (ADR-078, 2026-08-17):** утверждение «клиент считает `is_admin_level` через `GET /api/permissions/catalog` + `coversFullCatalog`; `catalogPending` → Spinner; ошибка каталога → не admin» **более не действует**. Клиент читает `me.is_admin_level` из `GET /api/auth/me`. Каталог остаётся только для матрицы `/roles`. См. [ADR-078](ADR-078-me-is-admin-level.md).

> **[ОТМЕНЁН — не реализовывать]** абзац ниже — историческая сверка клиента на момент ADR-076. Норма — [ADR-078](ADR-078-me-is-admin-level.md).

**Клиентский `is_admin_level` (историческая сверка `frontend/src/features/auth/adminLevel.ts` на момент ADR-076; норма — [ADR-078](ADR-078-me-is-admin-level.md)).** Каталог (`GET /api/permissions/catalog`) грузится только если `!is_superadmin && role !== "admin" && roles:view ∈ permissions` (`needsPermissionsCatalog`). Пока `needsCatalog && !catalogReady` — `catalogPending=true`: пункт «Пользователи» в ряду — Spinner (не ссылка и не скрытие); `AdminRoute` — Spinner «Загрузка…», **не** заглушка. Ошибка загрузки каталога → `catalogPending=false`, `isAdmin` = покрытие (без каталога — false) → заглушка как обычно.

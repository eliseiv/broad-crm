...
- **Frontend `/tg/mail`** (публичный маршрут, self-hosted Telegram WebApp SDK, CSP `script-src 'self'` не ослабляется — как `/tg/sms`): на входе шлёт `initData` в `/api/mail/telegram/auth`, при `200` — сохраняет JWT и рендерит ленту `GET /api/mail/messages` под обычным `MailScope` (письма команд пользователя), вкладка «Сообщения» по умолчанию. Наследует тему Telegram.

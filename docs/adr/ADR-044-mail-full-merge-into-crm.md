...
>
> **`mail_tags.is_builtin` УПРАЗДНЁН ЦЕЛИКОМ** (миграция `0023_mail_tags_drop_is_builtin`). Конкретно **отменены** следующие нормы настоящего ADR:
> - **§2 DDL `mail_tags`:** колонки `is_builtin BOOLEAN NOT NULL DEFAULT false` **НЕТ** (дропнута).
> - **§5 схема `MailTagFull`:** поля `is_builtin` **НЕТ**; правило «`is_builtin=true` — удалять нельзя (`409`)» **отменено** — **удалить можно ЛЮБОЙ тег** (`DELETE /api/mail/tags/{id}` → `204`/`404`, ветки `409` нет).
> - **§5 «Builtin-теги, 6 кастомных и seed»:** **`seed_builtin_tags` в lifespan УБРАН** (вместе с `services/mail_bootstrap.py`, `domain/mail_builtin_tags.py`, `MailTagRepository.seed_builtin`). Причина: сев при старте **воскрешал** удалённый пользователем тег при ближайшем рестарте. Первичный сев 10 канонических тегов для **чистых** инсталляций перенесён в **data-миграцию `0023`** (`INSERT ... ON CONFLICT (name) DO NOTHING`, данные вшиты в тело миграции). Деления на «builtin / кастомные» больше не существует — каталог целиком управляется пользователем.
>
> **Прочее в §2/§5 (глобальность тегов, `UNIQUE (name)`, движок матчинга, `mail_tag_rules` с четырёхзначным CHECK) — ДЕЙСТВУЕТ.** Плюс [ADR-047](ADR-047-mail-fix-pack.md) добавляет `mail_accounts.number`/`app_name` (миграция `0024`) — см. §3 того ADR. Актуальный контракт — [04-api.md §Mail](../04-api.md#mail), [03-data-model.md](../03-data-model.md#таблицы-модуля-почты-mail_accounts-mail_messages-mail_tags-), [modules/mail](../modules/mail/README.md).
>
> **§12 (follow-up-правки docs) — ВЫПОЛНЕН** [ADR-047](ADR-047-mail-fix-pack.md) §0 (2026-07-11).
...
- **`mail_tags`** (глобальный админский каталог — **у тега нет владельца**, применяется ко всем письмам всех команд):
  - `id UUID PK`, `name TEXT NOT NULL`, `color TEXT NOT NULL CHECK (color ~ '^#[0-9A-Fa-f]{6}$')`, `match_mode TEXT NOT NULL DEFAULT 'any' CHECK (match_mode IN ('any','all'))`, ~~`is_builtin BOOLEAN NOT NULL DEFAULT false`~~ (**колонка УПРАЗДНЕНА** — [ADR-047](ADR-047-mail-fix-pack.md) §1, миграция `0023`), `created_at`/`updated_at`.
...
> **Уточнение (2026-07-13, амендмент — агрегаторский `ADR-0046`; контракт и вариант A не меняются).** Формулировка «на **каждое** изменение статуса синка» разворачивается в агрегаторе в **исчерпывающий проверяемый перечень точек hook H1–H7** (+ не-hook точки N1–N6 с обоснованием) — источник истины: `mail-agregator/docs/adr/ADR-0046-mailbox-status-hook-points.md` §3/§4. CRM-сторона (эндпоинт, тело из 5 полей, коды, идемпотентность по `down_alert_sent_at`) — **без изменений**. Нормативно для CRM из этого амендмента следует два факта:
> - **`last_synced_at` = время последней УСПЕШНОЙ синхронизации** (агрегаторский `ADR-0046` §1): ошибочные ветки синка его **не обновляют**. Следствие для UI: у сбоящего ящика колонка «Синхронизация» **стареет** — это корректное поведение, а не потерянный push. Кружок статуса от свежести `last_synced_at` **не зависит** (формула — только `is_active`/`consecutive_failures`/`last_sync_error`, см. [08-design-system.md](../08-design-system.md#вкладка-почты-нормативно)).
> - **OAuth needs-consent** (Outlook, `invalid_grant`) агрегатор отражает **через существующее поле** `last_sync_error` (маркер `oauth_needs_consent: …`) + push статуса — новых полей в контракте **не появляется**, кружок краснеет по имеющейся формуле. Раньше такой ящик оставался в CRM «зелёным», не синкаясь вовсе.
...
- **`MailTagFull`** `{ id: uuid, name: string(1..64), color: string(^#[0-9A-Fa-f]{6}$), match_mode: enum{any,all}, rules: MailTagRule[], created_at, updated_at }`. **⚠️ Поля `is_builtin` НЕТ и правило «удалять нельзя (`409`)» ОТМЕНЕНО** ([ADR-047](ADR-047-mail-fix-pack.md) §1): **удалить можно ЛЮБОЙ тег**.
...
**~~Builtin-теги, 6 кастомных и seed~~ — ЧАСТИЧНО ОТМЕНЕНО ([ADR-047](ADR-047-mail-fix-pack.md) §1).** ⚠️ **`seed_builtin_tags` в lifespan УБРАН** (воскрешал удалённые теги при рестарте); признак `is_builtin` упразднён; первичный сев 10 канонических тегов для чистых инсталляций — **однократно data-миграцией `0023`**. Текст ниже описывает ИСХОДНЫЙ план миграции прода (исполнен) и сохранён как исторический. Действующее состояние каталога (16 глобальных тегов, разметка воспроизведена пере-применением) — не меняется.

**Исходный текст (historical):** Итоговый каталог — **16 глобальных**: **10 builtin** (`is_builtin=true`) + **6 кастомных** (`is_builtin=false`: `Поддержка`, `Билд в коннекте`, `Small Business`, `Билд не дошёл`, `Ждет Ревью 2`, `Билд не приняли` — §10).
...
>
> **Одноразовый ETL-скрипт `backend/scripts/migrate_mail_data.py` — УДАЛЁН** ([ADR-047](ADR-047-mail-fix-pack.md) §1): отработал, повторный прогон невозможен по существу (агрегатор демонтирован; плюс скрипт обращался к дропнутой колонке `is_builtin` и удалённому модулю builtin-тегов). Процедура ETL остаётся нормативно описанной **здесь** (маппинги, идемпотентность, порядок cut-over); сам файл — в истории git.
...
> ✅ **ВЫПОЛНЕНО 2026-07-11 — [ADR-047](ADR-047-mail-fix-pack.md) §0.** Перечисленные ниже правки внесены: `modules/mail/README.md` переписан, mail-раздел `04-api.md` переписан (сняты `MailTeam`/`GET /api/mail/teams`/`group_id`/`mail_group_not_found`; добавлены `/ingest`, `/mailbox-status`, `/me/settings`, `/telegram/*`, компаундный курсор), mail-таблицы добавлены в `03-data-model.md`, security/deployment/design-system/vision синхронизированы, ADR-038/043 помечены `superseded` двусторонне. Остаток: **`teams.mail_group_id` НЕ дропнут** (колонка и поля контракта `/api/teams` живы, для почты мертвы) — ведётся как [TD-051](../100-known-tech-debt.md). Список ниже сохранён как исторический чек-лист.

...
/**
 * Супер-админ из `.env` (ADR-008): у него НЕТ строки в `users`, поэтому нет и личного
 * состояния (`mail_user_settings`, прочитанность писем — ADR-050 §2.5). UI по этому признаку
 * скрывает персональные контролы (индикатор непрочитанного, фильтр «Непрочитанные», кнопка
 * «Отметить непрочитанным»), чтобы он не ловил `403` на каждом клике. Граница — backend.
 */

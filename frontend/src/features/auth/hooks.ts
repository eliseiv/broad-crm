...
/**
 * Мутация входа: при успехе-`false` кладёт токен в стор (память + localStorage,
 * персист 24 ч + мульти-вкладка, ADR-041) и догружает права принципала через
 * GET /api/auth/me (ADR-021, «Гейтинг»). При
...
/**
 * Супер-админ из `.env` (ADR-008). Личное состояние у него ЕСТЬ — системная строка-якорь в
 * `users` (ADR-051 §1): прочитанность писем работает, и гейта по этому признаку в ней НЕТ
 * (ADR-051 §3; норма ADR-050 §2.5 отменена).
 *
 * Единственная оставшаяся супер-админ-развилка в UI — контрол «Уведомления»
 * (`MailNotificationsToggle`): bootstrap-учётке запрещена Telegram-привязка, сервер отдаёт
 * `403` на `/api/mail/me/settings` (ADR-051 §1.6), поэтому контрол скрывается. Новые
 * персональные контролы по этому признаку скрывать НЕ нужно. Граница — всегда backend.
 */

...
interface AddProxyCardProps {
  onClick: () => void;
}

/** Glass/blur карточка «+ Добавить» для прокси (зеркало AddServerCard, 08-design-system.md). */
export function AddProxyCard({ onClick }: AddProxyCardProps) {
  return (
    <button
      type="button"
      onClick={onClick}
    ...
    >
      ...
      <span className="max-w-[220px] text-center text-[13px] text-text-secondary">
        Подключить новый прокси для мониторинга
      </span>
    </button>
  );
}

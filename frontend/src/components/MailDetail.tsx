...
interface MailDetailProps {
   ...
   * обработчик передан. Гейта по `me.is_superadmin` НЕТ (ADR-051 §3): `/mail` передаёт
   * обработчик любому носителю `mail:view`, включая супер-админа. Опциональность нужна
   * read-only-потребителям детали (напр. без права отката), а не супер-админу. Клик не
   * закрывает деталь: письмо остаётся открытым, индикатор в ленте загорается, авто-пометка
   * повторно не срабатывает (её триггер — смена письма).
   */

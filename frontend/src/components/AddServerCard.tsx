...
/** Glass/blur карточка «+ Добавить» (08-design-system.md). */
export function AddServerCard({ onClick }: AddServerCardProps) {
  return (
    <button
      type="button"
      onClick={onClick}
      className="group flex min-h-[280px] w-full flex-col items-center justify-center gap-3 rounded-card border border-dashed border-border-strong bg-surface-1/40 backdrop-blur-md transition-all duration-200 hover:-translate-y-0.5 hover:border-accent hover:bg-surface-1/60 hover:shadow-[0_0_0_1px_var(--accent),0_8px_24px_rgba(99,102,241,0.18)] focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-accent"
    >
      ...
      <span className="max-w-[220px] text-center text-[13px] text-text-secondary">
        Подключить новый сервер к мониторингу
      </span>
    </button>
  );
}

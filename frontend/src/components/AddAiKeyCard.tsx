...
interface AddAiKeyCardProps {
  onClick: () => void;
}

/** Glass/blur карточка «+ Добавить» для AI-ключей (зеркало AddServerCard, 08-design-system.md). */
export function AddAiKeyCard({ onClick }: AddAiKeyCardProps) {
  return (
    <button
      type="button"
      onClick={onClick}
    ...
    >
      ...
      <span className="max-w-[220px] text-center text-[13px] text-text-secondary">
        Подключить новый AI-ключ для мониторинга
      </span>
    </button>
  );
}

...
// Команды канала «Почты» из `/api/auth/me` (ADR-055 §5.1) — источник опций фильтра И
// резолва имени команды в колонке. Admin-уровень: 1 команда + «Без команды» = 2 варианта
// ⇒ порог §6.2 выполнен.
...
// `GET /api/teams` на вкладке «Почты» БОЛЬШЕ НЕ ИСПОЛЬЗУЕТСЯ (ADR-055 §6.3): и опции фильтра,
// и РЕЗОЛВ имени команды в колонке берутся из `me.mail_teams` (`GET /api/auth/me`). Спай
// обязан остаться НЕВЫЗВАННЫМ — иначе у не-админа (без `teams:view`) колонка молча пустела.
...
describe('MailboxesTab — контрол поиска = ui/Combobox (ADR-052 §3)', () => {
  beforeEach(() => loginAs({ isSuperadmin: true, mailTeams: MAIL_TEAMS }));
...
// ============================================================================
// Экран 2 из пяти: `/mail` вкладка «Почты» — фильтр «Команда», порог 2 (ADR-055 §6.2)
// + резолв имени команды в колонке из `me.mail_teams` (§6.3, НЕ из `GET /api/teams`)
// ============================================================================

describe('MailboxesTab — фильтр «Команда»: порог 2 (ADR-055 §6.2, экран 2 из пяти)', () => {
  const SUPPORT = { id: 'team-9', name: 'Поддержка' };

  beforeEach(() => {
    vi.clearAllMocks();
    catalogState.mailboxes = [mailbox()];
    catalogState.isLoading = false;
  });
  ...
  it('2 команды у НЕ-АДМИНА → контрол ЕСТЬ (гейт ADR-050 §1.2 ОТМЕНЁН, TD-058 закрыт)', () => {
    ...
    // Источник — `/api/auth/me`, а НЕ `GET /api/teams` (у оператора нет `teams:view`).
  ...
  it('под admin-уровнем `GET /api/teams` тоже НЕ вызывается (источник один на всех, §6.3)', () => {
...
describe('MailboxesTab — резолв имени команды в колонке (ADR-055 §6.3)', () => {
  beforeEach(() => {
    vi.clearAllMocks();
    catalogState.isLoading = false;
  });
  ...
  it('не-админ БЕЗ `teams:view`: имя команды резолвится из `me.mail_teams` (не «—», не пусто)', () => {
    // Прежде колонка тянула `GET /api/teams` (гейт `teams:view`) ⇒ у mail-оператора список
    // приходил пустым и имя команды молча деградировало. §6.3: источник — `/api/auth/me`.
  ...
  it('`team_id === null` → надпись «Без команды» (отображение, а не опция выбора)', () => {
    ...
    // Ищем именно в ТАБЛИЦЕ: одноимённая ОПЦИЯ фильтра «Без команды» — другой контрол
    // (там это выбор, здесь — отображение факта «у ящика нет команды»).
    const table = screen.getByRole('table');
    expect(within(table).getByText('Без команды')).toBeInTheDocument();
  });
});
